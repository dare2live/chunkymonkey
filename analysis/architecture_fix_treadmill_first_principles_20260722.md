...
> Status: **PLAN**（授权 Phase 0 = 本文件 + `goal.md` 指针；**默认不做**生产重写；Phase 1+ 需 owner 排期）
...
| **1 outcome model**（小代码轮，owner 排期） | 报告 JSON 派生 typed `run_outcome`；wrapper+dispatcher+UI 读它；取代 rc==1 heuristic / `--skip-macos` | owner schedule | 下次 UI 日更：pending_publish 晨间显示 **SOFT_WAITING（非 FAIL）**+ 预期通知条数 | 若仍三连炸 / 仍显 FAIL → 回退到过渡桩，重设计派生规则 |
...
Label: **PLAN — Phase 0 authored（doc + `goal.md` 指针）；Phase 1+ pending owner schedule**. Residual owner: owner（排期 Phase 1 outcome-model 代码轮 / Phase 3 viz MVP）。
