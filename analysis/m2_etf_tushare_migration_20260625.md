...
→ **D repoint 3 消费方 SQL** (etf_engine.calc_etf_momentum/etf_snapshot_manager/etf_mining_engine 读 etf qfq 表 + 验证 vs fund_div 非 mootdx) → **E 物删 mootdx/tx 链 = escalate**。premise 纠正(mootdx/tx 非 akshare)+ mootdx 分红 bug 已通知用户; M3/M4 任务措辞同样需核实际源。
