...
→ **E 物删 mootdx/tx 链 = escalate 用户确认** (不可逆: 退役 sync_etf_universe_and_kline 的 sync_kline 块 + fetch_etf_kline tdxhub/tx 分支 + etf_db 写路 + 物删 etf_price_kline 旧表 + 调和 etf_sync_state 状态面板指 tushare)。premise 纠正(mootdx/tx 非 akshare)+ mootdx 分红 bug + 陈旧 已通知用户; M3/M4 任务措辞同样需核实际源。
