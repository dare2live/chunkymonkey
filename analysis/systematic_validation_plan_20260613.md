...
> goal.md 放精简骨架 + 指针, 本文件是完整 owner。本文件 = 方法论框架 (L0-L4 逐层 gate + 异常核查
> 协议 + 死亡条款), 不绑定具体已废 alpha 裁决 (reset 前/污染期裁决已清, 见 2026-06-17 清验证墓地)。

## 核心命题

"项目的 alpha/特征/live 到底可不可信" 必须**从地基往上逐层用机器可检 gate 回答**, 不是一堆零散
实验。地基 = 喂模型/live 的特征面板 PIT 干净度 — **标签泄漏的隐形变体 = 消费方把标签/前瞻列当特征
喂进面板** (审计面板构建 SQL 抓不到这层, 须查消费侧 feature_cols 与 builder EXCLUDE 的 set 差)。
任一层 gate 未 PASS 即 block 其上所有层; L0 红时
...
| **L4 含成本 paper_sim+forward** | 含 tx_cost/T+1 验是否真达 KPI (年化>=30%/max_dd>=-20%/超额>0/月胜率>=55%) | (a) all_kpi_pass=True (当前态以 doctor --fast 实测为准, 不引文档旧时点); (b) forward 兑现落 paper_sim 区间; (c) 滚动监控+送达告警 | 真金白银上线 | paper_sim_v2 + forward 监控 |

## immediate_next (此刻就能跑, 不依赖重训练)

**L0 第一步 = 消费者泄漏 set 检**: 对每个训练/打分消费者提取实际 feature_cols, 与 builder
`MODEL_INPUT_EXCLUDED_COLS` 做 set 相等检查 (∩ == ∅), 单特征 AUC 层兜住手写漏排。工具:
`leakage_probe --stage feature-consumer`。(reset 前的具体 live 链 sniper / v7 / lambdamart-v6 +
mart_p0a_v5 面板均已删; 重建任一消费者时强制按此 gate 接入, 不手抄 EXCLUDE。)
...
alpha (相对已有特征正交)。当前无系统性 per-域研究 (reset 前零散 alpha 裁决已清, 不引为定论); 在
结构型主升浪 GT + universe 硬门下逐域跑。判据冻结 + leakage_detect 事前闸 (防又拿前瞻列论证)。
...
## 异常核查协议 (用户 2026-06-13: 异常不应直接排除而应核查)

异常高信号 (AUC>0.75 / |RankIC|>0.15 / 相对+50%) = **触发核查, 不是直接排除**。四步:
ablation (剔除看崩多少) → PIT 溯源 (builder lineage 是否 <=t) → 剔除重跑 (落正常带=泄漏源) →
shuffle 对照 (管道级)。**核实泄漏才修; 核实真 edge 则保留 + 新冻结红线重跑 (不复用被看过判据);
绝不因"看着高"丢真实增强**。两个反向都要防: (a) 把泄漏当 alpha 上线 (S3 险些, 核查救回);
(b) 把真 edge 当泄漏误杀 (本协议防)。核查由 leakage_detect ANOMALY_PROTOCOL + 人核共同完成。

## 死亡条款

- 感知死: L0 live 泄漏体检 7 天内不跑 = 计划空转 (live 可能正带泄漏决策)。
- 判断死: 任一层 gate 改判据于看到结果后 = 谄媚死作废。
- 横切: 异常高数字一律走上方核查协议 — 不放宽红线 (谄媚死) 也不直接排除 (误杀真 edge), 而是核查定性。
