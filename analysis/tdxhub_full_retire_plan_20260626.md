...
- [x] 单元6 xdxr热备 (DONE 2026-06-27): 先收口 4 个 main 上红 K线测试(Fix A price_kline_qfq_tushare schema-init缺失=真生产bug已修 + Fix B 删M3退役tdxhub-upsert死测试) → 物删 build_price_kline_tdxhub.py(dead builder 0caller) + mini_market.py(dead fixture) + PRICE_KLINE_TDXHUB_DDL 从 market_schema/db/read 移除(防僵尸) + 撤 update_watermark_sla xdxr SLA + test_real_data删xdxr测试/test_canonical清 + db_lifecycle_delete 物删 price_kline_tdxhub_adjustment_event(735行, market库, archive+deletion_record lifecycle_tdxhub_xdxr_retire_20260627)
- [x] 单元7 server健康 (DONE 2026-06-27): tdx_source server_health DB函数(TDX_SERVER_HEALTH_DDL/ensure/load/record, 仅builder调=dead)标RETIRED不删(与live in-memory circuit-breaker共享helper, surgically删有炸连接池风险; ensure仅builder调故不僵尸) + seed mart_tdx_server_health删 + workbench _relation_exists守卫优雅降级保留 + 物删 mart_tdx_server_health(117行, market库)。验: 0残留 + circuit-breaker完好 + ensure_market_schema无僵尸 + moth PASS45 + data_layer_audit PASS + CI offline 90过。
