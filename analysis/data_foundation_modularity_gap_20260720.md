...
>
> **Amend 2026-07-21（现行真相）**：**S1–S6 transport strangler FIXED**（见 §8）。§0–§4 为 **2026-07-20 诊断当时**快照（fused `capture_and_publish` 路径）；**勿**再当作「模块化编排未交付」的执法输入。近端 residual = **S7 续减 ssot**（inventory 29/46）；运营路径已 caller-only land→accept + derive + serve。
...
| S7 legacy raw plane | **PARTIAL** | derive+form+pipeline clean 默认 accepted-only；daily accepted `20190102`→`20260720`；inventory **29/46 ssot**（SW→PIT；pulse builders→mart incl. top_list；daily_basic→dim；stk_limit→form；stock_basic→dim；adj→qfq）；serve/multi-consumer probe **0 cut**（gate locks leaf COMPAT） |

Residual owner：S7 续（**build** stock-day flow/limit publication / DC PIT / index|seat plane；禁把 pulse mart 当 leaf COMPAT；禁盲删 raw）；ST 不可对称扩至 2019。
...
Next verification：S7 B2 stock-day mart publication or B1 DC PIT；或 E0 org residual；E/F same-protocol remeasure when scheduled（window unblocked）。
