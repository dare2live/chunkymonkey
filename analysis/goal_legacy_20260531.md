...
| Rule 10 | 已在 `safe_commit.sh` 增加 Step 4.5 blocking gate；P0 follow-up 正在补非空 skip reason 与行为测试 | `.py` staged commit 必须被 `Codex-Reviewed: APPROVE/APPROVE_WITH_NOTES` 或 8+ 字符 skip reason 硬约束；`REQUEST_CHANGES` 不可放行 |
| 顶层设计审查 | **APPROVE_WITH_NOTES / 未最终闭环** | `docs/top_level_design_review.md` 已补“二阶审查”，确认流程/功能/模块/数据表/配置文件这套管理方式本身合理；但 freshness/PIT/complexity gate 未全闭环，不能当架构完成证书 |
...
| 库内 artifact/cache 污染审计 | **WARN / 未发现循环日志同型事故**: 只读审计显示 `data/smartmoney.duckdb` 29G，主要由预测/特征/财务面板大表构成；未发现 `reports/reports`、`logs/logs` 递归目录；DuckDB/SQLite 中报告路径命中很少，Optuna SQLite 为标准 study 表；全库仅 1 个 BLOB 列 `raw_tdx_industry_file_snapshot.raw_bytes`，合计约 596KB | 仍发现 `mart_today_signal_cache.signals_json` 单行约 20MB（9286 signals）和 `mart_global_data_quality_gate.evidence_json` 最大约 185KB；这不是循环嵌套日志污染，但暴露 DB summary、明细 payload、外部 artifact 的存储边界不清。后续 P0/P1 增加 artifact/cache storage contract：DB 放索引/摘要/可查询事实，大 payload 归明细表或受控 artifact；清理前必须用 CodeGraph + `rg` 定消费者并补回归 |
| 测试工具可信度 | **PRE-TEST GATE ACTIVE / PASS**: `backend/config/test_tool_registry.yaml` + `backend/scripts/audit_test_tool_health.py` + `docs/test_tool_governance.md` 已覆盖全部 selected test artifacts；root micro-batch 13 已完成 updater 9 个 root tests 与剩余 system/strategy/tdx/v3/utils/conftest/xdxr 等 14 个 root artifacts 的 owner/status/evidence 登记；Rule 10 行为测试已加入；full audit 为 0 FAIL / 0 WARN / 365 selected / 365 registered / 100% registry coverage | 这只证明测试工具 registry 与默认/opt-in scope 对齐，不证明业务数据或策略有效；updater targeted pytest 89 passed / 2 warnings；剩余 14 个 root artifacts 默认 pytest 76 passed / 63 deselected / 15 warnings；`system_routes` 与 `v3_*` route smoke 已标 `realdb` opt-in，realdb collect-only 63/66 collected，未执行真实库测试；`test_tdx_source.py` 仍有 `datetime.utcnow()` deprecation 小债 |
...
| 画像 / 股票档案 | **PLAN / gated**: `docs/profile_lineage_roadmap.md` 已新增 | 股票画像、机构画像、主力行为画像和前端股票档案必须先走数据需求契约 + lineage/freshness contract；前端不能先把 `unknown/proxy/stale` 包装成生产证据 |
...
| main 工作区 | 183 dirty entries / `scripts/chunkyctl worktree --format markdown` 已可读分桶 / unknown=0 | 不可盲目 `git add .`；当前分桶: audit_gate_scripts 22, backend_services_api 29, config_project 4, controller_state 9, data_source_lineage_profiles 8, docs_archive_moves 6, pipeline_build_scripts 14, project_docs 11, startup_tooling 10, tests 36, universe_governance 4, updater_split 30；后续按 P0 controller_state/startup_tooling/config_project/universe_governance/data_source_lineage_profiles/docs_archive_moves -> P1 updater_split/audit_gate_scripts/backend_services_api/pipeline_build_scripts/tests -> P2 project_docs 顺序清理/验证 |
| CodeGraph 索引状态 | `scripts/chunkyctl audit --run ...` 本轮已执行 `codegraph sync .`；`codegraph status .` 仍显示 49 Added pending；`scripts/chunkyctl doctor --fast` 已解释 pending_added=49 正好匹配 untracked indexable `.py/.js/.jsx` files=49 | 当前大量 untracked 架构拆分文件会让 status 继续提示 pending；不能用强行 stage/commit 掩盖，后续继续把 sync 与 complexity 成对执行并在验收中报告 |
| 工具更新 | **LOCAL WRAPPER FIRST / PROJECT ASSISTANT SEED**: `audit_tooling_gate.py` / `chunkyctl doctor` 当前显示 183 dirty entries、CodeGraph pending 49；无 baseline 时 complexity diff 已改为 `baseline_unavailable`，只报 `unclassified_high_count`，不把历史 HIGH 误标为新增回归；已生成本地忽略基线 `data/reports/tooling/complexity_baseline.json`（capped 40 findings），`doctor --fast` 默认加载后显示 `diff_status=compared`、`new_high_count=0`、`unchanged_count=40`；`chunkyctl preflight` 已修复 `build` 命中 `ui` 子串的 frontend 假阳性；`chunkyctl worktree` 已支持 JSON + `--format markdown`，dirty 分桶 unknown=0；`chunkyctl audit` 已修复把 `backend/*.yaml` 误当 Python scope 的 py_compile/complexity 误判；`docs/chunkyctl_session_quickstart.md` 已明确为启动契约 | GitHub 调研后暂不引入 Worktrunk/lazygit/delta/git-absorb/git-town/git-branchless；先把 dirty/codegraph/complexity 机器化，再逐步收束成 `chunkyctl` 类项目专用审计/开发辅助入口；后续任何 session 启动模式、gate、工具入口或 controller/agent 工作法变化，必须同批更新 quickstart，不允许只留在对话或 handoff |
| `docs_archive_moves` | **APPROVE_WITH_NOTES**: `DATA_INTEGRITY_AUDIT_20260517.md`、`PLAN_V3.md`、`市场感知开发计划.md` 已逐字迁到 `analysis/` 归档；subagent 只读审计确认 SHA/行数/字节数一致，`diff -u` 无输出 | 后续 staging 时三份 root 删除和三份 `analysis/` 归档必须同组处理；旧 root 名称剩余命中为 Docs Map、迁移记录、归档正文标题或 test fixture，不是活跃入口 |
...
| `docs/implementation_plan.md` | 已同步为当前架构计划 | 继续以本节和该文件为执行入口 |
...
| 优先级 | 文档 | 用途 |
|---:|---|---|
| 1 | `docs/PROJECT_CONSTITUTION.md` | 最高规则: 真相源、分层、gate、完成标准 |
| 2 | `docs/top_level_design_review.md` | 流程/功能/模块/数据表/配置文件的第一性原理 + 奥卡姆审查门禁 |
| 3 | `docs/profile_lineage_roadmap.md` | 数据血缘、股票画像、机构画像、主力行为画像、股票档案前端的分阶段路线 |
| 4 | `docs/test_tool_governance.md` | 测试工具 registry、状态流转、过期测试审计和清理顺序 |
| 5 | `docs/agent_parallel_execution_policy.md` | 并发 agents 的长期流程规则与 controller 边界 |
| 6 | `analysis/handoff_20260527.md` | 当前执行任务、剩余违规、验收标准 |
| 7 | `analysis/codex_bootstrap_20260527.md` | 操作手册、命令、gate、红线 |
| 8 | `docs/architecture_reform_context.md` | 300616 哨兵案例和架构改革原因 |
| 9 | `goal.md` 本节 | 当前执行计划和状态账本 |
| 10 | `docs/implementation_plan.md` | 正式计划文档，已同步为架构优先口径 |
...
| 顺序 | 优先级 | 内容 | 依赖 / Gate |
|---:|---|---|---|
| 1 | P0 | 数据需求契约与血缘先行: 新画像字段先登记 need、grain、PIT key、freshness、consumer | `tdx_data_need_coverage.yaml`, `audit_tdx_data_need_coverage.py`, `docs/data_lineage_spec.md` |
...
详细路线见 `docs/profile_lineage_roadmap.md`。该路线不改变当前“先架构后业务”的优先级: P0 仍是治理、universe、Rule 10、数据契约、复杂度和 updater 边界；全局前端重设计放在主线验证之后。

### P0-P3 实施计划

| 阶段 | 优先级 | 目标 | 主要动作 | 验收 |
|---:|---|---|---|---|
| 0 | P0 | 冻结现场和 scope | `git status --short`; 分类 tracked dirty / untracked; 不 stage data parquet / skill 目录 / 无关文档 | 本轮要动的文件列表明确 |
...
| 4.5 | P0/P1 | 顶层设计审查制度化 | `docs/top_level_design_review.md` 已新增，明确流程/功能/模块/表/配置的 owner、truth source、gate、奥卡姆审查 | 后续架构变更能先填审查模板，避免新增表/配置/模块无主膨胀 |
...
| 4.8 | P0/P1 | 画像与股票档案路线契约化 | 新增 `docs/profile_lineage_roadmap.md`；把股票画像、机构画像、主力行为画像、股票档案前端按 lineage -> profile contract -> mart/service -> API -> frontend 排序 | 任何画像/前端展示必须携带 source、PIT/freshness、`unknown/proxy/production` 状态和 lineage_ref |
...
| 6.6 | P0/P1 | 工具门禁 JSON 化 | 新增 `backend/scripts/audit_tooling_gate.py`：解析 git status、CodeGraph status、complexity markdown，输出 baseline/diff；无 baseline 时输出 `baseline_unavailable` + `unclassified_high_count`，加载 baseline 后才输出有效 `new_high_count`；`chunkyctl doctor` 默认加载本地忽略文件 `data/reports/tooling/complexity_baseline.json`；2026-05-31 已生成 capped baseline，`doctor --fast` 证实 baseline loaded / `new_high_count=0` / `unchanged_count=40`；`chunkyctl preflight` 改为 token 匹配 task risk，避免 `build` 误触发 `ui` frontend gate；新增 `docs/tooling_update_review_20260527.md` 记录 GitHub 工具调研 | dirty worktree 不再只靠人工读长列表；历史 HIGH 和新增 HIGH 可分开治理，且不会因缺 baseline 把历史债误报成新增回归 |
| 6.7 | P0/P1 | 项目专用审计/开发辅助工具雏形 | 新增 `backend/scripts/chunkyctl.py` + `scripts/chunkyctl` + `docs/chunkyctl_session_quickstart.md`，以 `audit_tooling_gate.py` 为 seed，保留 `doctor/worktree/preflight/audit` 四个最小入口；规则来自 AGENTS/goal/skills/config，不做黑箱大 prompt；`worktree` 默认 JSON，`--format markdown` 给 controller/agent 人读审查；`audit` 只对 `.py` scope 跑 py_compile/complexity，避免 YAML/INI 等配置误入 Python gate；quickstart 已写明启动契约维护规则 | 新 session 默认只需按 `docs/chunkyctl_session_quickstart.md` 接手并跑 `scripts/chunkyctl doctor --fast`；doctor 已内嵌 worktree bucket summary 与 CodeGraph pending/untracked reconciliation，dirty 时再跑 `scripts/chunkyctl worktree --format markdown` 和 `scripts/chunkyctl worktree --bucket <name> --format markdown` 下钻；当前 183 entries 已分 12 桶且 unknown=0；入口语不绑定架构优化阶段，后续按当前用户请求、`goal.md` 和最新 handoff 继续；各 session 用同一工具拿事实和 gate，再由总控评审方向；后续增删改文档、优化工具或改变 gate/agent 启动流程时，必须同批更新 quickstart 并在 handoff/final 中说明 |
| 6.8 | P0/P1 | 文档归档 bucket 审计 | `docs_archive_moves` 三份 root 历史文档已迁入 `analysis/`，controller + subagent 均验证旧 root 与新归档逐字一致；活跃代码注释已从裸 `PLAN_V3` 指向 `analysis/plan_v3_20260514_archived.md`；`docs/README.md` 已升级为文档索引/生命周期规则；一次性 RCA `docs/cron_automation_breakage_rca_20260529.md` 已移到 `analysis/`；旧 Codex prompt `docs/market_perception_codex_prompt.md` 经 `rg` 验证无外部引用后删除 | staging 时三份 root 删除和三份 `analysis/` 归档必须同组处理；`cron_automation_breakage_rca_20260529.md` 作为 analysis evidence 处理；旧 prompt 不再保留，后续 session 只使用 `docs/chunkyctl_session_quickstart.md` |
| 6.9 | P0/P1 | artifact/cache storage 治理 | 已按 lifehack 类似事故做本项目只读排查：未发现循环嵌套日志目录或报告路径批量入库；发现 `mart_today_signal_cache.signals_json` 20MB 单行大 JSON，需要从“整包缓存入库”改为 summary/index + 明细表或外部 artifact 的明确边界 | 先用 CodeGraph/`rg` 锁定 `signals_v2`、`routers/signals.py`、Workbench 读模型消费者；新增只读审计阈值（大 TEXT/BLOB、路径入库、报告/日志表膨胀）；再做兼容迁移与回归，禁止直接删生产缓存造成 API 空响应 |
...
| 动作 | 命令/方法 | 注意 |
|---|---|---|
| 查看 dirty | `scripts/chunkyctl doctor --fast` + `scripts/chunkyctl worktree --format markdown` | 以实时 gate 为准；最新快照 183 dirty entries = 112 modified + 3 deleted + 68 untracked，不能覆盖 |
...
- 2026-05-27 Codex 更新: 并发 agents 默认用于非阻塞只读探索；worker 只用于完全不重叠写入范围。CodeGraph 与 complexity 必须成对使用，agent 改 Python 后仍由主控跑 `codegraph sync .`、targeted tests、complexity scan 和文档落账。
- 2026-05-27 工作模式固化: Codex 默认作为 controller/架构师/验收人，负责方向、任务拆解、红线、合并和 gate verdict；agents 负责明确 scope 内的具体执行。agent 输出只是候选，未经过 controller review、最终测试、CodeGraph/complexity 和文档落账前，不算项目事实。
- 2026-05-27 删除治理补充: 经 CodeGraph + `rg` + targeted tests/audits 证明可删的代码、测试、文档、配置必须直接删除；禁止用注释、隐藏 flag、改名、dead branch、空壳 stub 或“留待以后”伪装清理。证据不足时只能保持 active/quarantined 并写明缺口。
