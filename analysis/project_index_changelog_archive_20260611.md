# PROJECT_INDEX Changelog Archive (cut 2026-06-11)

> 来源: PROJECT_INDEX.md 顶部 [INDEX] 增量区 + §14 Session 增量日志 (246 条)。
> 此后历史叙事唯一 owner = analysis/project_state_ledger.md; INDEX 只留活索引。

## 顶部 [INDEX] 增量区

## [INDEX] 2026-06-04 增量

- **Provider-neutral execution contract + retired GCP surface removal**: 项目内旧 GCP 执行面已删除，不再作为“谨慎使用”的活跃路径。删除范围包括 `gcp/`、`scripts/gcp_*`、phase5 monitor、cost tracker、GCS sync、`backend/config/gcp_policy.yaml`、旧 guard tests 和失效 launchd plist；`~/Library/LaunchAgents` 中对应 label 当前 absent。新入口是 `backend/config/experiment_jobs.yaml`、`backend/services/experiment_jobs.py` 和 `scripts/chunkyctl jobs`；当前 `local` backend active，`modal` planned/blocked。`backend/scripts/audit_execution_surface.py --include-live-launchd` 是删除/退役脚本、provider、cron、launchd、installer、dashboard、registry、Moth evidence path 后的必跑审计，当前 PASS。
- **After-close data-health refresh complete**: 2026-06-04 盘后已补 K-line / HS300 / feature-panel tail / feature prune，并串行刷新 holder/F10、GPCW、capital 相关 blocking slices。`data_health_snapshot.py --dry-run --format text` 当前为 `green=326 / yellow=16 / red=0 / blocking_yellow=0`；剩余黄色是 warning-quality 资产，不再是 startup blocker。
- **2026-06-05 data-health SLA rollover + refresh**: 2026-06-05 08:39 CST 复跑 `doctor --fast` 后，freshness SLA 滚动使 data-health 变为 `green=321 / yellow=21 / red=0 / blocking_yellow=4`；08:50 CST 已用窄写窗口刷新 financial PIT tail、technical stage tail、picture daily 2026-06-04、feature drift histogram baseline。复验 `doctor --fast`: `green=325 / yellow=17 / red=0 / blocking_yellow=0`，剩余 yellow 是 warning-only writer/SLA debt；feature drift 新 snapshot 有 `critical=3`，是模型漂移风险信号，不是 data-health freshness blocker。
- **Holder/F10 replace replay safety**: `backend/scripts/ingest_holders_tdxhub.py` 的 `--replace-facts` replay 改为先解析成功 raw，再按 raw key 批量删除 holder/plan/trade/controlling 旧 facts，避免解析失败先删旧 facts；同时恢复 replay 相关索引，并在有 `availability_source` 列时直接插入该字段。回归覆盖 force refresh、replace replay、stale controlling 清理、parse failure 保留旧 facts、index restore 和 availability_source direct insert。
...
- **Design-review preflight gate**: `backend/scripts/chunkyctl.py preflight` 现在输出 `design_review_gate`，把 first-principles、Occam、owner、truth-source、failure-mode 和 drift-blocking gate 从 `docs/engineering_governance.md` 提升为机器可读启动字段。Moth 只暴露 evidence paths / instruction-source boundary，不承载业务 gate 规则。
- **Architect verify-verifier rule**: `/Users/dp/.codex/skills/architect-controller/SKILL.md` 已加入 “verify the verifier” 规则：当门禁结果异常干净或异常嘈杂时，先用精确 scope 和最小反例验证工具本身。同步修复 `/Users/dp/.agents/skills/complexity-optimizer/scripts/analyze_complexity.py` 的单文件扫描 false-negative；旧单文件 complexity “clean” 证据遇到异常时需要复验。
- **Moth complexity path normalization + baseline refresh**: Moth 不再用顶层 path root 猜测 baseline 不兼容；loaded baseline 会正常 compare，且 repo 内绝对路径会归一到相对路径，避免同源 finding 漂移。本机 ignored baseline `data/reports/tooling/complexity_baseline.json` 已刷新到当前全仓 scanner scope（80 条 `assets` findings），所以当前 live Moth snapshot 应为 `complexity.diff.status=compared` / `new_high_count=0`。该 baseline 不是 Git 资产；scanner scope 变化时要刷新同 scope baseline。
- **Codex instruction-source boundary**: `.moth/profile.yaml` 与 Moth registry profile 都暴露 `instruction_sources`，`chunkyctl preflight` 读取 Moth profile 后输出 `ignored_by_default=["CLAUDE.md"]`。Codex 默认使用 `AGENTS.md`、active docs、Codex skills、Moth evidence paths 和 live tooling output。Moth 最新提交 `dcb809a fix: sync ChunkyMonkey instruction sources` 已推送到 PUBLIC repo `dare2live/moth`，本机 `/Users/dp/.local/bin/moth` 指向该 repo venv。
...
- **Data-health dry-run is read-only**: `backend/scripts/data_health_snapshot.py --dry-run` 现在通过 read-only DuckDB 连接打开生产库并跳过 schema DDL，避免 startup/doctor dry-run 因写锁失败；`deprecation_status` / `replacement_table` 在旧 schema 下作为 optional cols 读取。最新 dry-run 为 `green=325 / yellow=17 / red=0`，yellow 仍是 warning-only 数据债。
- **Retention policy contract**: `backend/config/storage_retention.yaml` 的 `table_inventory` 已从 owner/action 分类升级为 owner + `db_alias` + truth source + consumers + delete gates + rollback evidence + compaction policy；`backend/services/storage_retention.py` 输出 `policy_contract`，并要求 every executable cleanup candidate 都匹配 inventory contract。当前生产库只读 dry-run 为 `candidate_count=0 / table_inventory_count=12 / policy_contract=FAIL on unknown_consumer_proof / compaction.recommended=false`，这是预期的保护状态：obsolete/cache entries 仍要先用 CodeGraph/`rg` 证明 no-live-consumer，再去 copied DuckDB 验证；不执行生产删除、VACUUM 或 compact。只读 sidecar 复核确认容量主因仍是 `smartmoney.duckdb` 内多版本宽面板/cache 并存；未发现能解释容量的 `.duckdb.bak/.gz/.zst` 或 snapshot-loop 写爆。
- **Storage payload cap recalibration**: `backend/config/storage_retention.yaml` 将 `fact_technical_trigger.reason_codes_json` 与 `mart_macd_state_history.reason_codes_json` 重新标定为 full-history evidence cap，并把 `mart_stock_picture_daily.institution_top_json` 纳入 `picture_daily` owner 的 bounded daily picture summary。`backend/tests/scripts/test_audit_storage_payloads.py` 锁住 default policy、picture reviewed rule 和既有 recursive/single-cap guard。真实库只读审计当前为 `323 columns / 0 FAIL / 0 WARN / 13 reviewed PASS`；这不是清表/compact，future recursive key、path marker、单行或总量超 cap 仍会重新 WARN/FAIL。
...
- **调度层 cron→launchd 迁移 + 失败告警送达 (2026-06-11)**: 体检确认 daily_update/nightly_audit 两条 cron 自 6 月起每天静默失败 (cron 无 FDA 读不了 `~/Documents`, crontab 头注释"cron 不受 FDA 限制"在本机为假), K 线断流 4+ 交易日无人知晓 = `feedback-data-sync-silent-failure` 教训复发。修复: (1) launchd TCC 实测矩阵 — bash 内联 `-c` OK / bash 执行 Documents 脚本 126 / bash 执行 `~/Library` 下转发脚本仍 126 (exec 链内层被拦) / **python3.13 (已有 FDA) 做 ProgramArguments[0] 再 spawn bash 全链 OK** (TCC 身份取入口二进制, 子进程继承); (2) 新增 `scripts/launchd_job_wrapper.py` 统一 job 入口 — 失败写 `/tmp/chunkymonkey_ALERT_<job>.flag` + macOS 通知, 成功清 flag, 决不静默; (3) `configs/launchd/*.plist` 入口改 `.venv/bin/python + wrapper`, 已 bootstrap 进 LaunchAgents 并 kickstart 实测 (nightly audit 正确产生 critical FAIL 告警 — K线滞后判定); (4) crontab 中两条 chunkymonkey 任务退役 (备份 /tmp/cron_backup_*). 防回退: 不要把 plist 入口改回 bash; 新增定时任务必须走 wrapper。
- **TuShare vendor-gateway 接入 + need_027 gate PASS (2026-06-11)**: 项目新增 `.venv` (`--system-site-packages`, 已入 `.gitignore`) 安装 tushare 1.4.29；`backend/services/data_sources/sources/tushare.py` 新增 `_pro_api()` 支持 `TUSHARE_HTTP_URL` 代理网关覆盖（token 与 gateway URL 只放本地 `.env`，被 `.gitignore` 覆盖，不进 repo/config/report）。`probe_source_capability.py` 的 `TUSHARE_TOKEN_ENV_VARS` 改为从 adapter import `TOKEN_ENV_VARS`（消除双真相源漂移），`_source_preflight` 的 `find_spec` 加 `ValueError`/`ImportError` 防护（sys.modules stub `__spec__=None` 不再崩 gate）。新增三个防回退测试：preflight-veto 分歧（live probe 100% ok + preflight blocked → group 仍 blocked，固化 anti-fake-PASS 语义）、find_spec 异常降级、`backend/requirements.txt` → `startup_tooling` bucket 契约（其他依赖清单路径仍落 `unknown`）。live gate 实测 **PASS**：tushare 组 3/3、`selected_source=tushare`，AkShare 组仍 `akshare_remote_disconnected`；代理网关实测 19/20 接口可用但有间歇 ~15s 空响应（writer 必须把 0 行当失败重试）。剩余 production gate：`pit_key`/`freshness_sla`/`writer`/`watermark`/`failure_queue_resolution`（见 `docs/implementation_plan_20260611.md` Phase 1）。验证：targeted pytest `89 passed`（probe/chunkyctl 64 + data-source 邻接 25）。
- **TuShare no-persist exact-flow probe wiring**: `backend/services/data_sources/sources/tushare.py` 新增 TuShare Pro adapter，登记 `moneyflow` / `moneyflow_dc` / `moneyflow_ths`；token 只从 `TUSHARE_TOKEN` / `TUSHARE_PRO_TOKEN` / `TS_TOKEN` 环境变量读，不写入 config 或 probe report。`backend/config/tdx_data_need_coverage.yaml` 为 `need_027` 加入三只 TuShare `moneyflow` 样本，`probe_source_capability.py --need027-exact-flow-gate` 现在把 AkShare `individual_fund_flow` 与 TuShare `moneyflow*` 按 `exact_flow.source_groups` 分组评估，避免候选源缺 token 反向否定另一个可用 exact source。当前 live no-persist gate 仍 `BLOCKED`：`6` probes / `0` valid，AkShare 组为 Eastmoney `RemoteDisconnected`，TuShare 组为缺 env token；`production_eligibility` 仍 `blocked`，没有 writer、DB 持久化或 failure-queue resolve。验证：targeted pytest `52 passed`，scoped `audit_test_tool_health.py` PASS。
- **need_027 probe diagnostics hardening**: `backend/scripts/probe_source_capability.py --need027-exact-flow-gate` 现在把 generic `probe_blocked` 拆成 controller-readable blockers：AkShare 远端断连为 `akshare_remote_disconnected`，TuShare 缺 token 为 `tushare_token_missing`，字段/日期/row-count/rank snapshot ignored 也分别有 `controller_blockers` 和 `next_action`。报告新增 `post_probe_gates`，明确 `field_mapping` / `date_coverage` 只能来自 selected no-persist source group，`pit_key` / `freshness_sla` / `writer` / `watermark` / `failure_queue_resolution` 仍 required 或 not_checked，`production_eligibility` 保持 `blocked`。新增 `need027_source_probe_gate_contract_tests` registry entry，标明该测试只证明 mock/no-persist gate contract，不证明 live source、token quota、PIT、freshness 或 writer。最新 live no-persist 摘要仍 `BLOCKED`: `6` probes / `0` valid，AkShare `akshare_remote_disconnected=3`，TuShare `tushare_token_missing=3`，全部 post-probe gates `not_checked`。验证：test-tool audit PASS，targeted pytest `53 passed`，CodeGraph up to date，complexity `new_high_count=0`，Rule 10 reviewer `APPROVE_WITH_NOTES`。
- **need_027 exact-flow probe gate**: `backend/config/tdx_data_need_coverage.yaml` 的 `need_027.source_probe_cases` 现在覆盖 AkShare `individual_fund_flow` 三股样本和 TuShare `moneyflow` 三股样本；`backend/scripts/probe_source_capability.py --need027-exact-flow-gate` 默认 no-persist，case-level `persist_status` 会被拒绝，只有顶层 `--persist-status` 才写 `mart_data_source_failure_queue`。gate 先校验 exact capability、row/date range、主力/超大/大/中/小单字段，再按 source group 决定 source-probe 是否有可用 exact source；malformed exact-flow 只能保持/open blocker，rank snapshot 仍是 research-side，persistence domain 为 `stock_fund_flow_rank_snapshot`，不能误清 `order_flow_fund_flow`。2026-06-06 live no-persist gate 仍 `BLOCKED`，AkShare 组 `RemoteDisconnected`，TuShare 组缺 env token，所以 `need_027` 仍为 production blocker。
- **Data-source capability router contract**: 数据源选择不再按单一 vendor primary/fallback 思考，而是按 data-need/capability 粒度路由：`tdxhub` 保持 K-line/xdxr/GPCW/F10/holder/local CYQ backbone；iFinD trial/personal 进入 industry-chain、concept/sector PIT snapshot、news/notice、EDB 研究/小批量 snapshot 候选；TuShare 2000 优先用于 `need_027` exact-flow 低成本 no-persist probe，5000/6000/8000+ 只在具体 capability 通过样本价值证明后再考虑。`backend/config/tdx_data_need_coverage.yaml` 现在为 `need_027.candidate_sources` 记录 TuShare/AkShare/TDX-xmtdx candidates，全部 `evidence_status=unknown` / `production_eligibility=blocked`，并以结构化 `required_validation` 明确 field/date/PIT/freshness/watermark/reconciliation/failure-queue 前置条件；`audit_tdx_data_need_coverage.py --summary-only` 会把这些 metadata 带入 blocked summary。该切片只更新 contract 和研究证据，不加 writer、不写 DB、不 resolve `need_027`。
- **Stage-opt candidate supply/readiness/schema contract**: `backend/config/stage_opt_candidate_supply.yaml` + `backend/services/stage_opt_candidate_supply.py` now own stage-opt upstream supply source/readiness/schema contract：`fact_technical_trigger` 是 PIT trade-trigger source，`mart_macd_state_history` 是 MACD diagnostic state-history source，allowed stage bins、`readiness.min_signals_per_key=5`、research-challenger formula scope override、source `required_columns`、and join `source_columns`/`target_columns`/`required_columns` 不再藏在 audit 脚本常量里。`audit_stage_opt_candidate_supply.py` 输出 `schema_version=1`、`candidate_supply_contract`、`source_schema_checks`、`source_schema_errors` 和 `source_load_errors`；schema failure 会在 row load 前短路为 structural `WARN` / `candidate_supply_source_schema`，避免被误判成普通 0 行或 `below_min_signals`。`chunkyctl doctor` 会透传 contract summary、`source_schema_errors`、`source_load_errors`、`summary.source_schema_error_count` 和 `summary.source_load_error_count`。这个切片只固化 truth-source / eligibility / grain / consumer / readiness / schema 边界，不跑 writer、不迁表、不调阈值，所以 stage-opt 仍是 `P1 / upstream_candidate_supply` blocker。2026-06-06 短 live audit `2026-06-01..2026-06-05` 为 `WARN`: `min_signals=5` from config, `source_schema_error_count=0`, `source_load_error_count=0`, `12155` keys blocked by `below_min_signals`。
- **Stage-opt signal-date K-line coverage evidence**: `audit_stage_opt_candidate_supply.py` 现在把 `v_price_kline_qfq` 按 `(stock_code, date)` join 进 trigger/state rows，并把 `has_kline_bar` 带入每个 signal row。candidate key 仍是 `(stock_code, formula_id, formula_variant, stage_bin)`，但 readiness 改为 `kline_signal_rows >= min_signals`；partial signal-date 缺口会显示为 `missing_signal_kline_bars`，无任何匹配仍是 `no_kline_bars`。`signal_rows_with_bars` / `signal_rows_without_bars` / `signal_kline_coverage_pct` 现在进入 attrition funnel、markdown、sensitivity 和 `chunkyctl doctor` summary。短 live audit `2026-06-01..2026-06-05` 为 `signal_kline_coverage_pct=100.0` / `signal_rows_without_bars=0`；full doctor 为 `signal_kline_coverage_pct=100.0` / `below_min_signals=90331`，所以当前 blocker 仍是 upstream formula coverage / signal density，不是 K-line coverage。
- **Stage-opt source-aware density diagnostics**: `audit_stage_opt_candidate_supply.py` 现在把 `source_id` 带入 trigger/state signal rows，并输出 `blocked_matrix_by_source_registry_family_stage` / `top_blocked_source_registry_family_stage_cells`；每个 source cell 的 `kline_signal_row_buckets` 按 source-specific kline row count 统计，避免混合 `fact_technical_trigger` + `mart_macd_state_history` 同一 key 时把 combined key count 误归因到每个 source。短 live audit `2026-06-01..2026-06-05` 仍 `WARN` / `below_min_signals=12155` / `signal_kline_coverage_pct=100.0`，新增 top source cell 指向 `fact_technical_trigger × live × reversal × stage 4`，`keys_blocked=3385`，bucket 只有 `1=1202 / 2=2183`，进一步确认下一步是 upstream source/formula density redesign，不是调 `min_signals` 或追 K-line。验证：test-tool audit PASS，targeted pytest `64 passed`，live short audit PASS-as-WARN evidence，`scripts/chunkyctl audit --run ...` PASS，Rule 10 reviewer `APPROVE`。
- **Validation**: `py_compile` PASS；`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope stage_opt_candidate_supply_contract_tests` PASS；current targeted pytest for stage-opt audit + `chunkyctl` passed (`56 passed`)；short live read-only audit `2026-06-01..2026-06-05` returned `WARN / source_schema_error_count=0 / source_load_error_count=0 / unique_keys=12155 / ready_keys=0 / signal_kline_coverage_pct=100.0 / blocked_reason_counts={"below_min_signals": 12155}`；full `scripts/chunkyctl doctor --fast` still `WARN` with stage-opt `signal_kline_coverage_pct=100.0` and `stage_focus=upstream_candidate_supply`; CodeGraph synced and status up to date；Moth complexity diff `new_high_count=0`；`git diff --check` PASS；Rule 10 reviewer `APPROVE_WITH_NOTES`.

## [INDEX] 2026-06-03 增量

- **Technical-stage residual policy governed + production rebuild**: `backend/config/technical_stage.yaml` 新增 `residual_above_mid_stage` / `residual_below_mid_down_stage` / `residual_below_mid_other_stage`，residual label 只允许 `1/3/4`，Stage `1.5` / `2` 仍只能由显式突破/上升趋势规则产生；`backend/services/formula_engine/technical_stage.py` 将足量历史但未命中显式规则的 residual 状态收成 `_classify_residual_stage()`，避免 `unknown` 污染 stage-opt，同时不再用 fallback 绕过 Stage 2 的 MA 顺序/回撤约束。代码提交 `ebd18209` 后已本地重建生产表：`fact_stock_technical_stage` 为 `3,973,319` rows (`2023-01-12 -> 2026-06-02`)，`fact_signal_context` 为 `4,093,116` rows (`2023-01-03 -> 2026-06-02`)。最新 full-history stage-opt audit 仍 `WARN`，但 `ready_coverage_pct` 从 `67.76` 提到 `74.81`，`ready_keys=268,198`，`dropped_unknown_stage_rows` 从 `4,433,034` 降到 `1,231,858`，`blocked_keys` 从 `101,824` 降到 `90,331`；剩余 blocker 仍全是 `below_min_signals`，后续继续 `P1 / upstream_candidate_supply`。验证：test-tool audit PASS，targeted stage/signal-context tests 14 passed，stage-opt audit tests 10 passed，`scripts/chunkyctl audit --run backend/config/technical_stage.yaml backend/services/formula_engine/technical_stage.py backend/tests/test_formula_engine.py` PASS，CodeGraph synced，complexity scan clean，`git diff --check` PASS，`scripts/chunkyctl doctor --fast` 仍 WARN 但 `data_health/universe/worktree` 均 PASS。

- **Stage-opt attrition evidence matrix**: `backend/scripts/audit_stage_opt_candidate_supply.py` 现在输出 advisory `verdict`、`attrition_funnel`、`formula_attrition`、`formula_family_attrition`、`blocked_matrix_by_stage_formula`、`top_blocked_stage_formula_cells`、`blocked_matrix_by_registry_family`、`top_blocked_registry_family_cells`，把 raw -> filtered -> unique -> blocked -> ready 损耗、`stage_bin × formula_id` blocker、`registry_scope × formula_family` blocker 明确暴露出来；`backend/scripts/chunkyctl.py` 的 doctor summary 会透传 top cells，`docs/chunkyctl_session_quickstart.md` 已澄清 stage-opt 现在没有安全的策略/阈值 knob 切片，但证据工具切片可以做。`goal.md` 同时修正 `need_027`：单只探测曾成功不等于生产恢复，2026-06-03 15:38 CST 只读 probe 又出现 `RemoteDisconnected`，后续应做小批量 exact-flow probe gate。验证：`PYTHONPATH=backend python -m pytest -q backend/tests/scripts/test_audit_stage_opt_candidate_supply.py backend/tests/scripts/test_chunkyctl.py` 37 passed；full-history `audit_stage_opt_candidate_supply.py --format json` 单独运行约 48.9s，输出 `verdict=WARN / raw_rows=9,159,813 / blocked_keys=101,824 / dropped_unknown_stage_rows=4,433,034`；scoped `audit_stage_opt_candidate_supply.py --formula ma_base_breakout gs_pullback_confirm volume_base_breakout --stock-codes 000001 300750 600519 --format json` 输出新增矩阵且 `verdict=WARN`；test-tool audit PASS；targeted complexity scan 无明显热点。

- **Stage-opt limited-sample verdict and sensitivity cost fix**: Rule 10 review 先抓到 `--limit-stocks` 会把 `signal_rows` 截成样本内股票，但 `dropped_unknown_stage_rows` 仍来自全量加载；修复后又抓到纯 unknown-stage 切片会因为从 surviving `signal_rows` 选样本而被清成 empty `PASS`，以及 sensitivity 重算完整 attrition 矩阵会把 doctor 路径拖慢。`audit_stage_opt_candidate_supply.py` 现在从 raw 非指数 rows 选择 limited sample，再按 allowed stock codes 同步过滤 raw/drop counters；纯 unknown-stage 样本仍保留 drop 计数并 `WARN`，且 recommendation 改成 `P1 / stage_context_coverage`，不再显示 `P2 / no blocking reasons`。`min_signals_sensitivity` 改为单次 key counter 计算，不再重复构建 full detail matrices。验证：`PYTHONPATH=backend python -m pytest -q backend/tests/scripts/test_audit_stage_opt_candidate_supply.py backend/tests/scripts/test_chunkyctl.py` 37 passed；`--start 2026-06-01 --end 2026-06-02 --limit-stocks 3` smoke 现在 `verdict=WARN / raw_rows=12 / dropped_unknown_stage_rows=12 / focus=stage_context_coverage`，完整短窗口为 `WARN / raw_rows=16,740 / dropped_unknown_stage_rows=16,730`。

- **Codex local ops + Moth profile governance**: Codex Mac app / CLI 本地故障处理已固化到 `/Users/dp/.codex/skills/codex-local-ops`，覆盖前端 compact 报错、plugin startup sync 429、worktree 清理、Terminal mail、启动项/cron/LaunchAgent/GCP monitor residue、以及默认并行 controller 模式；全局 `/Users/dp/.codex/AGENTS.md` 已把这类配置任务指向该 skill。ChunkyMonkey 侧新增 `.moth/profile.yaml`，`backend/services/moth_snapshot.py` 会优先解析匹配的 repo-local profile，`chunkyctl doctor` 改用 read-only `audit_tdx_data_need_coverage.py --summary-only`，`.moth/` 归入 `startup_tooling` dirty bucket。职责边界：Moth 只拥有共享 tooling metadata / evidence paths / dirty/codegraph/complexity 快照，`stage_opt` / `need_027` / `storage_payload` / `data_health` 仍由 repo audit scripts 和 `chunkyctl` 判定。验证：skill quick_validate PASS，plugin startup probe 复现 app-server 启动 git sync 调用且确认只有 `features.plugins=false` 会阻断该类远程同步但会禁用插件系统；targeted pytest 59 passed，test-tool audit PASS，docs graph PASS，chunkyctl audit PASS，doctor WARN 仅剩既有 dirty/complexity/storage/stage-opt/need_027，worktree unknown=0，`git diff --check` PASS，`codex review --uncommitted` 未发现 blocking correctness issue。

- **Skill dispatch + Moth evidence paths**: 本轮把 `clone timeout` / `early EOF` / `openai/codex-plugin-cc.git` 明确归入 Codex plugin startup_sync 同类问题，写入 `/Users/dp/.codex/skills/codex-local-ops` 和 `/Users/dp/.codex/AGENTS.md`；`AGENTS.md`、`docs/engineering_governance.md`、`docs/chunkyctl_session_quickstart.md` 已新增 Skill dispatch，明确 local ops 用 `$codex-local-ops`、项目执行前治理用 `$chunkymonkey-governance`、Rule 10 / commit review 用 `$chunkymonkey-review-gate`。`.moth/profile.yaml` 与 `/Users/dp/Documents/M/moth/profiles/chunkymonkey.yaml` 现在只把这些 skill 和 `docs/PROJECT_CONSTITUTION.md` 暴露为 evidence paths，不把业务 gate 规则搬进 Moth。

- **Data-health blocker triage complete**: `scripts/chunkyctl doctor --fast` 最新结果已经是 `data_health PASS` (`green=342 / yellow=0 / red=0`)，最后两条 warning-only writer `fact_lhb_event` / `mart_daily_recommendation_explanation` 也已刷新；当前 `WARN` 只剩 `need_027` blocked、stage-opt `below_min_signals`、dirty worktree 和 complexity 历史残余，不再是 data-health blocker。

- **Data-view render hot paths second pass**: `assets/js/data-view.js` 的第二轮收口把剩余的回调型渲染继续压平，`buildRouteSearchText()`、`buildSourceCardsModel()`、`buildHealthHeatmapModel()`、`buildSourcePriorityModel()`、`buildLinkOverviewModel()`、`renderLinkOverview()`、`renderSourceCards()`、`renderAuditResults()`、`renderRoutesTable()` 和 `_setUpdateButtonsBusy()` 现在也都收成直线型循环/拼接，去掉了残余 `.map()` / `.forEach()` 热点。验证：`node --check assets/js/data-view.js` PASS，`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/tests/contract/test_data_view.py --scope backend/tests/contract/test_workbench_frontend_contract.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_data_view.py backend/tests/contract/test_workbench_frontend_contract.py` 6 passed，`python /Users/dp/.agents/skills/complexity-optimizer/scripts/analyze_complexity.py /Users/dp/Documents/M/stock/chunkymonkey/assets/js/data-view.js --format markdown` targeted scan 无明显热点，`codegraph sync .` 已同步。

- **Data-view render hot paths flattening**: `assets/js/data-view.js` 的渲染热点继续收口，`renderHealthHeatmap()`、`renderSourcePriority()`、`renderFallbackPanel()`、`renderDriftQueue()`、`renderCapTable()`、`renderStepGrid()` 和 `startPolling()` 的日志聚合都已改成直线型 `for...of` / 字符串拼接，避免 `.map().join()` / `forEach()` 继续堆回调型热路径；验证：`node --check assets/js/data-view.js` PASS，`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/tests/contract/test_data_view.py --scope backend/tests/contract/test_workbench_frontend_contract.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_data_view.py backend/tests/contract/test_workbench_frontend_contract.py` 6 passed，`python /Users/dp/.agents/skills/complexity-optimizer/scripts/analyze_complexity.py /Users/dp/Documents/M/stock/chunkymonkey/assets/js/data-view.js --format markdown` targeted scan 无明显热点，`codegraph sync .` 已同步。

- **Stage-opt supply expansion via bc_absorbed challengers**: `backend/services/formula_engine/bc_absorbed_challengers.py` 新增 5 个 `bc_absorbed` challenger formulas 的 `FormulaBase` 适配器，并在 `backend/services/formula_engine/bootstrap.py` 纳入 live `REGISTRY`，让 `build_formula_signals_history.py` 能通过同一条信号管线生成它们的 `fact_technical_trigger` 行；`backend/tests/test_build_formula_signals.py` / `backend/tests/scripts/test_chunkyctl.py` 已补 live registry 与 next-action 回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/services/formula_engine/bc_absorbed_challengers.py --scope backend/services/formula_engine/bootstrap.py --scope backend/scripts/audit_stage_opt_candidate_supply.py --scope backend/tests/test_build_formula_signals.py --scope backend/tests/scripts/test_chunkyctl.py` PASS，`python -m py_compile backend/services/formula_engine/bc_absorbed_challengers.py backend/services/formula_engine/bootstrap.py backend/scripts/audit_stage_opt_candidate_supply.py backend/tests/test_build_formula_signals.py backend/tests/scripts/test_chunkyctl.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/test_formula_engine.py backend/tests/test_build_formula_signals.py backend/tests/scripts/test_audit_stage_opt_candidate_supply.py backend/tests/scripts/test_chunkyctl.py` 105 passed，`codegraph sync .` 已同步。

- **Bestchoice blueprint intake**: 已记录一份新的系统蓝图，方向是 `CatBoost` 特征矩阵 + 贝叶斯后验更新 + `Optuna TPE` 做参数寻优，执行层用 `ATR` / 时间止损 / 分数凯利做风险控制，底座是本地 `DuckDB` / `VectorBT`，数据湖走 `httpx` -> JSON -> `Parquet`；重回测和大规模寻优必须先登记 `experiment_jobs` job family，`local` 是当前 active backend，Modal provider adapter 仍是 planned/blocked。推荐顺序是先收敛数据管道和 truth-source，再跑通本地研究闭环，最后才接 provider adapter 编排。`bestchoice` 公式事项暂列后续，不和当前 `stage-opt` / `need_027` blocker 线程混在一起。记录已同步到 `goal.md` / `SESSION_HANDOFF.md`，后续推进时按这条顺序拆任务。

- **Chunkyctl action detail suffix helper extraction**: `backend/scripts/chunkyctl.py` 里的 `_stage_opt_candidate_action()` / `_need_coverage_blocked_action()` 现共用 `_format_action_detail_suffix()`，把两处重复的 details 后缀拼装收成一个纯 helper；`stage-opt` / `need_027` 的 next-action 文本和现有回归测试语义保持不变。`backend/tests/scripts/test_chunkyctl.py` 已补 helper 直测。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/scripts/chunkyctl.py --scope backend/tests/scripts/test_chunkyctl.py` PASS，`python -m py_compile backend/scripts/chunkyctl.py backend/tests/scripts/test_chunkyctl.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/scripts/test_chunkyctl.py` 28 passed，`analyze_complexity.py` 对 `backend/scripts/chunkyctl.py` 无明显热点，`codegraph sync .` 已同步。

- **Audit_tdx_data_need_coverage helper regression expansion**: `backend/tests/scripts/test_audit_tdx_data_need_coverage.py` 现直接覆盖 `_source_registration_summary()` 和 `_blocked_need_summary()` 两个新 helper，补强了 `need_027` 的 source registration / failure queue 字段映射回归；`_summarize_need_gaps()` 的外层集成测试仍保留，输出语义不变。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/scripts/audit_tdx_data_need_coverage.py --scope backend/tests/scripts/test_audit_tdx_data_need_coverage.py` PASS，`python -m py_compile backend/scripts/audit_tdx_data_need_coverage.py backend/tests/scripts/test_audit_tdx_data_need_coverage.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/scripts/test_audit_tdx_data_need_coverage.py` 24 passed，`analyze_complexity.py` 对 `backend/scripts/audit_tdx_data_need_coverage.py` 无明显热点，`codegraph sync .` 已同步。

- **Audit_tdx_data_need_coverage blocked-need summary helper extraction**: `backend/scripts/audit_tdx_data_need_coverage.py` 里的 `_summarize_need_gaps()` 现把 blocked need 字典拼装收成 `_blocked_need_summary()` 纯 helper，`need_027` 相关的 source registration / failure queue 结构保持不变，`blocked_needs` 输出没有改字段。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/scripts/audit_tdx_data_need_coverage.py --scope backend/tests/scripts/test_audit_tdx_data_need_coverage.py` PASS，`python -m py_compile backend/scripts/audit_tdx_data_need_coverage.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/scripts/test_audit_tdx_data_need_coverage.py` 22 passed，`analyze_complexity.py` 对 `backend/scripts/audit_tdx_data_need_coverage.py` 无明显热点，`codegraph sync .` 已同步。

- **Audit_tdx_data_need_coverage source-registration helper extraction**: `backend/scripts/audit_tdx_data_need_coverage.py` 里的 `_summarize_need_gaps()` 现把 blocked need 的 source registration 摘成 `_source_registration_summary()` 纯 helper，`preferred/fallback` 的 family、注册状态、capability 列表和 `individual_fund_flow` 支持判断都统一由这个 helper 生成，`need_027` 输出结构不变。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/scripts/audit_tdx_data_need_coverage.py --scope backend/tests/scripts/test_audit_tdx_data_need_coverage.py` PASS，`python -m py_compile backend/scripts/audit_tdx_data_need_coverage.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/scripts/test_audit_tdx_data_need_coverage.py` 22 passed，`analyze_complexity.py` 对 `backend/scripts/audit_tdx_data_need_coverage.py` 无明显热点，`codegraph sync .` 已同步。

- **Chunkyctl need_027 blocker action helper extraction**: `backend/scripts/chunkyctl.py` 里的 `_next_actions()` 现把 stage-opt / need_coverage 的 next-action 文本组装分别收成 `_stage_opt_candidate_action()` 和 `_need_coverage_blocked_action()`，主循环更短，但输出语义不变；`backend/tests/scripts/test_chunkyctl.py` 仍直接覆盖 stage-opt 推荐和 `need_027` blocked-gap 文本。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/scripts/chunkyctl.py --scope backend/tests/scripts/test_chunkyctl.py` PASS，`python -m py_compile backend/scripts/chunkyctl.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/scripts/test_chunkyctl.py` 27 passed，`analyze_complexity.py` 对 `backend/scripts/chunkyctl.py` 无明显热点，`codegraph sync .` 已同步。

- **Settings-view data source params model extraction**: `assets/js/settings-view.js` 里的数据源参数卡已收成 `buildDataSourceParamsModel()` 纯 helper，`renderDataSourceParams()` 现在只消费 model；row normalization 把 `sources` 统一成 `rows`、`sourceCount`、`totalCapabilities`、`isEmpty`，并把 `capabilities` 缺失作为空数组兜底，不再直接在 render 里读 raw payload。`backend/tests/contract/test_settings_view.py` 已补 helper 导出与输入归一回归。验证：`node --check assets/js/settings-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_settings_view.py backend/tests/contract/test_workbench_frontend_contract.py` 4 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对 `assets/js/settings-view.js` 无明显热点。

- **Settings-view about model extraction**: `assets/js/settings-view.js` 里的 about 区块也已收成 `buildAboutModel()` 纯 helper，`renderAbout()` 现在只消费 model；row normalization 把 `status` / `enabled_modules` 统一成 `backendLabel`、`enabledModules`、`isHealthy`，并把空响应兜底成 `异常`。`backend/tests/contract/test_settings_view.py` 已补 helper 导出与输入归一回归。验证：`node --check assets/js/settings-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_settings_view.py backend/tests/contract/test_workbench_frontend_contract.py` 5 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对 `assets/js/settings-view.js` 无明显热点。

- **Stock-view evidence filter single-pass cleanup**: `assets/js/stock-view.js` 里的 `renderTabEvidence()` 现在把 follow/watch 事件改成单次扫描，不再先 `filter()` 两遍再拼接；`backend/tests/contract/test_workbench_frontend_contract.py` 已补 `stock-view` 不应回到双 filter 版本的回归。验证：`node --check assets/js/stock-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_signal_adapter.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对 `assets/js/stock-view.js` 无明显热点。
- **Stock-view instType normalization cleanup**: `assets/js/stock-view.js` 里的 `collectOptions()` / `applyFilters()` 现在直接读 `topEvent.institutionType`，不再重复从 `ruleChecks` 扫 `inst_type`；`backend/tests/contract/test_signal_adapter.py` 已补 `eventToView` 通过 `rule_breakdown.checks` 归一出 `institutionType` 的回归。验证：`node --check assets/js/signal-adapter.js assets/js/stock-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_signal_adapter.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 复扫后 `assets/js/stock-view.js` 无明显热点。

- **Data-view routes table event delegation cleanup**: `assets/js/data-view.js` 里的 `renderSourceCards()` / `renderRoutesTable()` / `renderDriftQueue()` / `renderStepGrid()` 现在都改成事件委托，不再在每次渲染后 `querySelectorAll(...).forEach(...)` 逐个绑定按钮；`_setUpdateButtonsBusy()` 也改成对 step-grid 容器统一切换 busy 样式。验证：`node --check assets/js/data-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_data_view.py backend/tests/contract/test_workbench_frontend_contract.py` 6 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 复扫后 `assets/js/data-view.js` 无明显热点，`codegraph sync .` 已同步。

- **Data-view routes table single-pass cleanup**: `assets/js/data-view.js` 里的 `buildRoutesTableModel()` 已改成单次扫描，`null` 路由会被直接跳过，`backend/tests/contract/test_data_view.py` 已补 `null` 路由不影响 route model 的回归。验证：`node --check assets/js/data-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_data_view.py backend/tests/contract/test_workbench_frontend_contract.py` 6 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。

- **App.js turtle/helper dead-code cleanup**: `assets/js/app.js` 里的 `turtleSystemLabel` / `turtleStateMeta` / `turtleStateTag` / `instLink` / `evTag` 死 helper 已删除，`backend/tests/contract/test_workbench_frontend_contract.py` 已补这组 dead helper 不应回流的回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/app.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_data_view.py` PASS，`node --check assets/js/app.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_data_view.py` 6 passed，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。

- **Data-view fallback/drift/capability single-pass cleanup**: `assets/js/data-view.js` 里的数据视图继续收口，`buildFallbackPanelModel()`、`buildDriftQueueModel()`、`buildCapabilityTableModel()` 现在都改成单次扫描，不再在 `filter()` / `map()` 链里重复遍历；`backend/tests/contract/test_data_view.py` 已补 null / invalid 输入不影响输出形状的回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/data-view.js --scope backend/tests/contract/test_data_view.py --scope backend/tests/contract/test_workbench_frontend_contract.py` PASS，`node --check assets/js/data-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_data_view.py backend/tests/contract/test_workbench_frontend_contract.py` 6 passed，`analyze_complexity.py` 复扫后 `assets/js/data-view.js` 无明显热点，`codegraph sync .` 已同步。

- **Data-view link overview manual count cleanup**: `assets/js/data-view.js` 里的数据链路总览继续收口，`buildLinkOverviewModel()` 现在把 `manual` 的 keep/watch/drop 计数改成单次扫描，不再重复 `filter()` 三次；`backend/tests/contract/test_data_view.py` 已补 `manual` 无效 decision 不影响计数的回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/data-view.js --scope backend/tests/contract/test_data_view.py --scope backend/tests/contract/test_workbench_frontend_contract.py` PASS，`node --check assets/js/data-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_data_view.py backend/tests/contract/test_workbench_frontend_contract.py` 6 passed，`analyze_complexity.py` 复扫后 `assets/js/data-view.js` 无明显热点，`codegraph sync .` 已同步。

- **App.js industry summary helper cleanup**: `assets/js/app.js` 里的 `loadIndustryOverviewSummary` / `resolveStockSummary` 死 helper 已删除，`backend/tests/contract/test_workbench_frontend_contract.py` 已补这两个 dead wrapper 不应回流的回归。验证：`node --check assets/js/app.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_settings_view.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。

- **Settings-view versions cleanup**: `assets/js/settings-view.js` 里的 `versions` 死字段已删除，schema versions model 现在只保留层级计数与漂移/对齐分组；`backend/tests/contract/test_settings_view.py` 已补 `versions` 不应再出现在 schema model 的回归。验证：`node --check assets/js/settings-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_settings_view.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。

- **Settings-view schema summary cleanup**: `assets/js/settings-view.js` 里的 `summary` 死字段已删除，schema versions model 现在只保留层级计数、版本列表与漂移分组；`backend/tests/contract/test_settings_view.py` 已补 `summary` 不应再出现在 schema model 的回归。验证：`node --check assets/js/settings-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_settings_view.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。

- **Signal adapter dead wrapper cleanup**: `assets/js/signal-adapter.js` 里的旧 `aggregateByStock()` wrapper 已删除，股票视图现在只消费 `fetchSignals()` 返回的 `byStock` 聚合结果；`backend/tests/contract/test_signal_adapter.py` 改成锁定 `eventToView` 映射与 dead wrapper removal，`backend/tests/contract/test_workbench_frontend_contract.py` 继续保证 signal adapter 仍在预期加载顺序内。验证：`node --check assets/js/signal-adapter.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_signal_adapter.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。

- **Signal-params win-rate formatter cleanup**: `assets/js/widgets/signal-params.js` 里的本地 `fmtWinRate` 已删除，cohort inline 里的胜率展示现在直接复用 `WidgetFormatUtils.formatWinRate()`，并补了 `backend/tests/contract/test_widget_format_utils.py` 的 SignalParamsWidget source contract；`backend/tests/contract/test_workbench_frontend_contract.py` 继续保证 `format-utils.js` 在 `signal-params.js` 之前加载。验证：`node --check assets/js/widgets/signal-params.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。
- **Screening-panel formatter cleanup**: `assets/js/widgets/screening-panel.js` 里的本地 `fmt` 已删除，选股扫描卡片现在直接复用 `WidgetFormatUtils.formatNumber()`，并补了 `backend/tests/contract/test_widget_format_utils.py` 的 ScreeningPanelWidget source contract；`backend/tests/contract/test_workbench_frontend_contract.py` 继续保证 `format-utils.js` 在 `screening-panel.js` 之前加载。验证：`node --check assets/js/widgets/screening-panel.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。
- **ETF list formatter cleanup**: `assets/js/widgets/etf-list.js` 里的本地 `etfNum` 已删除，ETF 全量筛选页现在直接复用 `WidgetFormatUtils.formatNumber()`，并补了 `backend/tests/contract/test_widget_format_utils.py` 的 ETFListWidget source contract；`backend/tests/contract/test_workbench_frontend_contract.py` 继续保证 `format-utils.js` 在 `etf-list.js` 之前加载。验证：`node --check assets/js/widgets/etf-list.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。
- **Backtest-panel / ETF strategy compare formatter extension**: `assets/js/widgets/format-utils.js` 的共享格式化 helper 继续扩到 `backtest-panel` / `etf-strategy-compare`，`backtest-panel` 的胜率展示和 `etf-strategy-compare` 的胜率 / 回撤展示现在都复用 `WidgetFormatUtils.formatWinRate()` / `formatPercent()`，并补了 `backend/tests/contract/test_widget_format_utils.py` 的 helper export / widget source contract；`backend/tests/contract/test_workbench_frontend_contract.py` 继续保证 `format-utils.js` 在这些 widget 之前加载。验证：`node --check assets/js/widgets/format-utils.js assets/js/widgets/backtest-panel.js assets/js/widgets/etf-strategy-compare.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。
- **ETF opportunity formatter cleanup**: `assets/js/widgets/etf-opportunity.js` 的本地 `etfNum` / `scoreNum` / `signedPct` / `pct` 已删除，机会发现页现在直接复用 `WidgetFormatUtils.formatNumber()` / `formatPercent()`，并补了 `backend/tests/contract/test_widget_format_utils.py` 的 ETFOpportunityWidget source contract；`backend/tests/contract/test_workbench_frontend_contract.py` 继续保证 `format-utils.js` 在 `etf-opportunity.js` 之前加载。验证：`node --check assets/js/widgets/etf-opportunity.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。
- **Widget format utils screening-panel/multidim-badge extension**: `assets/js/widgets/format-utils.js` 的共享格式化 helper 继续扩到 `screening-panel` / `multidim-badge`，`screening-panel` 的 `fmt` / 命中率展示和 `multidim-badge` 的分位 / 评分展示现在都复用 `WidgetFormatUtils`，并补了 Node `globalThis` 导出回归；`index.html` 已把 `format-utils.js` 提前到这两个 widget 之前加载，`backend/tests/contract/test_widget_format_utils.py` 继续负责 helper export / widget export contract，`backend/tests/contract/test_workbench_frontend_contract.py` 也补了 `format-utils.js` 在这些 widget 之前加载的回归。验证：`node --check assets/js/widgets/format-utils.js assets/js/widgets/signal-params.js assets/js/widgets/cohort-card.js assets/js/widgets/backtest-panel.js assets/js/widgets/screening-panel.js assets/js/widgets/multidim-badge.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。
- **Widget format utils signal widgets extension**: `assets/js/widgets/format-utils.js` 的共享格式化 helper 继续扩到 `signal-params` / `cohort-card` / `backtest-panel`，三者现在都复用 `WidgetFormatUtils`，`signal-params` / `cohort-card` / `backtest-panel` 也补了 Node `globalThis` 导出；`index.html` 已把 `format-utils.js` 提前到这组 widgets 之前加载，`backend/tests/contract/test_widget_format_utils.py` 继续负责 helper export / widget export contract，`backend/tests/contract/test_workbench_frontend_contract.py` 也补了 `format-utils.js` 在这些 widget 之前加载的回归。验证：`node --check assets/js/widgets/format-utils.js assets/js/widgets/signal-params.js assets/js/widgets/cohort-card.js assets/js/widgets/backtest-panel.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。
- **Widget format utils topk-strip extension**: `assets/js/widgets/format-utils.js` 的共享格式化 helper 继续扩到 `topk-strip`，`fmtScore` 现在复用 `WidgetFormatUtils.formatNumber()`，并补了 Node `globalThis` 导出和源码契约回归，避免它只在浏览器环境里偶然挂载；`backend/tests/contract/test_widget_format_utils.py` 继续负责 helper export / widget export contract，`backend/tests/contract/test_workbench_frontend_contract.py` 也补了 `format-utils.js` 在 `topk-strip` 之前加载的回归。验证：`node --check assets/js/widgets/format-utils.js assets/js/widgets/topk-strip.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。
- **Widget format utils institution-scorecard extension**: `assets/js/widgets/format-utils.js` 的共享格式化 helper 继续扩到 `institution-scorecard`，机构评分卡里的 `fmtNum` / `fmtScore` / `fmtGain` 现在也统一复用 `WidgetFormatUtils`，去掉本地重复 formatter；`backend/tests/contract/test_widget_format_utils.py` 继续负责 helper export / widget export contract，`backend/tests/contract/test_institution_scorecard_widget.py` 也补了 format-utils 依赖加载回归，`backend/tests/contract/test_workbench_frontend_contract.py` 继续保证 format-utils 在机构评分卡之前加载。验证：`node --check assets/js/widgets/format-utils.js assets/js/widgets/institution-scorecard.js assets/js/widgets/etf-analysis.js assets/js/widgets/workbench-health.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_institution_scorecard_widget.py` 4 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。
- **Widget format utils analysis/workbench-health extension**: `assets/js/widgets/format-utils.js` 的共享格式化 helper 继续扩到 `etf-analysis` / `workbench-health`，`etf-analysis` 里的 `fmtNum` / `fmtSignedPct` / `etfNum` 和 `workbench-health` 里的 `fmt` 现在都复用 `WidgetFormatUtils`，去掉各自的重复 local formatter 逻辑；`backend/tests/contract/test_widget_format_utils.py` 继续负责 helper export / widget export contract，`backend/tests/contract/test_workbench_frontend_contract.py` 也补了 format-utils 在这些 widget 之前加载的回归。验证：`node --check assets/js/widgets/format-utils.js assets/js/widgets/etf-analysis.js assets/js/widgets/workbench-health.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。
- **Widget format utils shared helper**: `assets/js/widgets/format-utils.js` 新增共享格式化 helper，`model-monitor` 和 `grid-optimizer` 现在统一复用 `WidgetFormatUtils.formatNumber()` / `formatPercent()`，去掉各自的重复 local formatter 逻辑；`index.html` 已把 helper 放在对应 widget 之前加载，`backend/tests/contract/test_model_monitor_widget.py` 也补了 helper export / format contract。验证：`node --check assets/js/widgets/format-utils.js assets/js/widgets/model-monitor.js assets/js/widgets/grid-optimizer.js assets/js/app.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_model_monitor_widget.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对两个改动文件无明显热点，`codegraph sync .` 已同步。
- **Widget format utils ETF extension**: `assets/js/widgets/format-utils.js` 的共享格式化 helper 继续扩到 `etf-sector-rotation` / `etf-opportunity` / `etf-strategy-compare`，三者现在统一复用 `WidgetFormatUtils.formatNumber()` / `formatPercent()`，去掉各自的重复 local formatter 逻辑；`index.html` 已保证 helper 在这些 widget 之前加载，`backend/tests/contract/test_widget_format_utils.py` 补了 helper export / ETF widget export contract，`backend/tests/contract/test_workbench_frontend_contract.py` 也补了 format-utils 在 ETF widget 之前加载的回归。验证：`node --check assets/js/widgets/format-utils.js assets/js/widgets/etf-sector-rotation.js assets/js/widgets/etf-opportunity.js assets/js/widgets/etf-strategy-compare.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_model_monitor_widget.py backend/tests/contract/test_workbench_frontend_contract.py` 4 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对三个改动文件无明显热点，`codegraph sync .` 已同步。
- **Widget format utils ETF list/workbench extension**: `assets/js/widgets/format-utils.js` 的共享格式化 helper 继续扩到 `etf-list` / `etf-workbench`，两个 widget 现在也统一复用 `WidgetFormatUtils.formatNumber()` / `formatPercent()`，去掉各自的重复 local formatter 逻辑；`backend/tests/contract/test_widget_format_utils.py` 继续负责 helper export / widget export contract，`backend/tests/contract/test_workbench_frontend_contract.py` 也补了 `format-utils.js` 在这两个 widget 之前加载的回归。验证：`node --check assets/js/widgets/format-utils.js assets/js/widgets/etf-list.js assets/js/widgets/etf-workbench.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_widget_format_utils.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对改动文件无明显热点，`codegraph sync .` 已同步。
- **Model monitor formatter helper cleanup**: `assets/js/widgets/model-monitor.js` 里重复的局部 `fmt` / `pct` 现在收成顶层 `formatNumber()` / `formatPercent()`，`renderModelComparison()` / `renderTdxValidation()` / `renderMetricsCards()` 直接复用共享 helper，输出格式不变；验证：`node --check assets/js/widgets/model-monitor.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_model_monitor_widget.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 复扫无明显热点，`codegraph sync .` 已同步。
- **Dead app.js wrapper cleanup**: `assets/js/app.js` 里的薄 wrapper `loadStocks()` / `loadResearch()` 已删除，`showView()` 直接分发到 `window.StockView.load()/reload()` 与 `loadInstScorecard()`，避免重复入口和维护歧义；`backend/tests/contract/test_workbench_frontend_contract.py` 已补这两条 wrapper 不应回流的回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/app.js --scope backend/tests/contract/test_workbench_frontend_contract.py` PASS，`node --check assets/js/app.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py` 2 passed，`analyze_complexity.py` 复扫后 `assets/js/app.js` 无明显热点，`codegraph sync .` 已同步。
- **Stock report widget removal**: `assets/js/widgets/stock-report.js` 与其专用契约测试已删除，`index.html` / `assets/js/app.js` 里的 stock-report 脚本加载与 `StockReportWidget` 入口也一并清理；`backend/tests/contract/test_workbench_frontend_contract.py` 已补“stock-report 不应回流到 app.js / index”的回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/app.js --scope backend/tests/contract/test_workbench_frontend_contract.py` PASS，`node --check assets/js/app.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`analyze_complexity.py` 复扫后 `assets/js/app.js` 无明显热点，`codegraph sync .` 已同步。
- **Rank matrix cache rows model extraction**: `assets/js/workbench-view.js` 里的 rank matrix cache 页已继续收成页级 model：`buildRankMatrixCacheModel()` 现在把 `summary` / `latest_benchmarks` / `cache_entries` 规范成 `summaryMetrics`、`benchmarkRows`、`cacheEntryRows`、`isEmpty`，`renderRankMatrixCache()` 只消费 model；`backend/tests/contract/test_workbench_frontend_render_smoke.py` 已补这组 model 的稳定性回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`node --check assets/js/workbench-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 23 passed，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。
- **Stability context rows model extraction**: `assets/js/workbench-view.js` 里的稳定性上下文继续收成页级 model：`buildStabilityContextModel()` 现在把 `summaries` / `diagnostics` 进一步规范成 `summaryRows`、`diagnosticRows`、`summaryCount`、`diagnosticCount`、`isEmpty`，`renderStabilityContext()` 只消费 model；`buildResearchModel()` / `buildChampionModel()` 也都直接消费这个 model。`backend/tests/contract/test_workbench_frontend_contract.py` 和 `backend/tests/contract/test_workbench_frontend_render_smoke.py` 已补 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`node --check assets/js/workbench-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 23 passed，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。
- **Stability context model extraction**: `assets/js/workbench-view.js` 里的稳定性上下文已收成 `buildStabilityContextModel()` 纯 helper，`buildResearchModel()` / `buildChampionModel()` 现在都直接消费该 model，`renderResearch()` / `renderChampion()` 只显示 `runId` 与归一后的 summaries/diagnostics；`backend/tests/contract/test_workbench_frontend_contract.py` 和 `backend/tests/contract/test_workbench_frontend_render_smoke.py` 已补 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`node --check assets/js/workbench-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 23 passed，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。
- **Data-view source cards detail cache**: `assets/js/data-view.js` 里的数据源卡片详情现在预先缓存成 `detailRowsHtml`，`renderSourceCards()` 只挂载 source model，`toggleDetail()` 直接从 `_state.sourceCardModelByName` 读取，避免每次点击都重新 `find/map` 重算 capability rows；`backend/tests/contract/test_data_view.py` 已补 source-cards model 的稳定性回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/data-view.js --scope backend/tests/contract/test_data_view.py` PASS，`node --check assets/js/data-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_data_view.py` 4 passed，`codegraph sync .` 已同步。
- **Data-view link overview model extraction**: `assets/js/data-view.js` 里的数据链路总览已收成 `buildLinkOverviewModel()` 纯 helper，`renderLinkOverview()` 现在只消费 model；row normalization 把 `summary` / `by_layer` / `tdxValidation` / `sourceHealth` 统一成 `snapshotAt`、`keep`、`watch`、`drop`、`pit`、`sourceLabel`、`nodes`，并把 `statusFromCounts` 的决策也前移到 model 层。`backend/tests/contract/test_data_view.py` 已补 link overview 的稳定性回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/data-view.js --scope backend/tests/contract/test_data_view.py` PASS，`node --check assets/js/data-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_data_view.py` 4 passed，`analyze_complexity.py` 复扫后 `assets/js/data-view.js` / `assets/js/stock-view.js` 无新增明显热点，`codegraph sync .` 已同步。
- **Stock timeline ordering delegation**: `assets/js/signal-adapter.js` 里的股票事件聚合继续模型化，`aggregateByStockViews()` 现在除了 `events` 还预先产出 `timelineEvents`，把 `renderTabTimeline()` 的日期排序从 UI 渲染路径挪到共享数据层；`assets/js/stock-view.js` 已直接消费 `s.timelineEvents || s.events`。`backend/tests/contract/test_signal_adapter.py` 已补 timeline 顺序回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/signal-adapter.js --scope assets/js/stock-view.js --scope backend/tests/contract/test_signal_adapter.py` PASS，`node --check assets/js/signal-adapter.js assets/js/stock-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_signal_adapter.py` 1 passed，`analyze_complexity.py` 复扫后 `assets/js/signal-adapter.js` / `assets/js/stock-view.js` 无新增明显热点，`codegraph sync .` 已同步。
- **Data-view cockpit panel model extraction**: `assets/js/data-view.js` 里的数据视图 cockpit 面板继续收口成纯 model：新增 `buildHealthHeatmapModel()`、`buildSourcePriorityModel()`、`buildFallbackPanelModel()`、`buildDriftQueueModel()`、`buildCapabilityTableModel()`，对应的 `renderHealthHeatmap()` / `renderSourcePriority()` / `renderFallbackPanel()` / `renderDriftQueue()` / `renderCapTable()` 现在只消费 model；`backend/tests/contract/test_data_view.py` 已补这组 builder 的稳定性回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/data-view.js --scope backend/tests/contract/test_data_view.py` PASS，`node --check assets/js/data-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_data_view.py` 4 passed，`analyze_complexity.py` 复扫后 `assets/js/data-view.js` 无明显热点，`codegraph sync .` 已同步。
- **Pipelines model extraction**: `assets/js/workbench-view.js` 里的 pipelines 页已收成 `buildPipelinesModel()` 纯 helper，`renderPipelines()` 现在只消费 model；row normalization 把 `recent` / `slowest` / `blockers` 统一到 page-level model，并保留 `status_counts`、`latest`、`slowestName`、`slowestDurationS`、`isEmpty`。workbench contract / smoke test 已补上 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 22 passed，`node --check assets/js/workbench-view.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。
- **Research model extraction**: `assets/js/workbench-view.js` 里的研究页已收成 `buildResearchModel()` 纯 helper，`renderResearch()` 现在只消费 model；row normalization 把 `research_schedule` / `model_stability` / `ranker_profiles` / `ranker_policy` / `rank_matrix_cache` / `stability_context` / `stock_horizon_profile` / `shareholder_plan_*` / `temporal_synergy` / `industry_pit` 统一到 page-level model，workbench contract / smoke test 已补上 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 21 passed，`node --check assets/js/workbench-view.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。
- **Today signal cache model extraction**: `assets/js/workbench-view.js` 里的今日信号快照已收成 `buildTodaySignalCacheModel()` 纯 helper，`renderTodaySignalCache()` 现在只消费 model；row normalization 把 `today_signal_cache` 统一成 `status`、`statusTone`、`signalCount`、`freshnessDays`、`sourceMaxNoticeDate`、`currentSourceMaxNoticeDate`、`builtAt`、`error`、`step`、`isEmpty`，并把 dataSources 页头部的信号快照统计也切到 model 口径。`buildDataSourcesModel()` 也开始返回 `signalCacheModel`，workbench contract / smoke test 已补上 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 20 passed，`node --check assets/js/workbench-view.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。
- **Processing monitor model extraction**: `assets/js/workbench-view.js` 里的处理工具监控 / 拒绝原因块已收成 `buildProcessingMonitorModel()` 纯 helper，`renderProcessingMonitorTable()` 现在只消费 model；row normalization 把 `processing_monitor` 统一成 `recentRuns`、`reasonCounts`、`totalRejectedRows`、`runCount`、`recentRunCount`、`reasonCount`、`isEmpty`，并把数据源页头部清洗拒绝统计也切到 model 口径。`buildDataSourcesModel()` 也开始返回 `processingMonitor` model，workbench contract / smoke test 已补上 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 19 passed，`node --check assets/js/workbench-view.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。
- **Asset governance table model extraction**: `assets/js/workbench-view.js` 里的资产用途与质量契约表已收成 `buildAssetGovernanceTableModel()` 纯 helper，`renderAssetGovernanceTable()` 现在只消费 model；row normalization 把 `asset_health.items` 统一成 `rows`、`rowCount`、`isEmpty`，并保留原有 hidden_internal 过滤、quality gate 排序与 80 行上限语义。`buildDataSourcesModel()` 也开始返回 `assetGovernanceTable` model，workbench contract / smoke test 已补上 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 18 passed，`node --check assets/js/workbench-view.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。

- **TDX server health model extraction**: `assets/js/workbench-view.js` 里的 TDX K 线服务器健康块已收成 `buildTdxServerHealthModel()` 纯 helper，`renderTdxServerHealthTable()` 现在只消费 model；row normalization 把 `servers` / `summary` 统一成 `capabilities`、`totals`、`rows`、`rowCount`、`updatedAt`、`isEmpty`，并兼容 `healthy_count` / `timeout_server_count` 与 success/fail/timeout 汇总口径。`buildDataSourcesModel()` 也开始返回 `tdxServerHealth` model，workbench contract / smoke test 已补上 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 17 passed，`node --check assets/js/workbench-view.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。

- **TDX F10 source-date DQ model extraction**: `assets/js/workbench-view.js` 里的 TDX/F10 source-date DQ 已收成 `buildTdxF10SourceDateDqModel()` 纯 helper，`renderTdxF10SourceDq()` 现在只消费 model；row normalization 把 `summary` / `details` 统一成 `headerCounts`、`details`、`gateRunId`、`gateStatus`、`endedAt`、`detailCount`、`isEmpty`，并预先算好 `statusTone` / `violationCount` / `tableName` / `columnName` / `checkName`。`index.html` 不变但 workbench contract / smoke test 已补上 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 16 passed，`node --check assets/js/workbench-view.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。
- **TDX F10 source-date audit model extraction**: `assets/js/workbench-view.js` 里的 TDX F10 source-date audit 已收成 `buildTdxF10SourceDateAuditModel()` 纯 helper，`renderTdxF10SourceDateAudit()` 现在只消费 model；row normalization 把 `summary` / `rows` 统一成 `headerCounts`、`rows`、`runId`、`builtAt`、`rowCount`、`isEmpty`，并预先算好 `candidateLabel` / `candidateTone` / `roleTone` / `futureTone` / `dateRange`。`index.html` 不变但 workbench contract / smoke test 已补上 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 15 passed，`node --check assets/js/workbench-view.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。
- **Storage model extraction**: `assets/js/workbench-view.js` 里的存储页已收成 `buildStorageModel()` 纯 helper，`renderStorage()` 现在只消费 model；row normalization 把 `latest_manifest` / `retention` / `architecture` / `architecture_cleanup` 统一成 `manifest` / `retentionCandidates` / `architectureCleanupCandidates` / `cleanupCandidates` / `classificationCounts` / `cleanupStatusCounts` / `cleanupSmokeCounts` / `retentionCandidateCount` / `cleanupCandidateCount` / `isEmpty`，`index.html` 不变但 workbench contract / smoke test 已补上 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 14 passed，`node --check assets/js/workbench-view.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。
- **Paper sim model extraction**: `assets/js/workbench-view.js` 里的 Paper Sim 视图已收成 `buildPaperSimModel()` 纯 helper，`renderPaperSim()` 现在只消费 model；row normalization 把 `data.data` / `data.latest` / `data.meta` 统一成 `rows` / `latest` / `meta` / `sourceMode` / `rowCount` / `isEmpty`，`index.html` 不变但 workbench contract / smoke test 已补上 helper 导出与输入归一回归。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/workbench-view.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_workbench_frontend_render_smoke.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_workbench_frontend_render_smoke.py` 13 passed，`node --check assets/js/workbench-view.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/workbench-view.js` 无明显热点，`codegraph sync .` 已同步。
...
- **Stock summary delegate cleanup**: `assets/js/app.js` 里的 stock summary 兜底聚合已收成对 `StockSummaryWidget.mergeStockSummary()` 的薄委派，`topCountEntries()` 与本地重复计数/分组块从 `app.js` 删掉，重复 truth source 只保留在 `assets/js/widgets/stock-summary.js`；`backend/tests/contract/test_workbench_frontend_contract.py` 新增 `topCountEntries(` 与 `local.topIndustries = topCountEntries` 不应回流的契约。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/app.js --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_stock_summary_widget.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_stock_summary_widget.py` 3 passed，`node --check assets/js/app.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/app.js` 无明显热点，`codegraph sync .` 已同步。
- **Data-view audit model cleanup**: `assets/js/data-view.js` 里的审计结果拆分已收成 `buildAuditResultsModel()` 纯 helper，`renderAuditResults()` 只消费 model；`null` 审计行会被安全忽略，issues / okRows / n_error / n_warn / n_ok / n_tables / run_at 都在单一 model 层归一。新增 `backend/tests/contract/test_data_view.py` 锁住 helper 行为，并把 `backend/tests/contract/test_workbench_frontend_contract.py` 里的 data-view 契约收紧到 `buildAuditResultsModel(`。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope assets/js/data-view.js --scope backend/tests/contract/test_data_view.py --scope backend/tests/contract/test_workbench_frontend_contract.py --scope backend/tests/contract/test_settings_view.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_data_view.py backend/tests/contract/test_workbench_frontend_contract.py backend/tests/contract/test_settings_view.py` 4 passed，`node --check assets/js/data-view.js` PASS，`analyze_complexity.py` 复扫后 `assets/js/data-view.js` 无明显热点，`codegraph sync .` 已同步。
...
- **Workbench health widget extraction**: `assets/js/app.js` 里的工作台健康 / 连通性实现已抽到 `assets/js/widgets/workbench-health.js`，`app.js` 现在只保留 `refreshWorkbenchHealthBar()` / `refreshNetwork()` 薄 wrapper + 依赖注入；widget 自带 `refreshWorkbenchHealthBar` / `refreshNetwork` / `normalizeSourceName` / `setSourcePill`，把健康卡、事件成熟度、机构概览、信号概览、管线状态与 network pills 收口到单独模块，`index.html` 已调整为先加载 widget 再加载 `app.js`。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/tests/contract/test_workbench_health_widget.py --scope backend/tests/contract/test_workbench_frontend_contract.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_workbench_health_widget.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`node --check assets/js/widgets/workbench-health.js assets/js/app.js` PASS，`analyze_complexity.py` 复扫后新 widget 无明显热点，`codegraph sync .` 已同步。
- **Signal adapter aggregation cleanup**: `assets/js/signal-adapter.js` 的 `aggregateByStock()` 现改成先 `map(eventToView)` 再单次分组聚合，股票组内只做一次排序和一次 finalize，不再在同一批事件上反复 `map/filter/reduce`；`fetchSignals()` 复用已经标准化的 views，避免重复转换，同时模块入口改成 Node-safe 的 `typeof window !== 'undefined' ? window : globalThis`；新增 `backend/tests/contract/test_signal_adapter.py` 锁住分组、排序、premiumAvg、latestNotice 与 noticeSourceCounts 语义。验证：`node --check assets/js/signal-adapter.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_signal_adapter.py` 1 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对该文件无明显热点，`codegraph sync .` 已同步。
- **Settings view schema version summary cleanup**: `assets/js/settings-view.js` 里的 schema versions 渲染逻辑现在改成 `buildSchemaVersionsModel()` 单次聚合，`byLayer` / `driftRows` / `okRows` / `latestNotice` 都在一轮遍历里完成，`renderSchemaVersions()` 只消费这个 view model；模块入口也改成 Node-safe 的 `typeof window !== 'undefined' ? window : globalThis`，并新增 `backend/tests/contract/test_settings_view.py` 锁住分组、排序和计数语义。验证：`node --check assets/js/settings-view.js` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_settings_view.py` 1 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对该文件无明显热点，`codegraph sync .` 已同步。
## [INDEX] 2026-06-02 增量

- **ETF opportunity widget extraction**: `assets/js/app.js` 里的 ETF 机会发现页主实现已抽到 `assets/js/widgets/etf-opportunity.js`，`app.js` 现在只保留 `loadEtfOpportunity()` 薄 wrapper + 依赖注入；widget 自带 `etfOverviewTone` / `etfWatchTags` / `buildOpportunityHtml` / `buildOpportunityMiningHtml` / `mountOpportunity`，并保持机会入口、挖掘建议和轮动面板的原始语义；`index.html` 已调整为先加载 widget 再加载 `app.js`。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/tests/contract/test_etf_opportunity_widget.py --scope backend/tests/contract/test_workbench_frontend_contract.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_etf_opportunity_widget.py backend/tests/contract/test_etf_list_widget.py backend/tests/contract/test_etf_analysis_widget.py backend/tests/contract/test_workbench_frontend_contract.py` 5 passed，`node --check assets/js/widgets/etf-opportunity.js assets/js/app.js` PASS，`analyze_complexity.py` 对新 widget 无明显热点，`codegraph sync .` 已同步。
- **ETF list widget extraction**: `assets/js/app.js` 里的 ETF 列表主实现已抽到 `assets/js/widgets/etf-list.js`，`app.js` 现在只保留 `loadEtfList()` 薄 wrapper + 依赖注入；widget 自带 `collectEtfCategoryCounts` / `sortEtfCategories` / `applyEtfListFilters` / `buildEtfFilterBarHtml` / `buildEtfListRowHtml` / `buildEtfListTableHtml` / `mountEtfList`，`index.html` 已调整为先加载 widget 再加载 `app.js`。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/tests/contract/test_etf_list_widget.py --scope backend/tests/contract/test_workbench_frontend_contract.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_etf_list_widget.py backend/tests/contract/test_etf_analysis_widget.py backend/tests/contract/test_workbench_frontend_contract.py` 4 passed，`node --check assets/js/widgets/etf-list.js assets/js/app.js` PASS，`analyze_complexity.py` 对新 widget 无明显热点，`codegraph sync .` 已同步。
- **Institution scorecard widget extraction**: `assets/js/app.js` 里的机构评分卡主实现已抽到 `assets/js/widgets/institution-scorecard.js`，`app.js` 现在只保留 `loadInstScorecard()` / `loadResearch()` 薄入口 + 依赖注入；研究页 `index.html` 已补回 `instScorecardFramework` / `instScorecardStats` / `instScorecardParams` 容器，同时把此前没有任何调用链的机构列表/管理死控件收束掉，避免把未接线控件误当作可用功能。widget 自带 `mountScorecard` / `renderInstScorecardStats` / `renderInstFrameworkRules` / `renderScoreParamCard` / `renderStockFrameworkLayer` / `renderStockFrameworkRules`。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/tests/contract/test_institution_scorecard_widget.py --scope backend/tests/contract/test_workbench_frontend_contract.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_institution_scorecard_widget.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`node --check assets/js/app.js assets/js/widgets/institution-scorecard.js` PASS，`analyze_complexity.py` 对两文件无明显热点，`codegraph sync .` 已同步。
- **ETF deep-analysis widget extraction**: `assets/js/app.js` 里的 ETF 深度分析主实现已抽到 `assets/js/widgets/etf-analysis.js`，`app.js` 现在只保留薄 wrapper + 依赖注入；widget 自带 `mountDeepAnalysis` / `buildDeepAnalysisHtml` / `buildEtfTradeTimelineHtml` / `buildNavCurveSvg`，`index.html` 已调整为先加载 widget 再加载 `app.js`。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/tests/contract/test_etf_analysis_widget.py --scope backend/tests/contract/test_workbench_frontend_contract.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/contract/test_etf_analysis_widget.py backend/tests/contract/test_workbench_frontend_contract.py` 3 passed，`node --check assets/js/app.js assets/js/widgets/etf-analysis.js` PASS，`analyze_complexity.py` 对两文件无明显热点，`codegraph sync .` 已同步。

- **Preflight switched to Moth-backed tooling gate**: `backend/scripts/chunkyctl.py` 的 `preflight` 现在不再本地拼 `git status` / `codegraph status` 文本，而是直接复用 `moth snapshot --repo ... --profile chunkymonkey --format json` 的 shared tooling gate；因此 `backend/services/codegraph_status.py` 这条本地 parser 已退役，`backend/tests/services/test_codegraph_status.py` 也一并删除。`backend/config/test_tool_registry.yaml` 的 test-tool 归属同步移除旧 parser scope，`docs/chunkyctl_session_quickstart.md` 和 `goal.md` 已写明新的 preflight 口径。验证：`python -m py_compile backend/scripts/chunkyctl.py backend/tests/scripts/test_chunkyctl.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/scripts/test_chunkyctl.py backend/tests/services/test_worktree_status.py` 28 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对 `backend/scripts/chunkyctl.py` / `backend/services/worktree_status.py` 无明显热点，`codegraph sync .` PASS，`scripts/chunkyctl docs --format markdown` PASS。
- **LambdaMART train-log stability sort cleanup**: `backend/scripts/audit_lambdamart_train_log_stability.py` 现在把 `worst_windows` 从全量 `sorted(... )[:8]` 收成 `heapq.nsmallest(8, ...)`，同时 `regime_breakdown` 改为按输入 group 的遇到顺序输出，不再做一次额外的组内排序；`strategy_return` / `relative_drop` 语义和输出字段不变。验证：`PYTHONPATH=backend python -m pytest -q backend/tests/test_audit_lambdamart_train_log_stability.py` 4 passed，`py_compile` PASS，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对该文件无明显热点。
- **Feature catalog current per-table stats batching**: `backend/scripts/build_feature_catalog_current.py` 现在把每张表的 `COUNT(*) + MIN/MAX(date) + COUNT(col...)` 收成一次聚合查询，不再分开跑总行数 / 日期边界 / 非空计数的多个 round-trip；`_date_bounds()` / `_count_non_null_columns()` 的职责合并到 `_table_stats()`，语义不变。验证：`PYTHONPATH=backend python -m pytest -q backend/tests/pipeline/test_feature_catalog_current.py` 2 passed，`py_compile` PASS，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对该文件无明显热点。
- **P0a panel v3 PIT confidence batching**: `backend/scripts/audit_p0a_panel.py` 的 v3 分支现在把 5 个关键源 NULL 率从逐列 `COUNT(col)` 收成一次批量聚合，再按列回填 PASS/WARN 结果；分布查询仍保留，missing-column 行为不变。验证：`PYTHONPATH=backend python -m pytest -q backend/tests/scripts/test_audit_p0a_panel.py` 2 passed，`py_compile` PASS，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对该文件无明显热点。
- **Paper-sim legacy diff helper simplification**: `backend/scripts/backfill_paper_sim_cache_metadata.py` 现在把每个 section 的配置差异收进 `_section_param_diff()`，不再对每个 `portfolio` / `swap` section 做 `sorted(set(...))` 的重复扫描；`param_diff_json()` 仍然输出同样的 compact JSON 结构，语义不变。验证：`python -m py_compile backend/scripts/backfill_paper_sim_cache_metadata.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/test_backfill_paper_sim_cache_metadata.py` 6 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对该文件无明显热点，`codegraph sync .` 已同步。
- **Survivorship audit missing-code sampling simplification**: `backend/scripts/audit_survivorship.py` 现在把 `missing` 样本从 `sorted(missing)[:5]` 改为 `heapq.nsmallest(5, missing)`，避免在每个 spot check window 上做完整排序；新增 `backend/tests/test_audit_survivorship.py` 锁住字典序最小样本和 limit/empty 边界。验证：`python -m py_compile backend/scripts/audit_survivorship.py backend/tests/test_audit_survivorship.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/test_audit_survivorship.py` 2 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对该文件无明显热点，`codegraph sync .` 已同步。
- **MACD feature bucket enrichment cache**: `backend/scripts/analyze_macd_feature_buckets.py` 现在先为每只股票构建一次日期索引和固定持仓窗口的滚动最小值缓存，再用 O(1) 索引切片计算 forward return / drawdown，避免对每个 MACD 信号重复做 `dates.index(...)` 和持仓窗口内的线性扫描；新增 `backend/tests/test_analyze_macd_feature_buckets.py` 锁住 rolling minima 和 enrichment 结果。验证：`python -m py_compile backend/scripts/analyze_macd_feature_buckets.py backend/tests/test_analyze_macd_feature_buckets.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/test_analyze_macd_feature_buckets.py` 2 passed，`audit_test_tool_health.py` PASS，`analyze_complexity.py` 对该文件无明显热点，`codegraph sync .` 已同步。
- **LambdaMART train-log stability return attach sweep**: `backend/scripts/audit_lambdamart_train_log_stability.py` 现在先对 `port_returns` 做一次排序和前缀和 / 前缀积预处理，再用二分边界为每个 window 切片，避免 `attach_strategy_returns()` 在每个 window 上重复全量扫描 returns；`worst_windows` 的排序 key 也改成单次 `_safe_float` 计算，语义不变。验证：`python -m py_compile backend/scripts/audit_lambdamart_train_log_stability.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/test_audit_lambdamart_train_log_stability.py` 4 passed，`analyze_complexity.py` 对该文件无明显热点，`codegraph sync .` 已同步。
- **Feature catalog current per-table count batching**: `backend/scripts/build_feature_catalog_current.py` 现在把 `COUNT(col)` 从每个特征一次查询改成每张表一次批量查询，`non_null_rows` 直接从 `non_null_counts` 读取，保留原有 `coverage_pct` / `pit_join_plan` / `exclusion_reason` 语义不变。验证：`python -m py_compile backend/scripts/build_feature_catalog_current.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/pipeline/test_feature_catalog_current.py` 1 passed，`analyze_complexity.py` 对该文件无明显热点，`codegraph sync .` 已同步。
- **Feature-drift root-cause UTC + summary single-pass cleanup**: `backend/scripts/build_feature_drift_root_cause.py` 现在用 `datetime.now(timezone.utc)` 生成 `run_id` / `built_at`，同时把 summary 聚合从多次 `values` 扫描收成单次遍历，`avg_psi` / `max_psi` / `scopes` / `fold_ids` / `recommendation` 的输出保持不变。验证：`python -m py_compile backend/scripts/build_feature_drift_root_cause.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/pipeline/test_feature_drift_root_cause.py` 1 passed，`analyze_complexity.py` 对该文件无明显热点。
- **Feature-drift mitigation nested transform loop flattening**: `backend/scripts/build_feature_drift_mitigation_panel.py` 现在用 `itertools.product` 直接遍历 `mitigated × transform_types`，把 `transformed_map` 改成 `defaultdict(list)` 并保持原有顺序不变；这只是一刀把嵌套循环收成单层笛卡尔遍历，语义和输出顺序不变。验证：`python -m py_compile backend/scripts/build_feature_drift_mitigation_panel.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/pipeline/test_feature_drift_mitigation_panel.py` 2 passed，`analyze_complexity.py` 对该文件无明显热点，`codegraph sync .` 已同步。
- **Daily recommendation profile top-row scan de-dup**: `backend/scripts/build_daily_position_recommendations.py` 现在把 `all_rows` 同步写入 `rows_by_profile`，打印每个 profile 的 top 5 不再重复扫描整条候选列表；`long` 桶分布也直接从 `rows_by_profile["long"]` 读，减少了候选池上的重复遍历。验证：`PYTHONPATH=backend python backend/scripts/audit_test_tool_health.py --scope backend/scripts/build_daily_position_recommendations.py --scope backend/tests/scripts/test_build_daily_position_recommendations.py` PASS，`python -m py_compile backend/scripts/build_daily_position_recommendations.py backend/tests/scripts/test_build_daily_position_recommendations.py` PASS，`PYTHONPATH=backend python -m pytest -q backend/tests/scripts/test_build_daily_position_recommendations.py` 4 passed，`analyze_complexity.py` 对该文件无明显热点，`codegraph sync .` 已同步。
- **Shared config boundary cleanup**: 通用 holding_days、stage 口径、MACD 诊断窗口、turtle_breakout 量能确认门槛、shareholder-plan walk-forward 默认 fold/train/holdout、multidim walk-forward 默认模型参数/degenerate 门槛、stock-formula optuna bucket / high-conviction 阈值已收敛到 `backend/config/stock_formula_optuna.yaml`，共享 vol/amt/p60 bins 已拆到 `backend/config/shared_feature_bins.yaml`，backtest 默认 stop/target/trailing 已收敛到 `backend/config/strategy_defaults.yaml`；`backend/services/formula_engine/shared_windows.py` 统一加载共享窗口，formula-specific YAML 只保留专用阈值，per-stock 最佳持股周期继续表化在 `mart_per_stock_stage_strategy_optimal_pit` / `mart_stock_horizon_profile` 这类 marts 中，不再在 Python 常量里复制一套一样的窗口。
- **BestChoice 资金量价共振候选待接入**: 现有 5 个 BestChoice 公式对“`20` 个交易日内主力资金流入 + `1-2` 次倍量上涨”的形态只算部分覆盖，最接近的是 `volume_base_breakout` / `activity_breakout`；缺的是直接资金流向特征，因此这类候选已记入后续接入计划，等架构 / 数据治理完成后再评估是否加入 BestChoice 模块。BestChoice 的共享 run-id / walk-forward 日期 / ensemble train-test windows 已统一到 `backend/config/bestchoice_pipeline.yaml`，由 `backend/services/bestchoice_config.py` 严格加载，避免 import / feed / audit / paper-sim / ensemble 脚本各自复制同一套默认值。
- **BestChoice 回调十字星 / CYQ 研究资产待接入**: 回调十字星相关的 `pullback_doji` / `formula_limit_up_pullback` 已作为研究资产存档，CYQ 筹码分布规格见 `docs/chip_distribution_cyq_spec.md`；它们目前仍是研究资产，不计入 live BestChoice supply，等架构 / 数据治理完成后再一起评估是否作为 challenger 接入。
- **stock-feature bins shared config externalization**: `backend/config/shared_feature_bins.yaml` 现在统一承载 `vol_bins` / `amt_bins` / `p60_bins`；`backend/services/shared_feature_bins_config.py` 做 strict fail-closed loader，`backend/scripts/build_stock_formula_optuna.py` / `_v2.py` / `build_stock_formula_buy_signal_daily.py` / `build_daily_position_recommendations.py` / `build_daily_formula_buys.py` / `analyze_macd_feature_buckets.py` / `audit_portfolio_sizer_profile_attrition.py` 只消费共享 bins，不再各自 hardcode 一套相同 bins；`backend/tests/test_shared_feature_bins_config.py` 锁住正常加载、缺 key fail 和 bin shape 校验。验证：模块导入 smoke OK，`PYTHONPATH=backend python -m pytest -q backend/tests/test_stock_formula_optuna_config.py backend/tests/test_shared_feature_bins_config.py backend/tests/scripts/test_build_daily_position_recommendations.py` = 10 passed，`audit_test_tool_health.py` PASS，`docs graph` PASS，`codegraph sync` PASS；共享规则继续留在 config，个股最佳持股周期继续表化，不回 Python literal。
- **stage-opt 2026-06-02 turtle_breakout_55 rel_std 0.4 probe**: `turtle_breakout_55` 只把 `volume_multiple` 从 `0.5` 再降到 `0.4`，不动其它阈值；历史重算后该 formula 提升到 `151,472 signal_rows / 11,446 keys / 66.21% coverage`，全局审计刷新为 `raw_signal_rows=6,422,516 / filtered_signal_rows=3,178,449 / unique_keys=159,814 / ready_keys=120,259 / ready coverage=75.25% / below_min_signals=39,555`，`min_signals=4/3/2` 现在对应 `80.19% / 85.55% / 91.74%`（`128,160 / 136,717 / 146,617` ready keys）；`dynamic_ma_iterative_cross` / `turtle_breakout_55` / `reversal_1m_deep` 仍是 weakest formulas，controller recommendation 依旧是 `P1 / upstream_candidate_supply`，说明这次只是验证了最后一个低风险 config probe，主线方向没有变化。
- **stage-opt 2026-06-02 reversal_1w rel_std 0.07 probe**: `reversal_1w` 只把 `rel_std_max` 从 `0.06` 放宽到 `0.07`，不动其它阈值；历史重算后该 formula 提升到 `280,065 signal_rows / 13,639 keys / 75.54% coverage`，全局审计刷新为 `raw_signal_rows=6,421,901 / filtered_signal_rows=3,177,918 / unique_keys=159,805 / ready_keys=120,242 / ready coverage=75.24% / below_min_signals=39,563`，`min_signals=4/3/2` 现在对应 `80.19% / 85.55% / 91.74%`（`128,150 / 136,710 / 146,610` ready keys）；`dynamic_ma_iterative_cross` / `turtle_breakout_55` / `reversal_1m_deep` 仍是 weakest formulas，controller recommendation 依旧是 `P1 / upstream_candidate_supply`，说明这次只是验证了最后一个低风险 config probe，主线方向未变。
- **stage-opt 2026-06-02 reversal_1m_mild rel_std 0.08 probe**: `reversal_1m_mild` 只把 `rel_std_max` 从 `0.07` 放宽到 `0.08`，不动其它阈值；历史重算后该 formula 提升到 `357,425 signal_rows / 14,300 keys / 73.75% coverage`，全局审计刷新为 `raw_signal_rows=6,305,481 / filtered_signal_rows=3,120,795 / unique_keys=158,359 / ready_keys=118,608 / ready coverage=74.90% / below_min_signals=39,751`，`min_signals=4/3/2` 现在对应 `79.90% / 85.36% / 91.68%`（`126,525 / 135,168 / 145,180` ready keys）；`dynamic_ma_iterative_cross` / `turtle_breakout_55` / `reversal_1w` 仍是 weakest formulas，controller recommendation 依旧是 `P1 / upstream_candidate_supply`，说明这次只是验证了最后一个低风险 config probe，主线方向未变。
- **stage-opt 2026-06-02 reversal_1m_deep rel_std 0.10 probe**: `reversal_1m_deep` 只把 `rel_std_max` 从 `0.09` 放宽到 `0.10`，不动其它阈值；历史重算后该 formula 提升到 `315,555 signal_rows / 18,962 keys / 73.29% coverage`，全局审计刷新为 `raw_signal_rows=6,200,134 / filtered_signal_rows=3,071,213 / unique_keys=157,327 / ready_keys=117,547 / ready coverage=74.72% / below_min_signals=39,780`，`min_signals=4/3/2` 现在对应 `79.81% / 85.32% / 91.64%`（`125,556 / 134,239 / 144,171` ready keys）；`dynamic_ma_iterative_cross` / `turtle_breakout_55` / `reversal_1m_mild` 仍是 weakest formulas，controller recommendation 依旧是 `P1 / upstream_candidate_supply`，说明这次只是验证了最后一个低风险 config probe，主线方向未变。
- **stage-opt 2026-06-02 reversal_1m_deep rel_std 0.09 probe**: `reversal_1m_deep` 只把 `rel_std_max` 从 `0.08` 放宽到 `0.09`，不动其它阈值；历史重算后该 formula 提升到 `284,627 signal_rows / 18,503 keys / 71.97% coverage`，全局审计刷新为 `raw_signal_rows=6,134,459 / filtered_signal_rows=3,040,285 / unique_keys=156,868 / ready_keys=116,967 / ready coverage=74.56% / below_min_signals=39,901`，`min_signals=4/3/2` 现在对应 `79.68% / 85.23% / 91.61%`（`124,994 / 133,697 / 143,710` ready keys）；`dynamic_ma_iterative_cross` / `turtle_breakout_55` / `reversal_1m_deep` 仍是 weakest formulas，controller recommendation 依旧是 `P1 / upstream_candidate_supply`，说明这次只是验证了最后一个低风险 config probe，主线方向未变。
...
- **stage-opt 2026-06-02 reversal_1m_mild rel_std 0.07 probe**: `reversal_1m_mild` 只把 `rel_std_max` 从 `0.06` 放宽到 `0.07`，不动其它阈值；历史重算后该 formula 提升到 `307,843 signal_rows / 13,239 keys / 72.11% coverage`，全局审计刷新为 `raw_signal_rows=6,054,317 / filtered_signal_rows=3,003,680 / unique_keys=156,179 / ready_keys=116,157 / ready coverage=74.37% / below_min_signals=40,022`，`min_signals=4/3/2` 现在对应 `79.54% / 85.15% / 91.57%`（`124,231 / 132,994 / 143,011` ready keys）；`dynamic_ma_iterative_cross` / `turtle_breakout_55` / `reversal_1m_deep` 仍是 weakest formulas，controller recommendation 依旧是 `P1 / upstream_candidate_supply`，说明这是最后一个低风险 config probe，主线方向未变。
...
- **Source-watermark UTC warning cleanup**: `backend/services/source_watermarks.py` now uses timezone-aware `datetime.now(timezone.utc)` timestamps for failure queue / watermark updates, which removes the lingering `datetime.utcnow()` deprecation warnings from `backend/tests/test_source_watermarks.py` and `backend/tests/scripts/test_probe_source_capability.py`; verification on the current slice is `pytest -q backend/tests/test_source_watermarks.py backend/tests/scripts/test_probe_source_capability.py` 15 passed, `audit_test_tool_health.py` PASS, and `codegraph sync .` already synced the one changed file.
- **System data-health snapshot fail-closed in `doctor`**: `backend/scripts/data_health_snapshot.py` is now a first-class system gate inside `scripts/chunkyctl doctor --fast`; dry-run JSON caps `warning` / `monitor_only` assets to yellow and lets startup health surface data freshness before any business work can claim readiness. 2026-06-01 repaired the writer timestamp normalization path, so compact timestamps no longer crash the insert, and the official `cron_daily.py` run surfaced the real system prompts instead of dying on a parse error: `sync_raw` exceeded its 60s budget, later phases still ran, health settled at 0 red / 0 yellow / 342 total, and each red/yellow row now emits `writer_prompt` owner/sync_step hints for self-triage. `sync_raw` progress snapshots now also feed `run_context.step_progress` / `/update/status`, and the raw ingest cadence now refreshes on a 10-count / 30-second threshold, so long raw fetches expose a live progress hint instead of looking frozen. Feature panel lane was refreshed successfully and removed `fact_feature_panel` / `mart_feature_panel_validation` / `mart_feature_panel_prune_run` from red, the subsequent `capital_behavior` lane cleared `dim_capital_behavior_latest`, holder/shareholder-plan lane also cleared red after parse-raw-only + backfill + mart rebuild, and GPCW / raw_aif10 are green / on-demand governance rather than blockers; `blocking_yellow_tables` are surfaced separately so `quality_gate_level=blocking` yellow assets get next-action priority before generic yellow maintenance. 2026-06-04 live startup health is WARN (`0 red / 28 yellow / 342 total`, `blocking_yellow=11`), so current controllers must clear blocking-yellow writer freshness before claiming data-health green again; `mart_architecture_cleanup_plan` remains on-demand governance. The fresh 2026-06-01 official `cron_daily.py --full-sync` also validated the new `sync_raw` cadence end-to-end: `raw_fetch` progressed from `0/5201` to `5201/5201`, the run completed 31/31 phases (`lineage / watermarks / topk / selection_log / selection_outcome / selection_summary / formula_weights / health / drift / audit` all surfaced), and the raw F10 extra path no longer carries the legacy `fact_top10_holder_period.idx_fact_hp_*` indexes that had been left behind in the current DB, so the earlier DuckDB invalidation warning is now framed as a resolved index-debt issue rather than a general Python failure.
- **Main-force / risk-factor catch-up + sharpe cap relaxation**: `backend/scripts/backfill_capital_flow_pit.py --start 2026-05-27 --end 2026-05-29` 把 `fact_capital_flow_pit_daily` 补到 `2026-05-29`（13,169 rows / 5,163 stocks / 822 dates）；`backend/scripts/backfill_risk_factors_history.py --start 2026-05-27 --end 2026-05-29` 先因 `sharpe_60d` 字典 cap `[-5, 5]` 拒绝写入，随后把 `backend/config/field_dictionary.yaml` 与 `backend/scripts/backfill_risk_factors_history.py` 同步到 `[-10, 10]` 并顺利重跑（15,530 rows / 5,178 stocks / 3 dates）；`backend/scripts/backfill_sector_momentum_history.py --start 2026-05-27 --end 2026-05-29` 将 `fact_sector_momentum_daily` 补到 `2026-05-29`（39 rows / 13 sectors）。`audit_data_completeness.py` 现在是 PASS with WARN（0 FAIL / 2 WARN）：`fact_lhb_event` 与 `fact_technical_trigger` 已注册为 `sparse_event_presence_only`，因此只作为事件覆盖 WARN evidence，不再是 completeness blocker。主力资金链路已从 stale 变成 fresh，但 LHB 仍只能当事件覆盖证据，不能上升为生产结论；`audit_pit_coverage.py` 仍是 4/4 PASS，`fact_lhb_event` gain_20d coverage 83.9% > 60%，所以 LHB 只是 completeness WARN，而不是 PIT 风险。2026-06-01 还把 `mart_stock_fund_flow_rank_snapshot_daily` 补到 `2026-06-01`（5,188 rows / 5,188 codes），并把对应 `mart_data_source_failure_queue` 的 `stock_fund_flow_rank_snapshot` 行从 open 收敛为 resolved；这仍只是研究侧排行快照辅助观测，不改变 `need_027` exact-flow blocked 结论。
- **TDX need coverage blocked-gap summary**: `backend/scripts/audit_tdx_data_need_coverage.py` 现在会显式输出 blocked need summary；当前 `backend/config/tdx_data_need_coverage.yaml` 里只有 `need_027` 仍是 `blocked/unknown`，其余 26 个 need 分布在 `eligible` / `research_only` 两个层级。这样后续 triage 不再需要手工从 YAML 里找唯一 blocker。
- **TDX need coverage source-registration evidence**: 2026-06-01 进一步把 source registration facts 写进 blocked summary；`need_027` 的 preferred `akshare` 在当前 registry 中已注册，而 declared fallback 标签 `miaoxiang` 归到 `aif10` 家族，但当前 `aif10` adapter 仍未实现 `individual_fund_flow`，所以 fallback 仍是概念路径。`backend/tests/scripts/test_audit_tdx_data_need_coverage.py` now 21 passed，当前 audit 输出仍是 27 coverage / 10 priority / 14 reassignment rows + 1 blocked need；2026-06-01 这条 blocked summary 进一步携带 `mart_data_source_failure_queue` 的 open / resolved 快照，所以 triage 现在能同时看到标签、家族、当前能力缺口和现场失败证据，`scripts/chunkyctl doctor --fast` 也会把这条 blocked-gap triage 直接放进 `next_actions`。2026-06-01 另把 registry-side `lhb_daily` 对齐到和 `services.lhb_client` 一样的 date-bounded helper，resolve/probe 不再出现旧的 aif10 全历史假象。
- **portfolio_sizer profile thresholds config owner + PIT attrition audit**: `backend/config/portfolio_sizer_profiles.yaml` / `backend/services/portfolio_sizer/config.py` 已把 short/mid/long 阈值从 `profiles.py` 硬编码迁到配置 owner，`backend/services/portfolio_sizer/attrition.py` + `backend/scripts/audit_portfolio_sizer_profile_attrition.py` 把推荐池 attrition 审计固化成正式入口；`backend/tests/test_portfolio_sizer.py` 新增 config loader + attrition 回归并保持当前行为不变；`pytest -q backend/tests/test_portfolio_sizer.py` 34 passed，`codegraph sync .` 已更新 3 个文件。随后对 2026-05-29 recommendation candidate pool 做 attrition audit: raw candidates 353（20 `stage_pit` / 9 `stage_pit_formula_fallback` / 324 `cross_stage_fallback`），但最终 `short/mid/long` 选择仍全是 cross-stage，因为 `hp/n_signals/Wilson` 门槛会把 exact PIT 候选截掉。新增的 `fail_reasons_by_match_tier` 进一步显示 exact-tier 主要卡在 `stage_pit` 的 `hp/n_signals/Wilson`、`stage_pit_formula_fallback` 的 `hp/n_signals`，而 `cross_stage_fallback` 主要卡在 `hp/wilson`；新补的 `fail_holding_days_by_match_tier` 进一步把 hp 失败拆到 holding_days，exact PIT 的 `hp` 失败主要集中在 20/30/60/90 这些 off-anchor 档位。`goal.md` / `SESSION_HANDOFF.md` / `analysis/workflow_checkpoint.md` 已同步这条新口径，后续 tuning 必须 evidence-gated，不允许回到 Python 硬编码。
- **portfolio_sizer sensitivity audit**: `backend/scripts/audit_portfolio_sizer_profile_sensitivity.py` 在同一真实候选池上进一步验证了该结论：`base` / `hold+20` / `min_n_signals-2` / `min_wilson_win-0.05` 都没有改变 `short/mid/long` 的 selected_rows（仍 5/1/2），因此后续 tuning 重点应从 profile 微调转回上游 candidate supply / formula coverage，而不是继续在 `profiles.py` 上做小幅 knob 调整。
- **stage-opt candidate supply audit**: 新增 `backend/scripts/audit_stage_opt_candidate_supply.py` + `backend/tests/scripts/test_audit_stage_opt_candidate_supply.py`，`audit_test_tool_health.py` scoped PASS，`pytest -q backend/tests/scripts/test_audit_stage_opt_candidate_supply.py` 2 passed，`codegraph sync .` 已更新 2 个文件，`complexity-optimizer` 复扫后该 slice 没有引入新的 HIGH；2026-06-01 先后回填 `fact_stock_technical_stage` / `fact_signal_context` 的 2025-08-01→2026-05-29 断档后，再跑 full-history 审计时已看到 `raw_signal_rows=1,381,657 / filtered_signal_rows=733,083 / unique_keys=120,273 / ready_keys=57,986 / ready coverage=48.21% / below_min_signals=62,287 / dropped_index_rows=1,355 / dropped_unknown_stage_rows=647,219`，且 `codes_without_bars=0`；本轮还修正了 `raw_signal_rows` 被 filtered summary shadow 的报告 bug，所以 raw 和 filtered 现在分开显示。随后又用 `compute_start=2022-01-01 / write_start=2023-01-01 / end=2023-09-11` 补了 `fact_stock_technical_stage` 早期窗口 427,436 行（min date 到 2023-01-13），但 rerun 审计结果完全不变，说明 stage-opt 的瓶颈不在 2023 年初空白；2026-06-02 先把 `reversal_1m_deep` 阈值从 15-30% 放宽到 10-30%，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `76,635 / 11,968 / 42.54%`，随后又把 `reversal_1m_mild` 阈值从 5-15% 放宽到 4-15%，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `372,661 / 9,265 / 62.43%`；2026-06-02 又将 `reversal_1w` 从 5-10% 放宽到 2-10% 后实跑到 `369,822 / 15,937 / 66.18%`，并将 `dynamic_ma_iterative_cross` 默认迭代轮数从 10 轮压到 2 轮，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `225,783 / 20,076 / 51.63%`；当前全局审计已刷新为 `1,798,647 raw_signal_rows / 917,845 filtered_signal_rows / 130,084 unique_keys / 68,336 ready_keys / 52.53% ready coverage / 61,748 below_min_signals`，`min_signals=4` / `3` 分别把 ready coverage 抬到 `58.99%` / `68.71%`，2026-06-02 额外把 `min_signals=2` 继续推到 `84.80%`（`113,506` ready_keys / `22,872` below_min_signals），但 controller 仍把下一步指向 `P1 / upstream_candidate_supply`，说明瓶颈仍然是上游供给而不是单个公式默认轮数；`build_formula_signals_history.py --recompute-horizon-evidence` 现在也已在补 `defaultdict` import 后恢复可跑，不再在 horizon evidence 重算阶段抛 NameError；2026-06-02 的 `min_signals` probe 进一步显示 `min_signals=4` / `3` 会把全局 ready coverage 分别抬到 `64.84%` / `73.75%`，对应 `ready_keys=86,796` / `98,721`、`below_min_signals=54,129` / `41,063`，同时 `reversal_1m_deep` ready coverage 提升到 `50.97%` / `62.34%`，但 controller 仍把下一步指向 `P1 / upstream_candidate_supply`；新增 blocked-reason breakdown 说明瓶颈纯粹是 `below_min_signals`，其中 `macd_golden_cross` 和 stage 3/4 最弱；脚本现在还会自带 `next_action_recommendation`，直接把下一步指向 `P1 / upstream_candidate_supply`，并把 `macd_golden_cross` 与最弱 stage bins 作为优先 triage 目标；`2024-03-06` 起的 `dropped_unknown_stage_rows` 降到 454,158，说明 2025 H2 的硬断档已修，剩余 `technical_stage='?'` 更偏结构性分类/预热缺口而不是新鲜 ETL outage；此外，当 `macd_golden_cross` 出现在 weakest cohort 时，`doctor` 还会显示 `fact_technical_trigger` PRIMARY KEY 的 schema 限制，提醒 controller 这是 schema evolution 问题，不是 state-only formula tweak。脚本默认 end 现在复用当前连接里的交易日历真相源，不再 nested `latest_closed_or_raise()` 新连接。2026-06-02 另外新增 `mart_macd_state_history` / `build_macd_state_history.py` 作为 MACD active-state 诊断 mart，先用 180 天 warm-up 再切写入窗；`audit_stage_opt_candidate_supply.py --formula macd_golden_cross` 现在会把 `raw_trigger_rows=161,279` 与 `raw_state_history_rows=370,039` 一并计入，MACD 相关 coverage 抬到 `47.13%`（`16,474` ready keys，`raw_signal_rows=2,103,143 / filtered_signal_rows=1,110,280 / unique_keys=133,857 / ready_keys=76,480 / ready coverage=57.14% / below_min_signals=57,377`），但 controller 仍指向 `P1 / upstream_candidate_supply`，说明 state-history 只是把 evidence 做实，不改变结构性供给不足。2026-06-02 05:07-05:08 又把 `dynamic_ma_iterative_cross` 默认迭代轮数从 2 轮压到 1 轮，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `248,214 / 20,350 / 53.67%`，全局审计刷新为 `raw_signal_rows=5,145,959 / filtered_signal_rows=2,585,682 / unique_keys=147,948 / ready_keys=103,056 / ready coverage=69.66% / below_min_signals=44,892`，`min_signals=4/3/2` 现在对应 `76.02% / 82.77% / 90.50%`，但 controller 仍然指向 `P1 / upstream_candidate_supply`。
- **stage-opt 2026-06-02 4-30% deep lift**: `reversal_1m_deep` 继续从 `5-30%` 收到 `4-30%`，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `248,022 / 17,814 / 70.21%`；全局审计刷新为 `raw_signal_rows=5,840,232 / filtered_signal_rows=2,906,972 / unique_keys=153,975 / ready_keys=113,949 / ready coverage=74.00% / below_min_signals=40,026`，`min_signals=4/3/2` 现在对应 `79.27% / 85.04% / 91.58%`（`122,051 / 130,941 / 141,007` ready keys）；controller recommendation 仍然是 `P1 / upstream_candidate_supply`，说明这刀继续抬升了 supply，但结构性瓶颈仍然在上游候选供给。
- **stage-opt 2026-06-02 1-15% mild lift**: `reversal_1m_mild` 继续从 `2-15%` 收到 `1-15%`，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `249,394 / 16,818 / 69.95%`；全局审计刷新为 `raw_signal_rows=5,919,142 / filtered_signal_rows=2,942,263 / unique_keys=154,605 / ready_keys=114,597 / ready coverage=74.12% / below_min_signals=40,008`，`min_signals=4/3/2` 现在对应 `79.34% / 85.05% / 91.55%`（`122,666 / 131,485 / 141,543` ready keys）；controller recommendation 仍然是 `P1 / upstream_candidate_supply`，说明这刀继续抬升了 supply，但结构性瓶颈仍然在上游候选供给。
- **stage-opt 2026-06-02 dynamic_ma 0-iteration + turtle 0.9 lift**: `dynamic_ma_iterative_cross` 默认迭代轮数从 1 轮压到 0 轮后，历史重算把该 formula 提升到 `144,282 / 20,522 / 56.83%`；随后 `turtle_breakout_55` 的量能门槛从 `1.0` 收到 `0.9`，历史重算把该 formula 提升到 `139,053 / 17,143 / 64.62%`；全局审计刷新为 `5,530,639 raw_signal_rows / 2,765,531 filtered_signal_rows / 151,881 unique_keys / 110,336 ready_keys / 72.65% ready coverage / 41,545 below_min_signals`，`min_signals=4/3/2` 对应 `78.35% / 84.47% / 91.32%`（`119,000 / 128,298 / 138,696` ready keys）。这刀继续抬高 evidence density，但 controller 仍指向 `P1 / upstream_candidate_supply`，所以结构性瓶颈没变。
- **reversal_1w supply lift**: 2026-06-02 将 `reversal_1w` 从 5-10% 放宽到 2-10%，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `369,822 / 15,937 / 66.18%`；但 `audit_stage_opt_candidate_supply.py --formula reversal_1w` 仍给出 `P1 / upstream_candidate_supply`，说明即便供给抬升，stage-opt 的主因仍是 `below_min_signals` 而不是单个周反转阈值。
- **reversal_1w 2026-06-02 1-10% supply lift**: `reversal_1w` 进一步从 2-10% 放宽到 1-10%，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `222,942 / 16,610 / 72.28%`，全局 stage-opt 刷新为 `5,664,501 raw_signal_rows / 2,826,350 filtered_signal_rows / 152,554 unique_keys / 111,794 ready_keys / 73.28% ready coverage / 40,760 below_min_signals`；`min_signals=4/3/2` 现在对应 `78.76% / 84.74% / 91.44%`，弱公式集合转成 `dynamic_ma_iterative_cross` / `turtle_breakout_55` / `reversal_1m_mild`，但 controller 仍是 `P1 / upstream_candidate_supply`，说明这刀继续抬供给，但结构性瓶颈没变。
- **doctor stage-opt surfacing**: `scripts/chunkyctl doctor --fast` 现在会把 stage-opt 的 `next_action_recommendation` 一并带出来，controller 可以直接看到 upstream candidate-supply triage 提示，不必单独 rerun audit 才拿推荐；2026-06-02 之后还会一并暴露 `raw_trigger_rows` / `raw_state_history_rows`，让 MACD state mart 的组成证据也直接进入 controller 面。
- **doctor stage-opt sensitivity surfacing**: `scripts/chunkyctl doctor --fast` 现在也会带出 `min_signals_sensitivity`，把 `min_signals=4` / `3` / `2` 对 `ready_keys`、`ready_coverage_pct`、`below_min_signals` 的提升幅度直接写进 `doctor`，分别是 `64.84% / 86,796`、`73.75% / 98,721` 和 `84.80% / 113,506`，但 controller 推荐仍然是 `P1 / upstream_candidate_supply`，说明阈值是有效杠杆而不是最终修复。
- **doctor live registry boundary**: 2026-06-03 `audit_stage_opt_candidate_supply.py` / `doctor --fast` 现在会把 live formula registry 显式带出来，默认 live stage-opt supply 为 18 个公式 id；`pullback_doji` 与 `monthly_stage2_daily_volume_confirm` 已回退到 research-candidate-only，不再进入默认 live 回填。这样共享历史回填默认只跑 `bootstrap.LIVE_FORMULA_IDS`，而 `bestchoice` 的 `gs_*` / `volume_base_breakout` / `activity_breakout` / `ma_base_breakout` families 仍继续作为 research challenger surface 单独展示，不和 live supply 混淆。
- **doctor research challenger boundary**: 2026-06-02 controller now also surfaces 5 research challengers (`gs_raw_buy`, `gs_pullback_confirm`, `ma_base_breakout`, `activity_breakout`, `volume_base_breakout`) separately from live stage-opt supply, so future triage can see the challenger surface without mistaking it for production supply.
- **stage-opt 2026-06-02 05:02 latest rebuild**: `reversal_1m_deep` 继续维持 8-30%，`mart_macd_state_history` 仍作为 180-day warm-up 的诊断 mart；最新 full-history audit 变成 `raw_signal_rows=5,123,528 / filtered_signal_rows=2,574,836 / unique_keys=147,674 / ready_keys=102,500 / ready coverage=69.41% / below_min_signals=45,174`，其中 `turtle_breakout_20` 达到 `199,495 signal_rows / 19,413 keys / 80.81% coverage`，`turtle_breakout_55` 达到 `115,911 signal_rows / 16,898 keys / 60.17% coverage`，`min_signals=4/3/2` 现在对应 `75.85% / 82.68% / 90.43%`。这说明把 turtle breakout 的量能门槛外置到 config 并轻微放宽后，evidence density 又往上推了一格，但 controller 仍指向 `P1 / upstream_candidate_supply`，所以瓶颈结论没变。
- **stage-opt 2026-06-02 05:17-05:18 turtle breakout 1.1 lift**: 继续把 `turtle_breakout_55` 的量能确认门槛从 `1.2` 收到 `1.1`，full-history audit 再次刷新为 `raw_signal_rows=5,183,518 / filtered_signal_rows=2,609,421 / unique_keys=148,117 / ready_keys=103,984 / ready coverage=70.20% / below_min_signals=44,133`；`turtle_breakout_20` 到 `214,919 signal_rows / 19,476 keys / 83.33% coverage`，`turtle_breakout_55` 到 `124,226 signal_rows / 17,004 keys / 62.07% coverage`，`min_signals=4/3/2` 对应 `76.41% / 83.06% / 90.65%`。这一步再次证明量能门槛外置 + 轻微放宽能继续抬 evidence density，但 controller 仍指向 `P1 / upstream_candidate_supply`，所以结构性瓶颈结论没有变化。
- **stage-opt 2026-06-02 reversal_1m_deep / mild config lift**: `backend/services/formula_engine/reversal_short_term.py` 已把 1m mild / 1m deep / 1w 三个 variant 的阈值外提到 `backend/config/formula_reversal_short_term.yaml`，并把 `reversal_1m_deep` 的收益率上界从 `-0.08` 放宽到 `-0.05`、`reversal_1m_mild` 的收益率上界从 `-0.04` 放宽到 `-0.03`；`backend/tests/test_formula_engine.py` 新增 loader 回归 + 3% 温和下跌样例 + 6% 深跌样例，`pytest -q backend/tests/test_formula_engine.py` 39 passed。历史重写后 `reversal_1m_deep` 从 `118,098 signal_rows / 14,389 keys / 51.30% coverage` 抬到 `210,293 signal_rows / 17,138 keys / 66.55% coverage`，`reversal_1m_mild` 也从 `180,130 signal_rows / 15,545 keys / 66.45% coverage` 抬上来，整条 stage-opt 刷新为 `5,472,946 raw_signal_rows / 2,732,904 filtered_signal_rows / 151,570 unique_keys / 109,073 ready_keys / 71.96% ready coverage / 42,497 below_min_signals`，`min_signals=4/3/2` 现在对应 `77.81% / 84.08% / 91.16%`（`117,935 / 127,438 / 138,170` ready keys）。弱公式集合转成 `dynamic_ma_iterative_cross` / `turtle_breakout_55` / `reversal_1w`，但 controller 仍是 `P1 / upstream_candidate_supply`，说明这刀继续抬供给，但结构性瓶颈没变.
- **stage-opt 2026-06-02 turtle breakout 1.0 lift**: 继续把 `turtle_breakout_55` 的量能确认门槛从 `1.1` 收到 `1.0`，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `132,024 / 17,089 / 63.45%`；全局审计刷新为 `raw_signal_rows=5,483,722 / filtered_signal_rows=2,740,702 / unique_keys=151,655 / ready_keys=109,361 / ready coverage=72.11% / below_min_signals=42,294`，`min_signals=4/3/2` 现在对应 `77.93% / 84.18% / 91.21%`（`118,186 / 127,669 / 138,331` ready keys）。这一步再次证明量能门槛外置 + 轻微放宽能继续抬 evidence density，但 controller 仍指向 `P1 / upstream_candidate_supply`，所以结构性瓶颈结论没有变化.
- **stage-opt 2026-06-02 20/55 variant split**: `turtle_breakout_55` 进一步拆成 20 日博弈 / 55 日博弈两个 variant，保持 20 日博弈在 `0.9`、把 55 日博弈从 `0.9` 收到 `0.8`；历史重算后 `turtle_breakout_55` 提升到 `144,450 signal_rows / 17,181 keys / 65.44% coverage`，全局审计刷新为 `raw_signal_rows=5,671,811 / filtered_signal_rows=2,831,747 / unique_keys=152,592 / ready_keys=111,961 / ready coverage=73.37% / below_min_signals=40,631`，`min_signals=4/3/2` 现在对应 `78.82% / 84.79% / 91.47%`（`120,269 / 129,383 / 139,582` ready keys）。这一步再次证明量能门槛外置 + variant 化能继续抬 evidence density，但 controller 仍指向 `P1 / upstream_candidate_supply`，所以结构性瓶颈结论没有变化.
- **stage-opt 2026-06-02 2-15% mild lift**: `reversal_1m_mild` 从 `3-15%` 放宽到 `2-15%`，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `214,103 / 16,188 / 68.67%`；全局审计刷新为 `raw_signal_rows=5,748,314 / filtered_signal_rows=2,865,720 / unique_keys=153,235 / ready_keys=112,748 / ready coverage=73.58% / below_min_signals=40,487`，`min_signals=4/3/2` 现在对应 `78.95% / 84.86% / 91.49%`（`120,978 / 130,030 / 140,194` ready keys）。这一步再次证明温和反转的配置阈值外置 + 轻微放宽能继续抬 evidence density，但 controller 仍指向 `P1 / upstream_candidate_supply`，所以结构性瓶颈结论没有变化。
- **stage-opt 2026-06-02 0.7 turtle lift**: `turtle_breakout_55` 继续从 `0.8` 收到 `0.7`，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `147,973 / 17,245 / 65.78%`；全局审计刷新为 `raw_signal_rows=5,753,115 / filtered_signal_rows=2,869,243 / unique_keys=153,299 / ready_keys=112,848 / ready coverage=73.61% / below_min_signals=40,451`，`min_signals=4/3/2` 现在对应 `78.97% / 84.86% / 91.49%`（`121,053 / 130,087 / 140,247` ready keys）。这一步再次证明量能门槛外置 + 轻微放宽能继续抬 evidence density，但 controller 仍指向 `P1 / upstream_candidate_supply`，所以结构性瓶颈结论没有变化。

- **Picture / survey freshness catch-up and survey safe-window guard**: `backend/scripts/build_picture_daily.py` 已按单日事务补 2026-05-27/28/29，每日 5,203 stocks x 4 tables，`mart_stock_picture_daily` max 到 2026-05-29；`backend/scripts/build_survey_features.py` 新增 `--write-start`，读取 lookback 与写入窗口分离，默认阻断 0 行窗口误删，新增 `backend/tests/sentiment/test_build_survey_features_script.py` 覆盖 lookback、显式 read-start、非法窗口和 empty-window override。真实库用 `--write-start 2026-05-21 --end 2026-05-29` 读 2026-03-23 起事件并写 25,897 rows，`mart_stock_survey_features` max 到 2026-05-29。`audit_end_to_end.py` 已 PASS with WARN: 24 total / 23 OK / 1 WARN / 0 FAIL；`audit_data_completeness.py` now PASS with WARN（0 FAIL / 2 WARN），survivorship gate PASS，当前唯一主线 WARN 只剩 recommendation PIT coverage 0；LHB 仍是 completeness WARN，不再由 freshness 误伤。后续诊断补充：targeted PIT backfill 只把最新 cutoff 从 3 行补到 4 行，2-stock min-signals=3 smoke 仍 0 governance-pass rows，说明 current exact PIT 组合仍然太稀。
- **Stage / signal_context / technical_trigger safe-window freshness catch-up**: `backend/scripts/build_stage_formula_fitness.py` 新增 `--write-start` 与 `--stage-only`，只刷新 `fact_stock_technical_stage` 窗口，不误跑 `mart_stage_formula_fitness` 全量矩阵；`backend/scripts/build_signal_context.py` 新增可测试的 `build_signal_context()` 与 `--write-start`，把 120 日滚动计算预热起点和写入替换窗口拆开，空窗口默认拒删；`backend/scripts/build_formula_signals_history.py` 新增 `--write-start` 调用层过滤，计算可用宽窗口，写库只替换声明窗口，horizon evidence 默认不重算。真实库刷新: stage 25,894 rows、context 41,149 rows、trigger 17,950 signals，三表 max 均到 2026-05-29。2026-06-01 后续又把 `fact_stock_technical_stage` 早期窗口从 `2022-01-01` 开始补到 `2023-09-11`，新增 427,436 行并把 min date 推到 `2023-01-13`；`fact_signal_context` 早期窗口则补到 `2023-07-05`，新增 233,939 行并把 min date 推到 `2023-07-05`。`audit_stage_opt_candidate_supply.py` 在这些早期 backfill 之后仍维持 `1,381,657 raw_signal_rows / 733,083 filtered_signal_rows / 120,273 unique_keys / 57,986 ready_keys / 48.21% ready coverage / 62,287 below_min_signals` 不变，说明候选稀疏不是 2023 年初空窗导致；2026-06-02 先把 `reversal_1m_deep` 阈值从 15-30% 放宽到 10-30%，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `76,635 / 11,968 / 42.54%`，随后又把 `reversal_1m_mild` 阈值从 5-15% 放宽到 4-15%，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `372,661 / 9,265 / 62.43%`；全局审计也更新为 `raw_signal_rows=2,103,143 / filtered_signal_rows=1,110,280 / unique_keys=133,857 / ready_keys=76,480 / ready coverage=57.14% / below_min_signals=57,377`，且 `build_formula_signals_history.py --recompute-horizon-evidence` 已在补 `defaultdict` import 后恢复可跑；`audit_end_to_end.py` 已 PASS with WARN: 24 total / 23 OK / 1 WARN / 0 FAIL；freshness WARN 已不再来自 picture/survey，当前唯一主线 WARN 只剩 recommendation PIT coverage 0，而 `audit_data_completeness.py` 是 PASS with WARN（0 FAIL / 2 WARN），不再阻断策略主线。
- **Alpha158 safe-window writer + freshness catch-up**: `backend/scripts/build_alpha158_duck.py` 不再默认先 `DROP TABLE fact_alpha158_panel` 再整表重建；现在先计算临时 build table，再事务内按 `--write-start..--end` 替换目标窗口，空窗口默认 fail-closed，显式未来 `--end` 会拒绝。新增 `backend/tests/scripts/test_build_alpha158_duck.py` 覆盖窗口替换保留旧行、空窗口拒删、未来日期拒绝。真实库用 `--start 2026-03-01 --write-start 2026-05-26 --end 2026-05-29` 补齐 alpha158：`replace_window` / 20,736 rows / 5,185 codes / 4 dates；`audit_data_completeness.py` now PASS with WARN（0 FAIL / 2 WARN），`fact_alpha158_panel` 已 OK: max=2026-05-29 / n_codes=5,183 / `= cal`，刷新窗口 duplicate=0。后续 stage/context/trigger 已在同日补齐，剩余 freshness focus 转为 picture/survey/label/score/LHB。
- **K-line freshness catch-up completed; downstream freshness now system-audited**: 本地 `price_kline_tdxhub` 已用 tdxhub 增量补到交易日历 `2026-05-29`。首轮 `build_price_kline_tdxhub.py --skip-existing --target-date 2026-05-29 --pages 1 --workers 8 --max-inflight 32` 写入 15,281 行，92 股失败；随后以 `--workers 4 --max-inflight 16 --per-stock-retry-attempts 2` 重试 stale active，93 股成功 / 0 失败 / 275 行写入。`audit_data_completeness.py` now PASS with WARN（0 FAIL / 2 WARN），`price_kline_tdxhub` 已 OK: max=2026-05-29 / n_codes=5,188 / `= cal`；最近覆盖为 05-26=5201、05-27=5184、05-28=5184、05-29=5188，仍有 16 个历史/停牌/异常 code latest < 2026-05-29。`audit_end_to_end.py` 已 PASS with WARN: 24 total / 23 OK / 1 WARN / 0 FAIL；唯一主线 WARN 只剩 recommendation PIT coverage 0。最近补充的 PIT smoke 也证明 current exact 组合仍太稀：最新 cutoff 只从 3 行补到 4 行，2-stock min-signals=3 优化仍 0 governance-pass rows。
- **Daily recommendation DDL 防误删 + tooling diff 行号漂移修复**: `backend/scripts/build_daily_position_recommendations.py` 的 `DDL` 不再 `DROP TABLE mart_daily_position_recommendation`，避免每日推荐刷新时清掉历史推荐日期；新增 `backend/tests/scripts/test_build_daily_position_recommendations.py` 证明 DDL 重跑仍保留旧行。后来 `chunkyctl doctor` 的 tooling gate 迁到 Moth snapshot，complexity baseline/diff 默认仍按 `path_kind_message` 多重集身份处理，行号只做定位提示，避免删除/新增一行把历史 HIGH 误报成新 HIGH；同时保留 duplicate finding count，防止同文件同类新增热点被吞掉。验证: test-tool scoped PASS，py_compile PASS，`pytest ...test_audit_tooling_gate.py ...test_chunkyctl.py ...test_build_daily_position_recommendations.py` 30 passed，`scripts/chunkyctl audit --run --scope ...` PASS，旧 tooling gate 已退役，CodeGraph synced。
- **Recommendation PIT 诊断列补治理理由**: `backend/scripts/build_daily_position_recommendations.py` 的 `mart_daily_position_recommendation_pit_diagnostic` 新增 `governance_reject_count` / `governance_latest_reason` / `governance_latest_rejected_at` 三列，并用 `_ensure_pit_diagnostic_columns()` 兼容旧表；重新跑 `build_daily_position_recommendations.py --date 2026-05-29` 后，最新诊断行能直接看到 7 个 `stock_missing_pit` 和 1 个 `formula_missing_pit` 对应的治理拒绝原因，便于继续 triage PIT 覆盖稀疏问题。验证: `backend/tests/scripts/test_build_daily_position_recommendations.py` 3 passed，`build_daily_position_recommendations.py --date 2026-05-29` PASS，CodeGraph 已 sync。
- **Recommendation candidate loader aligns to live tables**: `backend/scripts/build_daily_position_recommendations.py` 现在会把 `mart_macd_state_history` 一起 union 进候选池，并把 cross-stage fallback 回接到现有 `mart_per_stock_strategy_optimal`，不再引用不存在的 `_pit` 表；2026-06-01 的 live run 已不再因错表崩溃，当前快照虽然是 0 candidates，但 loader 路径本身已恢复可运行。验证: `backend/tests/scripts/test_build_daily_position_recommendations.py` 4 passed，`audit_test_tool_health.py` scoped PASS，`codegraph sync .` 已更新 2 个文件。
- **Freshness builder 只读审计补充**: read-only explorer 核对真实库 max date 与构建入口后确认 P0 顺序: `price_kline_tdxhub` 先补到交易日历 `2026-05-29`；`fact_alpha158_panel` 与 `fact_lhb_event` 构建脚本存在全表 DROP，生产刷新前需安全阀或明确全量重建窗口；`mart_stock_survey_features` 有 start/end 但当前小窗口会漏 30/60d lookback；`mart_p0a_label_panel` 默认旧 label version 与 v3 当前覆盖不一致；sniper/institution score 脚本支持窗口但默认 end 是旧硬编码。该审计未写 DB、未跑 GCP/Optuna/长任务。
- **Formula signal scoped refresh safety**: `backend/scripts/build_formula_signals_history.py::write_signals_to_db()` 现在支持 `formula_ids/start/end`，显式窗口刷新只删除对应公式和日期窗口，不再为了修复近期 `fact_technical_trigger` freshness 而误删该公式全部历史信号；即使窗口内本次 0 signal，也会清掉该窗口 stale rows；公式或日期越界的 signals 会被拒绝，避免误写未删除区间。CLI 显式 `--start/--end` 默认跳过 `mart_formula_horizon_evidence` 重算，只有传 `--recompute-horizon-evidence` 才替换窗口 evidence。新增 `backend/tests/test_build_formula_signals.py` 三个回归覆盖窗口外历史保留、0-signal 窗口清 stale、越界 signals 阻断；`scripts/chunkyctl audit --run --scope ...formula...` PASS，目标 pytest 12 passed，CodeGraph synced，`doctor --fast` PASS / `new_high_count=0`。
- **Formula registry bootstrap centralization**: `backend/services/formula_engine/bootstrap.py` 统一承载当前 7 个公式的 import side-effects，`backend/scripts/build_formula_signals_history.py` 不再手工列出四个 formula module；`backend/tests/test_build_formula_signals.py` 新增 bootstrap 注册回归，确保 registry 里当前 7 个 formula IDs 持续完整。这样后续新增 formula 时只需要更新 bootstrap，减少脚本侧漏导入风险。
- **Storage payload / recursive JSON 门禁固定化 + today signal cache 迁移**: `backend/scripts/audit_storage_payloads.py` 现在只扫描 payload-like TEXT/JSON/BLOB 列，用 JSON key 形态识别 `latest_dispatch` / `queue_items` / `audit_json` 等递归风险，避免把普通 `stock_code/date/built_at` 或命令字符串误判成循环引用。`scripts/chunkyctl doctor --fast` 默认接入 storage payload summary。2026-06-01 已把真实库 `mart_today_signal_cache.signals_json` 单行 20,220,095 bytes / 9,286 signals 迁入新明细表 `mart_today_signal_cache_signal`，主表只留 summary/cache metadata + 兼容空数组；迁移脚本为 `backend/scripts/migrate_today_signal_cache_payload.py`，默认 dry-run，`--execute` 才写库。随后在 `backend/config/storage_retention.yaml` 为 12 个经审查的有界 raw/detail/lineage pointer 列登记 owner、classification、单条/总量 cap 和 path-marker 许可（新增 `mart_macd_state_history.reason_codes_json` 作为 MACD 诊断证据列）。最新真实库门禁: 323 columns / 0 FAIL / 0 WARN / 12 reviewed PASS / recursive hits 0；这不是清表或跳过审计，未来递归、超 cap 或未许可 path marker 会重新 WARN/FAIL。
- **Architecture inventory complexity diff 收束**: `backend/scripts/build_architecture_inventory.py` 将 backend/frontend edge 生成、import target resolution、route lookup、DDL statement execution 拆成小 helper，去掉 broad complexity scan 中该文件的 nested-loop / sort-in-loop / execute-in-loop HIGH。目标测试 `backend/tests/pipeline/test_architecture_inventory.py` + `backend/tests/contract/test_architecture_contracts.py` 为 15 passed，`scripts/chunkyctl audit --run --scope ...` scoped PASS。`scripts/chunkyctl doctor --fast` 仍 FAIL，但新 HIGH 已转移到下一批 BestChoice 脚本: `build_bestchoice_context_exit_policy.py` 7 个、`build_bestchoice_phase2_daily_feed.py` 2 个。
- **BestChoice/context + candidate/listing complexity diff 收束**: `backend/scripts/build_bestchoice_context_exit_policy.py` 改为按候选股票批量加载 K 线并用 `executemany` 写入 context policy，`backend/scripts/build_bestchoice_phase2_daily_feed.py` 改用 `bisect_right` helper 计算严格 T+1 buy date，并把 rank-before-dedup 契约固化为 helper 和单测。`backend/scripts/build_candidate_feature_panel.py` 将 DDL fallback 拆为 statement helper，`backend/scripts/build_dim_listing_status.py` 将 legacy schema backfill 拆为有界 column helper，并新增 `backend/tests/scripts/test_build_dim_listing_status.py`。BestChoice helper 单测 5 passed；listing/candidate scoped audit PASS，目标 pytest 3 passed。`doctor --fast` 仍 FAIL，下一窗口转为 daily recommendations / drift-safe feature candidates；subagent 审计认为大多是 bounded false positive，真实优先热点是 drift-safe fold-stable scoring 重复计算。
- **Drift-safe feature candidates complexity diff 收束**: `backend/scripts/build_drift_safe_feature_candidates.py` 将 fold-stable 贪心评分改为 `_FoldScoreState` 增量 fold-sum/state scoring，避免每个候选都重算 `stable + [row]` 全子集；将 balanced variant、feature-row serialization、drift JSON fold parsing 和 DDL fallback 拆成单层 helper，并把循环内重复 `sorted(excluded_severities)` 提到循环外。新增 `backend/tests/pipeline/test_drift_safe_feature_candidates.py::test_fold_stable_variant_matches_subset_score_greedy_order` 固化与旧 `_subset_fold_score` 贪心顺序等价。`scripts/chunkyctl audit --run --scope ...` PASS；per-file complexity 无热点；CodeGraph 已 sync。该 slice 当时把 next queue 转到 daily recommendations / ensemble context / executive trade events / feature association；后续 Phase7 context 与 executive trade events 已继续收束。
- **Phase7 context complexity diff 收束**: `backend/scripts/build_ensemble_v7_phase7_context.py` 将 per-stock K-line query 改成一次批量 JOIN + context map，避免 v7 预测行/股票数放大时的 DB N+1；新增 `backend/tests/scripts/test_build_ensemble_v7_phase7_context.py` 覆盖 code/date/freq/adjust 过滤、短/空价格序列跳过、正向 context 才保留 score。`scripts/chunkyctl audit --run --scope backend/scripts/build_ensemble_v7_phase7_context.py --scope backend/tests/scripts/test_build_ensemble_v7_phase7_context.py` PASS；该文件已不再出现在 complexity diff new HIGH。
- **Executive trade events complexity diff 收束**: `backend/scripts/build_executive_trade_events.py` 将 K-line 加载从 chunked query loop 改为临时 code 表单次 JOIN，并把 `_apply_forward_returns()` 从 per-event 全历史扫描改为 per-code date/close index + `bisect_right` 定位 T+1 入场；新增 `backend/tests/test_build_executive_trade_events.py` 覆盖乱序价格索引、重复 code 去重、daily/qfq bulk JOIN 过滤。`scripts/chunkyctl audit --run --scope backend/scripts/build_executive_trade_events.py --scope backend/tests/test_build_executive_trade_events.py` PASS；该文件已不再出现在 complexity diff new HIGH。
- **Feature association complexity diff 收束 + baseline 截断误报修正**: `backend/scripts/build_feature_association_duck.py` 去掉 DDL split execute loop，将 holdout fold 统计从每折 DROP/CREATE 临时表改为 fold range 过滤 base table + 一次性 fold row-count JOIN，并把 pairwise correlation / cluster assignment / horizon sensitivity 拆成有界 helper；`backend/tests/pipeline/test_feature_association_duck.py` 7 passed，`scripts/chunkyctl audit --run --scope backend/scripts/build_feature_association_duck.py --scope backend/tests/pipeline/test_feature_association_duck.py` PASS。清掉该文件后发现旧 `complexity_baseline.json` 只记录前 40 条，导致后面的历史 HIGH 冒充 `new_high`；已刷新本地忽略 baseline 到 220 条，旧 tooling gate 退役后这些 baseline 只作为 Moth/doctor 的对照证据继续被读取；`doctor --fast` 后续若 FAIL，应优先看 storage WARN、真实数据 freshness/PIT gate 和工作区 dirty，而不是被截断 baseline 误导。
...
### 2026-06-05 storage payload cap recalibration

- `backend/config/storage_retention.yaml`: `fact_technical_trigger.reason_codes_json` 与 `mart_macd_state_history.reason_codes_json` 的 reviewed total cap 已按当前 full-history evidence 调整；`mart_stock_picture_daily.institution_top_json` 新增 `picture_daily` owner、bounded daily picture summary classification、单行 cap 与总量 cap。
- `backend/tests/scripts/test_audit_storage_payloads.py`: 新增 picture institution summary reviewed PASS 测试，并锁住 default policy 中三个当前容量 reviewed rule；既有 recursive、path marker、单行 cap、总量 cap blockers 仍有效。
- 验证: storage payload live audit 为 `PASS / 323 columns / 0 FAIL / 0 WARN / 13 reviewed`；retention dry-run 仍 `candidate_count=0 / compaction.recommended=false`；targeted pytest 17 passed；`scripts/chunkyctl audit --run backend/config/storage_retention.yaml backend/tests/scripts/test_audit_storage_payloads.py goal.md SESSION_HANDOFF.md` PASS；Rule 10 reviewer `APPROVE_WITH_NOTES`。
...
- `backend/scripts/audit_tdx_data_need_coverage.py`: Python 内嵌 NEEDS/PRIORITIES/REASSIGNMENTS 搬到 YAML, 审计脚本 exact-sync 三张治理表并清理 obsolete rows；2026-06-01 还把 blocked summary 拆成 label / family 两层，避免把 `miaoxiang` 这类家族别名误当成 registry source name。
- `backend/services/data_sources/data_routes.py`: 资金流路线从"整体下架"改为"重新评估但生产阻断", 防止 CYQ/画像后续需求被误删。

**need_027 source probe 切片**:
- `backend/services/data_sources/sources/akshare.py`: 新登记 `individual_fund_flow` / `individual_fund_flow_rank` / `stock_fund_flow_individual` capability, 作为 `need_027` probe candidate；当前 `aif10` family 仍不实现 exact flow capability，因此 `miaoxiang` fallback 只是家族层别名，不是可执行生产证据。
- `backend/scripts/probe_source_capability.py`: 新增通用 capability probe CLI, 可按 capability + prefer_source 输出 JSON 摘要或 blocked 证据；默认压掉 `data_sources.registry` 的 fallback warning，只保留结构化 blocked JSON，`--show-registry-warnings` 可恢复原始 fallback log；`need_027` 现场 probe 仍会被 Eastmoney `ConnectionError` / `JSONDecodeError` remote disconnect 卡住，因此能力注册和实际 exact-flow 仍需分开看。
...
- 现场 `probe_source_capability.py --capability individual_fund_flow --prefer-source akshare --kwargs-json '{"stock":"600519","market":"sh"}'` 现在已先清代理再重试，但 Eastmoney 端点仍以 `ConnectionError` / `JSONDecodeError` remote disconnect 失败，因而 `need_027` 继续按 `unknown/blocked` 管理; blocked probe 现在会写入 `mart_data_source_failure_queue` 作为持久 triage evidence;如果 persistence 遇到 DB lock/schema 问题，报告会降级成 structured `persisted.status=error`，不再冒 traceback; `stock_fund_flow_individual` 作为 10jqka 研究侧排行快照已登记但不等同 exact flow；已同步 `goal.md` / `SESSION_HANDOFF.md` / `analysis/workflow_checkpoint.md` / `docs/implementation_plan.md`，而且 blocked summary 现在同时暴露 label / family 视角。
- `backend/scripts/build_fund_flow_rank_snapshot_daily.py` / `backend/tests/test_build_fund_flow_rank_snapshot_daily.py`: 新增 `mart_stock_fund_flow_rank_snapshot_daily` 研究侧排行快照 mart 与对应 root test，登记到 test registry 之后可作为 `stock_fund_flow_individual` 的辅助观测补充，但仍不改变 `need_027` exact-flow blocked 状态；这个快照只适合 research-side 观察，不可替代 exact flow。
...
## 滚动归档 2026-07-02 (只留7天规则: 06-25 前条目自 PROJECT_INDEX 移入)
