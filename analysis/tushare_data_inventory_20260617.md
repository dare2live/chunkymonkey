...
消费状态: **A**=live serving 在用 · **B**=仅本 session 探索脚本用 · **C**=拉了无任何消费(死数据)

| 域 | 表 (raw_tushare_) | 周期 | 行数 | 用? | 消费者 |
|---|---|---|---|---|---|
| **K线复权** | daily + adj_factor | 2019-01~2026-06 | 856万+874万 | **A** | market.price_kline_qfq_tushare(回测主源) |
| 筹码 | cyq_perf | 2018-01~2026-06 | 930万 | B | 9处实验(rally/yushen/episode) |
| 资金流-主 | moneyflow | 2020-01~2026-06 | 739万 | **A** | feature_panel.mf_trend_20 + 实验 |
| 资金流-东财个股 | moneyflow_dc | 2023-09~2026-06 | 384万 | B | rally_multivariate/episode |
...
| **P0** | **ths_index / dc_daily / dc_concept** | 6000 | **板块/概念指数行情** = 直接的板块同期热度/资金轮动(你 §板块维度的真信号, 比聚合个股干净; 板块L2基础率15.6pp已证板块重要) |
...
- **财报线接了没用 (income/forecast/express/fina_mainbz/advrecv)**: 质量/成长/PEAD 因子料, 但当前买点选股已证≈beta, 暂不急; 若转基本面方向再用。advrecv 停在 2021 + mainbz 仅 2025 = 周期残缺, 用前须回填。
...
3. 拉数据不是瓶颈(信息维度够); 瓶颈是买点≈beta(已证), 故新数据优先级服从"能否改善出场/仓位/板块条件层", 不再为买点选股盲目拉。
