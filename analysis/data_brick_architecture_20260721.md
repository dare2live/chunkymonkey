...
| 4 | **Dual-plane legacy raw** | formal 绿但 mart 仍 ssot raw | S7 inventory **23 ssot**；B1 dc_member → fact_dc_member_daily COMPAT；B2 limit+moneyflow(+dc)+index_daily+top_inst seat COMPAT |
...
| 域 | Gap | 下一刀 |
|---|---|---|
| **S7 legacy plane** | 23/46 ssot；B1 dc_member → fact_dc_member_daily observation-date PIT；B2 limit+moneyflow(+dc)+index_daily+top_inst seat → fact_* COMPAT | next = typed hard-stop 长尾；禁 pulse-mart theater / fake range PIT |
...
| **B2** | S7 flow/limit L0 | limit+moneyflow(+dc)+index_daily+top_inst → fact_stock_* **PARTIAL** | raw → compatibility (membership+flow+limit+seat done) |
