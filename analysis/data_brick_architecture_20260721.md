...
| **Derive variable (L2/L3)** | Tier1/2 publish job；FeatureBlock builder；`pipeline/process` | S7 **PARTIAL**（28/46 ssot） | 默认 legacy UNION |
...
| 4 | **Dual-plane legacy raw** | formal 绿但 mart 仍 ssot raw | S7 inventory **28 ssot**；dc_member、moneyflow drill L0 |
...
| 域 | Gap | 下一刀 |
|---|---|---|
| **S7 legacy plane** | 28/46 ssot；B2 `limit_list_d`→`fact_stock_limit_daily` COMPAT；moneyflow drill + dc_member still ssot | next = B2 moneyflow stock-day **或** B1 PIT；禁 pulse-mart theater |
...
| 序 | 切片 | 内容 | 退出 |
|---:|---|---|---|
| **B1** | S7 membership | dc_member formal\|sunset | ssot ↓；moth green |
| **B2** | S7 flow/limit L0 | `limit_list_d` → `fact_stock_limit_daily` **PARTIAL**；moneyflow residual | raw → compatibility (limit done；flow next) |
| **B3** | S7 长尾 ssot | `legacy_raw_plane.yaml` 逐域 | 单域零 ssot 或 fill 文档 |
