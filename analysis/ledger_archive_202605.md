# Ledger Archive 2026-05 (cut from project_state_ledger.md on 2026-06-11)

> 2026-05-27 架构重构执行计划全文. 已被 goal.md (2026-06-05 split) 取代,
> 仅作历史证据. Authority = goal.md + docs/implementation_plan.md.
...
| 业务推进 | 暂停 | 不做 300616 五公式、前端公式视图、Optuna/provider 全量跑批 |
...
| 优先级 | 文档 | 用途 |
|---:|---|---|
| 1 | `AGENTS.md` | Codex 操作政策: dirty worktree、CodeGraph+complexity、provider jobs、删除治理 |
...
| 5 | `docs/engineering_governance.md` | 第一性原理/奥卡姆、CodeGraph+complexity、测试工具、agents、provider jobs、删除治理 |
| 6 | `docs/data_product_contract.md` | 数据需求、血缘、画像、市场感知支持、前端契约 |
| 7 | `docs/strategy_validation_contract.md` | 回测、Optuna/provider jobs、paper_sim、forward、promotion、主升浪验证边界 |
...
| M5 全量 Optuna/provider job | 之前 29/34 无 search space 白跑、200 只深主板偏样本 | `backtest_preflight` + `plan_validator` + `experiment_jobs` plan 全 PASS |
...
| B | P0/P1 | startup/tooling: `startup_tooling=12`、部分 `config_project/tests/audit_gate_scripts` | 验收 `chunkyctl`、test-tool registry、Rule 10、safe_commit dry-run、provider job contract；通过后单独 stage/commit | test-tool scope PASS；`test_chunkyctl.py`、`test_safe_commit.py`、工具脚本 py_compile/pytest PASS；Rule 10 verdict 可写入 commit message |
...
若修改触及回测/验证/Optuna/provider job 入口，还必须先看:
...
> 2026-06-06 note: Stage-opt source-aware density diagnostics 已完成。`audit_stage_opt_candidate_supply.py` 现在在 signal rows 中保留 `source_id`，输出 source/registry/family/stage blocked density cells，并用 source-specific `kline_signal_row_buckets` 防止混合 source key 误归因；混合 `fact_technical_trigger` + `mart_macd_state_history` 回归已覆盖。短 live audit `2026-06-01..2026-06-05` 仍为 `WARN`，`signal_kline_coverage_pct=100.0`，`below_min_signals=12155`，top source-aware cell 为 `fact_technical_trigger × live × reversal × stage 4`，`keys_blocked=3385`，bucket `1=1202 / 2=2183`。验证：test-tool audit PASS，stage-opt/chunkyctl pytest `64 passed`，`scripts/chunkyctl audit --run ...` PASS，Rule 10 reviewer先 `REQUEST_CHANGES` 指出混合 source bucket 归因问题，修复后复审 `APPROVE`。
> 2026-06-06 note: Stage-opt freshness RCA 复核完成。当前可信 K-line max 是 `2026-06-04`，`latest_completed_trade_date` 为 `2026-06-05`，所以本轮 freshness repair 目标先到 `2026-06-04`。`fact_signal_context` 与 reversal `fact_technical_trigger` 均停在 `2026-06-02`，因此最小串行修复顺序是先 `build_signal_context.py --start 2025-01-01 --write-start 2026-06-03 --end 2026-06-04`，再 `build_formula_signals_history.py --start 2025-01-01 --write-start 2026-06-03 --end 2026-06-04 --formula reversal_1m_mild reversal_1m_deep reversal_1w`。写 DB 前后必须保持 DuckDB writer 串行，并 rerun short + YTD `audit_stage_opt_candidate_supply.py` 与 `scripts/chunkyctl doctor --fast`。
> 2026-06-06 note: Stage-opt freshness repair 已串行执行并复核。`build_signal_context.py --start 2025-01-01 --write-start 2026-06-03 --end 2026-06-04` 写入 `10,298` rows，`fact_signal_context` 当前 max=`2026-06-04`；`build_formula_signals_history.py --start 2025-01-01 --write-start 2026-06-03 --end 2026-06-04` 对 18 个 LIVE formula 写入 `14,838` rows，`fact_technical_trigger` 当前 max=`2026-06-04`；`build_macd_state_history.py --start 2026-05-29 --end 2026-06-04` 写入 `14,594` rows，`mart_macd_state_history` 当前 max=`2026-06-04`。最新实际 5 个 K 线日 default stage-opt audit (`2026-05-29..2026-06-04`) 为 `source_freshness_warnings=0`, `ready_keys=3010`, `ready_coverage_pct=12.72`; reversal-only 为 `ready_keys=1401`, `ready_coverage_pct=15.2`; YTD reversal-only为 `ready_keys=21530`, `ready_coverage_pct=62.7`。`scripts/chunkyctl doctor --fast` 仍为 WARN，但剩余主要是 `need_027` exact-flow blocker 和 7 个 warning-only data_health freshness 维护项；执行面、Moth、universe、worktree PASS。
> 2026-06-06 note: `need_027` no-persist gate 前置条件已补强。`probe_source_capability.py --need027-exact-flow-gate` 现在输出 source-level `source_preflight`，TuShare source-group 会显式报告 token env 和 package installed 状态；`backend/requirements.txt` 已登记 `tushare>=1.4.0`。当前本机 live no-persist gate 仍为 `BLOCKED`: AkShare `akshare_remote_disconnected=3`; TuShare `tushare_token_missing=3` 且 local preflight `tushare_package_missing=3`; `post_probe_gates` 全部 `not_checked`。尝试用 Homebrew-managed Python 直接 `python -m pip install 'tushare>=1.4.0'` 被 PEP 668 `externally-managed-environment` 阻止，因此下一步应在项目 runtime/virtualenv 安装 requirements 并设置 `TUSHARE_TOKEN`/`TUSHARE_PRO_TOKEN`/`TS_TOKEN` 后重跑 no-persist gate，不能用 `--break-system-packages` 硬装。
> 2026-06-06 note: DB capacity/retention 复核确认容量压力不是外部 `NO2`、压缩备份、`.bak/.gz/.zst/.zip` 或 session snapshot loop；`data/` 下仅 `smartmoney.duckdb` 是 34G 级容量驱动。主要风险面是 `smartmoney.duckdb` 内部多版本宽表和 cache 重叠：`mart_p0a_feature_label_panel_v3/v4/v5/unified_v1`、`fact_feature_panel_candidate`、`fact_feature_panel_tdx_keep_challenger`、两张 `mart_feature_rank_matrix_cache_*`。`plan_storage_retention.py` 当前 `candidate_count=0`，`audit_storage_retention_consumers.py` PASS，`audit_storage_payloads.py` PASS；生产 delete/VACUUM 仍 blocked，下一步只能先迁移或退役 panel/cache consumers，并在 copied DuckDB 上验证 manifest/rollback。

### Compute Job 规则保持

项目内 GCP 执行面已删除。本计划不启动 provider compute。后续重回测、训练、参数探索或外部 provider 任务必须先通过:

```bash
scripts/chunkyctl jobs --family <job-family> --backend local
```

任何新 provider backend 必须先落 `experiment_jobs.yaml`、adapter artifact contract、测试注册表和 active docs；不得通过旧脚本、注释、隐藏 flag 或 latch 恢复已删路径。
