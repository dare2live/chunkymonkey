...
### 1.0 第一性原理 + 奥卡姆剃刀 (项目宪法, 最高优先级)

> 反例: dim_all_ever_listed 用快照判退市 → 573 只活跃股误标; get_active_universe 98 处引用 + 42 处 bypass; 
> get_limit_up_pct 两处重复实现; ST 检查散落 10+ 文件. 全是因为没问"真的需要这一层吗".

**第一性原理**: 回到事实源头, 不引入中间层.
- K 线有数据 = 在交易. 不需要 "活跃列表" 表.
- 交易日历 = 日期真相. 不需要 "交易日 flag" 列.
- 交易所规则 = 涨跌停真相. 不需要多处 hardcode.

**奥卡姆剃刀**: 能删的删, 能合的合, 能配置化的不写代码.
- 多一张表 = 多一个 sync 失败的风险
- 多一个函数 = 多一个被 bypass 的可能
- 同一逻辑出现两次 → 立即合并, 不等以后

**强制执行 checklist** (写新模块 / 改架构 / 加表前必答):
...
**架构模式** (模块 + 数据表 + 配置文件):
```
config/*.yaml     — 规则 (涨跌停/前缀/ST/成本/排除)
services/*.py     — 模块 (读 config, 查真相源, 返回结果)
price_kline_tdxhub — 交易真相源 (有数据 = 在交易)
dim_active_a_stock — 仅用于 code→name 映射, 不用于 universe 判断
交易日历          — 日期真相源
```
