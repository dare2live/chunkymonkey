...
- **后台跑批 wrapper 必须 2>&1+EXIT 码落日志 (2026-07-02, kpl_list 回填 83min 静默死亡)**: 无 stderr 捕获的后台进程异常死亡零痕迹, tail 监听进程还活着造成假象。判活 pgrep python 主进程非 tail; 日志停更 >10min 即查活。详 chunkymonkey-ops skill §2 (2026-07-02 批) + §2.5 多 agent 并行纪律。
- **warmup 产物表增量检测禁 NOT IN 全集 (2026-07-02, B2 实现 agent 踩)**: 形态/特征类表因 warmup 前缀日期永远不产出行, 增量用 "源日期 NOT IN (SELECT trade_date FROM 产物表)" → warmup 期全部日期每次都算 missing (实测 251 天噪音, 幂等破坏)。修: watermark 语义 `> MAX(trade_date)`; 中间历史回补走 rebuild。同类表 (fact_stock_form_daily) 一律 watermark。segments/pulse 无 warmup 每天有行, NOT IN 才合法。
