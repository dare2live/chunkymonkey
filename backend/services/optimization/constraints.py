...
class HardConstraints:
    ...
    max_acceptable_drawdown:  float = -0.25  # 平均 max_dd ≥ -25%
    worst_single_loss:        float = -0.30  # 单笔最差 ≥ -30%
    max_loss_streak:          int   = 5      # 连续亏损 ≤ 5 笔
    min_traded:               int   = 5      # 至少开仓 5 次
...
def passes_hard_constraints(
    ...
    if len(traded) < constraints.min_traded:
        return False, f"min_traded {len(traded)} < {constraints.min_traded}"
    ...
    if avg_dd < constraints.max_acceptable_drawdown:
        return False, f"avg_dd {avg_dd:.2%} < {constraints.max_acceptable_drawdown:.2%}"

    # 2. 单笔最差不能超
    worst_loss = float(rets.min())
    if worst_loss < constraints.worst_single_loss:
        return False, f"worst_loss {worst_loss:.2%} < {constraints.worst_single_loss:.2%}"

    # 3. 最长连亏不能超
    streak = _max_loss_streak(rets)
    if streak > constraints.max_loss_streak:
        return False, f"loss_streak {streak} > {constraints.max_loss_streak}"
