...
def load_stock_scoring_breakdown(conn, stock_code: str) -> Optional[dict]:
    ...
    return build_stock_scoring_breakdown_payload(stock_row, object_id=stock_code)


def build_stock_scoring_breakdown_payload(stock_row: Optional[dict], object_id: Optional[str] = None) -> Optional[dict]:
    if not stock_row:
        return None

    stock = apply_forecast_score_aliases(dict(stock_row))
    quality_score_source = stock.get("company_quality_score_source") or "stock_scoring_v2"
    quality_snapshot_date = stock.get("quality_feature_snapshot_date")

    return {
        "ok": True,
        "card_type": "stock",
        "object_id": object_id or stock.get("stock_code"),
        "discovery_score": stock.get("discovery_score"),
        "company_quality_score": stock.get("company_quality_score"),
        "company_quality_score_source": quality_score_source,
        "quality_feature_snapshot_date": quality_snapshot_date,
        "quality_snapshot_date": quality_snapshot_date,
        "stage_score": stock.get("stage_score"),
        "forecast_score": stock.get("forecast_score"),
        "forecast_score_effective": stock.get("forecast_score_effective"),
        "raw_composite_priority_score": stock.get("raw_composite_priority_score"),
        "composite_priority_score": stock.get("composite_priority_score"),
        "composite_cap_score": stock.get("composite_cap_score"),
        "composite_cap_reason": stock.get("composite_cap_reason"),
        "stock_archetype": stock.get("stock_archetype"),
        "priority_pool": stock.get("priority_pool"),
        "priority_pool_reason": stock.get("priority_pool_reason"),
        "score_highlights": stock.get("score_highlights"),
        "score_risks": stock.get("score_risks"),
        "path_state": stock.get("path_state"),
        "data_completeness": stock.get("data_completeness"),
        "stage": extract_stage_payload(stock),
        "forecast": extract_forecast_payload(stock),
        "turtle": extract_turtle_payload(stock),
        "formula": "Composite = 发现35% + 质量30% + 阶段20% + 生效预测15% + 海龟执行覆盖；Stage<40 封顶69；Quality<45 且非周期/事件型封顶64；A池要求 Composite≥75 且 Stage≥50 且 Quality≥55 且 Discovery≥50",
        "factors": {
            "leader": {
                "inst": stock.get("leader_inst"),
                "score": stock.get("leader_score"),
                "source": "mart_institution_profile.quality_score",
                "weight": "30%",
            },
            "industry_match": {
                "stock_industry": stock.get("industry_level2") or stock.get("sw_level2"),
                "source": "mart_current_relationship 当前行业(level2优先, level1回退) vs leader best_industry",
                "weight": "25%",
            },
            "consensus": {
                "count": stock.get("consensus_count"),
                "source": "mart_current_relationship 中 quality_score ≥ 75th 的机构数",
                "weight": "10%",
            },
            "quality": {
                "score": stock.get("company_quality_score"),
                "source_type": quality_score_source,
                "snapshot_date": quality_snapshot_date,
                "source": "dim_stock_quality_latest.quality_score_v1 对齐最新财报后生效，否则回退 scoring.py stock_scoring_v2",
            },
            "timeliness": {
                "notice_age_days": stock.get("notice_age_days"),
                "notice_date": stock.get("latest_notice_date"),
                "source": "notice_date 距今天数（30日=100分，180日=0分）",
                "weight": "10%",
            },
            "price_path": {
                "entry_price": stock.get("price_entry"),
                "inst_ref_cost": stock.get("inst_ref_cost"),
                "inst_cost_method": stock.get("inst_cost_method"),
                "premium_pct": stock.get("premium_pct"),
                "premium_bucket": stock.get("premium_bucket"),
                "follow_gate": stock.get("follow_gate"),
                "return_to_now": stock.get("return_to_now"),
                "path_state": stock.get("path_state"),
                "source": "market_data.db 日K线计算",
            },
            "setup": {
                "tag": stock.get("setup_tag"),
                "priority": stock.get("setup_priority"),
                "reason": stock.get("setup_reason"),
                "confidence": stock.get("setup_confidence"),
                "level": stock.get("setup_level"),
                "institution": stock.get("setup_inst_name"),
                "event_type": stock.get("setup_event_type"),
                "industry_name": stock.get("setup_industry_name"),
                "setup_score_raw": stock.get("setup_score_raw"),
                "setup_execution_gate": stock.get("setup_execution_gate"),
                "setup_execution_reason": stock.get("setup_execution_reason"),
                "industry_skill_raw": stock.get("industry_skill_raw"),
                "industry_skill_grade": stock.get("industry_skill_grade"),
                "followability_grade": stock.get("followability_grade"),
                "premium_grade": stock.get("premium_grade"),
                "report_recency_grade": stock.get("report_recency_grade"),
                "reliability_grade": stock.get("reliability_grade"),
                "crowding_bucket": stock.get("crowding_bucket"),
                "crowding_yield_raw": stock.get("crowding_yield_raw"),
                "crowding_yield_grade": stock.get("crowding_yield_grade"),
                "crowding_stability_raw": stock.get("crowding_stability_raw"),
                "crowding_stability_grade": stock.get("crowding_stability_grade"),
                "crowding_fit_raw": stock.get("crowding_fit_raw"),
                "crowding_fit_grade": stock.get("crowding_fit_grade"),
                "crowding_fit_sample": stock.get("crowding_fit_sample"),
                "crowding_fit_source": stock.get("crowding_fit_source"),
                "report_age_days": stock.get("report_age_days"),
