"""main_rally B1 stock-state block (Phase F / F2 measured ablation).
...
Restricts B0's setup eligibles to codes with Tier1 stock state
(``axis_trend=up`` or ``is_breakout_event``) on the signal day when coverage
is sufficient; otherwise inconclusive with an explicit coverage reason.
...
BLOCK_ID = "B1"
FEATURE_BLOCK_ID = "stock_state_stage_pattern_v1"
REASON_B1_SCAFFOLD_NO_MEASURED_EDGE = "main_rally_b1_scaffold_stock_state_not_measured"
REASON_B1_DEPENDS_ON_B0 = "main_rally_b1_requires_b0_protocol_context"
...
class StockStateFeatureBlock:
    ...
    status: str = "declared"
    note: str = (
        "Intersect B0 pivot-confirmed-setup eligibles with Tier1 stock state "
        "(axis_trend=up or is_breakout_event) at signal_date; same "
        "snapshot/folds/costs/paper as B0"
    )
    definition_version: str = DEFINITION_VERSION
    config_hash: str = "trend_up_or_breakout_v1"
...
def finalize_b1_verdict(
    ...
    *,
    ...
    """B1 verdict: coverage → edge gates → strict holdout lift vs B0.
    ...
    if (
        ...
        if wants_accept:
            raise CanaryScopeOverclaimError(
                "main_rally B1 cannot accept under canary scope; "
    ...
    if edge.passed and stability.passed:
        return ExperimentVerdict(
            ...
            details={
                **details,
                "note": (
                    "B1 protocol + accept edge gates + holdout lift vs B0 "
                    "passed"
                ),
            },
        )
    ...
    if edge.passed and not stability.passed:
        return ExperimentVerdict(
            ...
            details={
                **details,
                "note": (
                    "B1 short-window edge gates passed but holdout return "
...
def run_b1_scaffold(
    *,
    ...
    state_by_day: Mapping[str, Any] | None = None,
    accepted_nominal_partitions=None,
) -> tuple[MainRallyB1Run, ExperimentVerdict]:
    run = build_b1_run(
        snapshot=snapshot,
