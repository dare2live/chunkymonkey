...
strata (考古 §3.4): 申万 L1/L2 as-of (raw_tushare_index_member_all, in_date<=底<=out_date, 含
  is_new='N' 历史区间 = 真 PIT) + 长底桶; B1/B2 列本次留 NULL (见 TODO)。
...
读: market.price_kline_qfq_tushare (K线真相源, ATTACH mk) + reference.dim_trading_calendar
  (ATTACH ref) + tushare_raw (独立只读连接 raw_conn — universe.load_st_calendar 需默认 catalog
  直连, 不能跨 ATTACH catalog 解析未限定表名; stock_basic / index_member_all 同连接读)。
...
def _write_strata(conn, raw_conn, cfg: dict, built_at) -> None:
    """申万 L1/L2 as-of (真 PIT, 含 is_new='N' 历史区间) + 长底桶; B1/B2 列 NULL (docstring TODO)。"""
    ...
    conn.execute(f"""
        CREATE TABLE {STRATA_TABLE} (
            ...
            mktcap_seg    VARCHAR,   -- TODO B1 dim_stock_segment_daily as-of (smartmoney 锁释放后回填)
            turnover_seg  VARCHAR,   -- TODO B1 同上
    ...
    conn.execute(f"""
        ...
        LEFT JOIN LATERAL (
            ...
            ORDER BY m.in_date DESC LIMIT 1) sec ON TRUE""", (built_at,))
...
def rebuild(conn=None, data_end=None, raw_conn=None) -> dict:
    ...
    conn: feature_store 写连接 (须已 ATTACH mk=market / ref=reference; None=自管+ATTACH)。
