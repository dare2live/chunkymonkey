...
Does NOT flip product TRUSTED by itself.  Evaluates evidence against explicit
criteria and exposes a product-visible gate status.

Criteria (all required for READY_TO_PROMOTE):
  1. accepted canonical margin covers the day under current contract (v3+)
  2. core venues SSE+SZSE present on accepted rows
  3. shadow verdict is EXTERNAL_HONEST_SHADOW (legacy sum == SSE+SZSE; no BSE delta)
  4. pulse serve surface has switched to accepted external_aggregate
     (``pulse_source_accepted=True``) — until then status stays PENDING_SERVE_CUTOVER
  5. explicit promote_allowed flag (yaml/config) — default False; never silent thaw

Population kind stays ``external_aggregate`` even after promote.  Project-universe
...
def evaluate_margin_pulse_promote_gate(
    ...
    *,
    ...
    # shadow may lag or include BSE; accepted-ready alone advances the gate to
    # PENDING_SERVE_CUTOVER without inventing TRUSTED.
    ...
    # Product trust stays UNTRUSTED until a separate promote cutover consumes
    # READY_TO_PROMOTE. This gate never returns TRUSTED.
    product_trust = "UNTRUSTED"
    if status == "READY_TO_PROMOTE":
        notes.append("ready_but_awaiting_promote_cutover_commit")
        product_trust = "UNTRUSTED"  # still — promote is a separate knife
