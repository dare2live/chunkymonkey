"""④ 存储 (Store) — 跨层生命周期治理: 水位刷新 / retention / 健康报告 / 告警送达。

旧 daily_update.sh: Step 2.97 (watermark refresh) + Step 4 (retention plan dry-run) + Step 5 (data-health report + 告警 + degraded 汇总)。
...
def run_store(ctx: PipelineContext) -> None:
    ctx.log("=== ④ 存储 STORE (水位/retention/报告/告警) ===")
    ...
    if not ctx.dry:
        ctx.run_script("backend/scripts/plan_storage_retention.py",
                       degraded_msg="retention plan 失败 (非 fatal)")
    else:
        ctx.log("DRY: 跳过 retention plan")
