...
def _evaluate_setup_candidate(holder: dict, profile: dict, stock_industry: dict,
    ...
    for level, industry_name in (
        ("level3", stock_industry.get("sw_l3")),
        ("level2", stock_industry.get("sw_l2")),
        ("level1", stock_industry.get("sw_l1")),
    ):
...
def _score_discovery(
    ...
    """
    stock_sw1 = stock_ind.get("sw_l1")
    stock_sw2 = stock_ind.get("sw_l2")
...
def calculate_stock_scores(conn) -> int:
    ...
    for code, ind in _ind_map.items():
        ...
        stock_industry_name[code] = ind.get("sw_l2") or ind.get("sw_l1", "")
    ...
    for row in fin_rows:
        ...
        if d.get("sw_l2"):
            fin_groups.setdefault(("l2", d["sw_l2"]), []).append(d)
        if d.get("sw_l1"):
            fin_groups.setdefault(("l1", d["sw_l1"]), []).append(d)
    ...
    def _fin_rank(stock_code: str, metric: str, sw_l2: Optional[str], sw_l1: Optional[str]) -> Optional[float]:
        if sw_l2 and fin_group_sizes.get(("l2", sw_l2), 0) >= 15:
            rank = fin_pct_map.get(("l2", sw_l2, metric, stock_code))
            if rank is not None:
                return rank
        if sw_l1 and fin_group_sizes.get(("l1", sw_l1), 0) >= 20:
            rank = fin_pct_map.get(("l1", sw_l1, metric, stock_code))
            if rank is not None:
                return rank
    ...
    try:
        ...
        for row in indicator_rows:
            ...
            if d.get("sw_l2"):
                indicator_groups.setdefault(("l2", d["sw_l2"]), []).append(d)
            if d.get("sw_l1"):
                indicator_groups.setdefault(("l1", d["sw_l1"]), []).append(d)
        indicator_metrics = {
            "roa_ak": False,
    ...
    def _indicator_rank(stock_code: str, metric: str, sw_l2: Optional[str], sw_l1: Optional[str]) -> Optional[float]:
        if sw_l2 and indicator_group_sizes.get(("l2", sw_l2), 0) >= 15:
            rank = indicator_pct_map.get(("l2", sw_l2, metric, stock_code))
            if rank is not None:
                return rank
        if sw_l1 and indicator_group_sizes.get(("l1", sw_l1), 0) >= 20:
            rank = indicator_pct_map.get(("l1", sw_l1, metric, stock_code))
            if rank is not None:
                return rank
