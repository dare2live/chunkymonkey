"""main_rally B2 market-sensing block (Phase F / F3 measured ablation).

Adds one named FeatureBlock on top of B0 pivot-confirmed setups under the
same disclosure ``DatasetSnapshot``, folds, costs and paper execution.
Gates B0's setup-eligible codes on EOD project-board breadth risk-on
(``MarketContextSnapshot``, mirrors ``institution_follow_b2``) per signal
day; legacy ``market_pulse`` mart is refused ``UNTRUSTED`` and a missing
``available_at`` fails closed — never a silent fallback.

Never claimable=true without both (a) accept edge gates passed and (b) a
strict holdout return lift vs B0 (``REQUIRE_HOLDOUT_LIFT_VS_B0``) — equal or
worse holdout vs B0 is not independent evidence and must reject, not accept.
"""
...
REASON_B2_SCAFFOLD_NO_MEASURED_EDGE = "main_rally_b2_scaffold_market_sensing_not_measured"
REASON_B2_DEPENDS_ON_B0 = "main_rally_b2_requires_b0_protocol_context"
...
class MarketSensingFeatureBlock:
    ...
    status: str = "declared"
    note: str = (
        "Gate B0 setup-eligible codes on EOD project-board breadth risk-on "
        f"(method={METHOD_ID}; population={POPULATION_KIND}); refuse "
        "UNTRUSTED pulse mart; fail closed on missing available_at; same "
        "snapshot/folds/costs/paper as B0; never widens B0's setup eligibles"
    )
    definition_version: str = DEFINITION_VERSION
    config_hash: str = f"risk_on_adv_dec_ratio_ge_{MIN_ADV_DEC_RATIO_RISK_ON:.1f}_v0"
...
def build_b2_run(
    *,
    ...
    b0_run: MainRallyB0Run | None = None,
    ...
    context_by_day: Mapping[str, Any] | None = None,
    source: str = SOURCE_NOMINAL_BARS,
    accepted_nominal_partitions=None,
) -> MainRallyB2Run:
    ...
    run_id = uuid4().hex[:12]
    experiment_id = f"{STRATEGY_PACKAGE}:{BLOCK_ID}:{base.snapshot_id}:{run_id}"
    fb_status = "declared"
    notes = [
...
def finalize_b2_verdict(
    ...
    *,
    ...
    """B2 verdict: coverage/trust → edge gates → strict holdout lift vs B0.

    Never fake improve/accept. claimable=true requires accept edge gates
    passed *and* a strict holdout return lift vs B0
    (``REQUIRE_HOLDOUT_LIFT_VS_B0``); equal/worse holdout vs B0 rejects.
    """
    ...
    if (
        ...
        if wants_accept:
            raise CanaryScopeOverclaimError(
                "main_rally B2 cannot accept under canary scope; "
    ...
    if not measured.coverage.sufficient:
        return ExperimentVerdict(
            ...
            details={
                ...
                "strategy_release": False,
                "note": (
                    "MarketContextSnapshot coverage/trust insufficient for "
                    "window; not a fake B2 improve; pulse mart refused when "
                    "UNTRUSTED"
                ),
            },
        )
...
def run_b2_scaffold(
    *,
    ...
    source: str = SOURCE_NOMINAL_BARS,
    accepted_nominal_partitions=None,
) -> tuple[MainRallyB2Run, ExperimentVerdict]:
    run = build_b2_run(
        snapshot=snapshot,
