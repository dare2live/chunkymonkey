"""Measured B1: Tier1 stock-state conditioning of main_rally setups (Phase F / F2).

Adds one named FeatureBlock on top of B0 pivot-confirmed setups under the
identical ``DatasetSnapshot``, folds, costs and paper execution as B0.
Restricts signal-day eligibility to the intersection of (a) B0's
...
REASON_B1_STATE_COVERAGE_INSUFFICIENT = (
    "main_rally_b1_stock_state_coverage_insufficient"
)
REASON_B1_PAPER_MEASURED = "measured_main_rally_b1_paper_stock_state_conditioned"
REASON_B1_NO_B0_CONTEXT = "main_rally_b1_requires_measured_b0_context"
...
def measure_b1_paper(
    ...
    *,
    ...
    """Intersect B0 setup eligibles with Tier1 stock-state under identical WF."""
    ...
    state_eligible = eligible_by_day_from_state(state_by_day)
    intersected: dict[str, set[str]] = {}
    for day in days:
        setup_codes = setup_eligible.get(day) or set()
        state_codes = state_eligible.get(day) or set()
        intersected[day] = {c for c in setup_codes if _code6(c) in state_codes}
