...
数据来源：
  ...
  - 行业映射：dim_stock_dc_industry (tdx_l1/tdx_l2, 以 tdx_l1_name 作板块名)
...
def calc_sector_momentum(smart_conn, mkt_conn) -> int:
    ...
    for row in smart_conn.execute(
        "SELECT stock_code, tdx_l1_name FROM dim_stock_dc_industry WHERE tdx_l1_name IS NOT NULL AND tdx_l1_name != ''"
    ).fetchall():
...
def calc_dual_confirm(smart_conn) -> int:
    ...
    events = smart_conn.execute(f"""
        ...
        LEFT JOIN dim_stock_dc_industry si ON e.stock_code = si.stock_code
