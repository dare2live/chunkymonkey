...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="L0 裸K线基准 Tier-1 RankIC (默认参数测量, 防泄露三门固化)")
    ...
    run_id = args.run_id or f"l0_baseline_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
