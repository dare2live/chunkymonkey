#!/usr/bin/env python3
"""P0a feature_label_panel v5 build CLI — Phase 4 features wired into canonical panel.

Codex round 20 verdict: Phase 4 7 modules (50 features) 都没 wire 到生产 panel.
v4 = v4 - 5 leak cols (Pattern 10) (mcap_decile, beta_60d/zscore, sector_momentum 9 raw, survey 4 raw, tom 7 inline).
...
from services.labels.feature_join_v5 import (
    FEATURE_PANEL_VERSION_V5,
    build_p0a_feature_label_panel_v5,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s", datefmt="%H:%M:%S")
log = logging.getLogger("build_p0a_v5")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build mart_p0a_feature_label_panel_v5")
    ...
    log.info(f"=== Build feature_label_panel_v5 (range {args.start_date} → {args.end_date}) ===")
    ...
    # v5 audit cols — mcap_decile/beta_60d dropped (Pattern 10 leak fix)
    audit_cols = [
        ("capital_flow lhb", "lhb_count_30d"),
        ("capital_flow exec", "exec_net_signal"),
    ...
    try:
        ...
        r = audit.execute(
            f"SELECT {', '.join(exprs)} FROM mart_p0a_feature_label_panel_v5"
        ).fetchone()
