...
def main():
    ...
    for ci in range(len(uniq)):
        ...
        for ei in entries_A(c, state, v):
            ...
            rows.append(dict(entry_date=dates[ei + 1] if ei + 1 < len(dates) else dates[ei], ret=r,
                             ...
                             has_inst=1 if inst_n > 0 else 0, has_lhb=1 if lhb_n > 0 else 0, has_buyrpt=1 if buy_n > 0 else 0))
    panel = pd.DataFrame(rows)
    panel["label"] = (panel["ret"] > 0).astype(int)
    panel["seg"] = np.where(panel["entry_date"] < OOS_CUT, "TRAIN", "OOS")
    panel["any_conf"] = ((panel["has_inst"] + panel["has_lhb"] + panel["has_buyrpt"]) > 0).astype(int)
    ...
    print(f"\n另类数据(龙虎榜+券商预期)买点因子检验 (PRIMARY=二次突破, 看机构conviction是否改善; 含成本)")
    ...
    for f in ["lhb_cnt", "lhb_net", "inst_cnt", "inst_net", "rpt_cnt", "rpt_buy", "tp_upside", "has_inst", "has_lhb", "has_buyrpt"]:
    ...
    judges = {"n": len(panel), "any_conf_cov": round(panel["any_conf"].mean(), 4),
              ...
              "cells": {f"{s}_{g}": res[(s, g)] for (s, g) in res}, "summary": verdict[:150]}
