...
def run_reproduction():
    ...
    # Output raw stale_rows (first 5 for verification if needed, or just count)
    print(f"raw stale_rows count: {len(stale_rows)}")
    if stale_rows:
        print(f"Sample stale_rows (code, max_date): {stale_rows[:5]}")
    ...
    print(f"suspended codes intersection: {intersected_suspended}")
    print(f"final computed stale_count: {stale_count}")
    print(f"final computed suspended_count: {suspended_count}")
