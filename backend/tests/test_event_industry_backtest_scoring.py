"""backtest_engine / scoring 行业直连口径测试（dim_stock_sw_industry sw_l1/2/3）。"""
...
def test_build_inst_industry_performance_joins_tdx_dim():
    conn = _make_conn()
    try:
        conn.executescript(
            """
        ...
        _create_tdx_dim(
            conn,
            [("600001", "T12", "T1204", "T120401", "信息产业", "计算机", "软件")],
        )
        conn.execute(
...
def test_build_holding_chains_joins_tdx_dim():
    conn = _make_conn()
    try:
        conn.executescript(
            """
        ...
        assert (row["industry_l1"], row["industry_l2"], row["industry_l3"]) == ("T12", "T1204", "T120401")
    finally:
        conn.close()


def test_build_cross_factor_analysis_joins_tdx_dim():
    conn = _make_conn()
    try:
        conn.executescript(
            """
        ...
        _create_tdx_dim(
            conn,
            [(f"6000{i:02d}", "T12", "T1204", "T120401", "信息产业", "计算机", "软件") for i in range(10)],
        )
        conn.execute(
            "INSERT INTO inst_institutions VALUES (?, ?, ?, ?)",
            ("inst_a", "机构A", "机构A", "公募"),
        ...
        assert row["factor_b_value"] == "T12"
    finally:
        conn.close()


def test_load_crowding_fit_lookup_skilled_l3_joins_tdx_dim():
    conn = _make_conn()
    try:
        conn.executescript(
            """
