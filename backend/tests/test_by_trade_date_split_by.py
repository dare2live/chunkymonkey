...
split_by: {param, values} 把一个交易日展开成 len(values) 个独立 API 调用, 是通用机制
(不限于 margin), 本门锁定: 每个交易日必须真实展开成 N 个调用, 每个调用带正确的
split 参数值, 且不破坏无 split_by 声明域的原有行为。
"""
...
def env(monkeypatch):
    ...
    monkeypatch.setattr(sr, "_trading_days", lambda start, end=None: [D])
