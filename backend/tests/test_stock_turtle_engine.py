    ...
    try:
        ...
        conn.execute(
            ...
            INSERT INTO dim_stock_forecast_latest (
                stock_code, model_id, sw_l1_name, sw_l2_name,
...
def test_build_stock_turtle_features_falls_back_to_shared_industry_alias_map(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(
            ...
            lambda _conn: {"600010": {"sw_l1_name": "电子", "sw_l2_name": "芯片"}},
        )
        conn.execute(
            "INSERT INTO mart_stock_trend (stock_code, stock_name, latest_notice_date, latest_report_date) VALUES (?, ?, ?, ?)",
        ...
        row = conn.execute(
            "SELECT sw_l1_name, sw_l2_name FROM dim_stock_turtle_latest WHERE stock_code = ?",
            ("600010",),
        ).fetchone()
        assert inserted == 1
        assert row["sw_l1_name"] == "电子"
        assert row["sw_l2_name"] == "芯片"
    finally:
        mkt_conn.close()
        conn.close()
