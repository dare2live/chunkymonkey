...
def test_build_hsgt_daily_merges_record_payloads(monkeypatch):
    conn = duck_mem()
    try:
        def fake_hsgt(market: str, indicator: str):
            if market == "沪股通":
                return [
                    {
                        "代码": "600000",
        ...
        monkeypatch.setattr(subject.ak, "stock_hsgt_hold_stock_em", fake_hsgt)

        inserted = subject.build_hsgt_daily(conn)
        rows = conn.execute(
            """
            SELECT snapshot_date, stock_code, stock_name, hold_shares
        ...
        assert rows[0]["snapshot_date"] == "20260102"
        assert rows[0]["hold_shares"] == 200.0
    finally:
        conn.close()
