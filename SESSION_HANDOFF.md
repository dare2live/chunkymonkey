...
| Data health | `FAIL`: 32 red / 4 yellow / 341 total; `scripts/chunkyctl doctor --fast` now includes `data_health_snapshot.py` and fails closed on red tables | Review red buckets bucket-by-bucket before trusting freshness claims; missing tables and stale writers are system blockers |
