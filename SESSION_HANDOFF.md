...
Snapshot: `2026-06-01 17:04:33 CST`
...
2026-06-01 also repaired `data_health_snapshot.py` writer timestamp normalization and reran the official `cron_daily.py` with the local API up. The run completed later phases and surfaced the real prompts instead of crashing: `sync_raw` exceeded its 60s budget, while health ended at `0 red / 2 yellow / 342 total` after the feature panel lane refresh, the `capital_behavior` lane cleared `dim_capital_behavior_latest`, the holder/shareholder-plan lane was rebuilt back out of red, and `raw_executive_trade` / `fact_executive_trade_event` were freshly rebuilt by the local writer; GPCW and raw_aif10 are now yellow maintenance only, so the remaining work is lane triage rather than blocker rescue. `doctor` now promotes `blocking_yellow_tables` ahead of generic yellow maintenance so the controller sees blocking-quality yellow assets first, and the `mart_p0b_lambdamart_v6_predictions` ensemble v7 context writer was refreshed to 2026-06-01 so blocking yellow is now 0. The latest smoke then restored `fact_institution_event` through the safe rebuild path, and `mart_architecture_cleanup_plan` was reclassified to on-demand governance, so the remaining health is just `fact_paper_sim_trade` and `raw_margin_daily` as ordinary yellow maintenance items.
