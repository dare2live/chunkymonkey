...
5. `scripts/chunkyctl doctor --fast` now also surfaces the stage-opt `next_action_recommendation` and the `need_027` blocked-gap triage, so upstream candidate-supply triage and main-force source blockers are visible without rerunning the audits manually; when `macd_golden_cross` appears in the weakest cohort, the controller also sees the `fact_technical_trigger` primary-key schema limit, so a schema change is not mistaken for a state-only formula tweak. `need_027` 现在已确认 aif10 家族没有 exact `individual_fund_flow` capability，因此后续动作不是反复试 aif10，而是只有在 registry 新增能力或 route 映射时才重开 exact-flow 恢复线。
