...
- shared-config boundary note: common holding windows, stage thresholds, MACD
  ...
  in shared YAML configs (`formula_shared_windows.yaml`,
  `technical_stage.yaml`, `formula_macd_golden_cross.yaml`,
  `formula_turtle_breakout.yaml`) plus the shared loader; formula-specific
...
|---|---|
| Branch | main |
| HEAD | `04044851 feat: finish config-owned formula loader cleanup | Codex-Reviewed: APPROVE_WITH_NOTES (chunkymonkey-review-gate) | test pass: formula_engine 70 passed, audit PASS, docs graph PASS, codegraph sync PASS | post-fix-audit cleanup verified 无残留 no stale` |
| 最近 24h commits | 116 |
| 未 commit 文件 | 0 |
