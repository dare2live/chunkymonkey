...
`wilson` as the dominant fail reasons. The new TDX need audit also surfaces
source registration facts, showing `need_027`'s preferred `akshare` is
registered while the declared fallback label `miaoxiang` is not a registry
source name, so that fallback remains conceptual.
