...
| Data health | `FAIL`: 9 red / 20 yellow / 341 total; `scripts/chunkyctl doctor --fast` now includes `data_health_snapshot.py`, and `warning/monitor_only` assets are capped to yellow before verdict aggregation | Review the 9 blocking red buckets bucket-by-bucket before trusting freshness claims; missing tables and stale writers only block when their quality gate is blocking, and each red/yellow row now carries a `writer_prompt` so triage can land on the owner/sync_step without relying on chat memory |
...
2026-06-01 also repaired `data_health_snapshot.py` writer timestamp normalization and reran the official `cron_daily.py` with the local API up. The run completed later phases and surfaced the real prompts instead of crashing: `sync_raw` exceeded its 60s budget, while health ended at `9 red / 20 yellow / 341 total` and the red/yellow rows exposed owner/sync-step prompts for triage.
