...
Snapshot: `2026-06-02 00:41:48 CST`

## Risk First

| Item | State | Action |
|---|---|---|
| Worktree | clean | If dirty, run `scripts/chunkyctl worktree --format markdown` and resolve by bucket; never use `git add .` |
...
| Storage payload | `PASS`: 320 scanned / 0 FAIL / 0 WARN / 11 reviewed PASS | Reviewed columns are governed by `backend/config/storage_retention.yaml`; recursive or over-cap payloads still block; `mart_pipeline_run_manifest.perf_summary_json` now uses `compact_perf_summary_payload()` and the largest live row is ~260,408 bytes |
| Data health | `WARN`: 0 red / 0 yellow / 342 total; `scripts/chunkyctl doctor --fast` now includes `data_health_snapshot.py`, and `warning/monitor_only` assets are capped to yellow before verdict aggregation | No blocking red or yellow buckets remain; keep the `writer_prompt` / `owner` / `sync_step` hints for future regressions, and treat any new yellow as bucketed maintenance rather than blocker. Feature panel lane was refreshed successfully, removing `fact_feature_panel` / `mart_feature_panel_validation` / `mart_feature_panel_prune_run` from red; `capital_behavior` lane then cleared `dim_capital_behavior_latest`, holder/shareholder-plan lane also cleared red after a parse-raw-only + backfill + mart rebuild, and GPCW / raw_aif10 are now green / on-demand governance rather than blockers; `doctor` now surfaces `blocking_yellow_tables` first so `quality_gate_level=blocking` yellow assets get explicit next-action priority. The `mart_p0b_lambdamart_v6_predictions` ensemble v7 context writer was refreshed to 2026-06-01, so blocking yellow is currently 0; `raw_executive_trade` and `fact_executive_trade_event` were freshly rebuilt by the local writer, and `sync_surveys` refreshed `raw_institution_surveys` / `mart_stock_survey_activity`, so the system health is now green after `fact_institution_event` was recovered by the safe rebuild path and `mart_architecture_cleanup_plan` was reclassified as on-demand governance; the fresh 2026-06-01 official `cron_daily.py --full-sync` also validated the new `sync_raw` 10-count / 30-second progress cadence all the way through 31/31 phases, and the `fact_top10_holder_period` legacy `idx_fact_hp_*` indexes were dropped from the current DB so the earlier `F10 extra parse failed` DuckDB invalidation path is no longer part of the present startup state. `backend/services/source_watermarks.py` now uses timezone-aware UTC timestamps, so `record_source_failure` / `resolve_source_failures` / `derive_watermark` / `upsert_watermark` no longer emit `datetime.utcnow()` warnings during the probe/watermark tests |
| CodeGraph | Synced after the data-health / source-watermarks / test slices; rerun `codegraph status .` after any further `.py` edits or docs-only sync | Keep the index aligned before the next controller handoff |
...
| Data freshness/PIT | **WARN/MIXED but non-blocking**: end-to-end freshness PASS (trading-date aligned); data completeness PASS with WARN (0 FAIL / 2 WARN, both sparse-event evidence); survivorship gate PASS on current label_version, legacy v2 only via explicit flag；`rank_and_size()` 已改为 PIT-tier-first，但当前推荐仍全是 `cross_stage_fallback`，因为 2026-05-29 的 PIT exact 候选多数卡在 `hp/n_signals/Wilson` 门槛，coverage 0 是候选稀疏/阈值问题；2026-06-01 的 `audit_portfolio_sizer_profile_attrition.py` 再次显示 353 个 raw candidates 中短/中/长档 selected_rows 仅 5/1/2，且全是 `cross_stage_fallback`，fail reasons 主要集中在 `hp` 与 `wilson`；最新 `fail_reasons_by_match_tier` 进一步把 exact-tier attrition 拆开：`stage_pit` 主要卡 `hp/n_signals/Wilson`，`stage_pit_formula_fallback` 主要卡 `hp/n_signals`，`cross_stage_fallback` 主要卡 `hp/wilson`；新补的 `fail_holding_days_by_match_tier` 再把 hp 失败拆到 holding_days：`stage_pit` 失败集中在 20/30/60/90 这些 off-anchor 档位，`stage_pit_formula_fallback` 失败集中在 20/30/60/90，`cross_stage_fallback` 则分布在 5/10/15/20/30/60/90 全部档位；2026-06-01 的 sensitivity audit 继续验证了这一点：`base` / `hold+20` / `min_n_signals-2` / `min_wilson_win-0.05` 对 selected_rows 没有影响，短/中/长仍然是 5/1/2；2026-06-02 先把 `reversal_1m_deep` 阈值从 15-30% 放宽到 10-30%，该 formula 的历史重算把 `rows/keys/ready coverage` 提升到 `76,635 / 11,968 / 42.54%`，随后又把 `reversal_1m_mild` 阈值从 5-15% 放宽到 4-15%，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `372,661 / 9,265 / 62.43%`，并把 `reversal_1w` 从 5-10% 放宽到 2-10% 后实跑到 `369,822 / 15,937 / 66.18%`；同日又将 `dynamic_ma_iterative_cross` 默认迭代轮数从 10 轮压到 2 轮，历史重算后该 formula 的 `rows / keys / ready coverage` 提升到 `225,783 / 20,076 / 51.63%`；全局审计也更新为 `raw_signal_rows=1,733,104 / filtered_signal_rows=878,179 / unique_keys=128,437 / ready_keys=65,426 / ready coverage=50.94% / below_min_signals=63,011`；`build_formula_signals_history.py --recompute-horizon-evidence` 现在也已在补 `defaultdict` import 后恢复可跑，不再在 horizon evidence 重算阶段抛 NameError；2026-06-02 的 `min_signals` probe 进一步显示 `min_signals=4` / `3` 会把全局 ready coverage 分别抬到 `58.99%` / `68.71%`，对应 `ready_keys=75,764` / `88,254`、`below_min_signals=52,848` / `40,267`，同时 `reversal_1m_deep` ready coverage 提升到 `50.97%` / `62.34%`，但 controller 仍建议继续看 upstream candidate supply；targeted backfill only moved latest cutoff from 3 rows to 4；`mart_daily_position_recommendation_pit_diagnostic` 现在带 `governance_reject_count` / latest reason / latest rejected_at，方便直接看每个 `stock_missing_pit` 的治理根因；2026-06-01 还把 `need_027` 的 blocked need summary 升级成 source registration evidence：preferred `akshare` 已注册，但 declared fallback 标签 `miaoxiang` 归到 `aif10` 家族，而当前 `aif10` adapter 仍未实现 `individual_fund_flow`，所以 fallback 仍是概念路径；`need_027` 主力资金源仍 blocked/unknown；`akshare.stock_individual_fund_flow` / `stock_individual_fund_flow_rank` capability 已登记，但现场 `probe_source_capability.py` 现在已先清代理再重试，但 Eastmoney 端点仍以 `ConnectionError` / `JSONDecodeError` remote disconnect 失败，blocked probe now persists into `mart_data_source_failure_queue`；if persistence hits DB lock/schema failure, the report downgrades it to structured `persisted.status=error` instead of a traceback. 2026-06-01 还新增 stage-opt candidate supply audit：在当前审计 slice（2023-01-01→2026-05-29，limit-stocks 50）上看到 `raw_signal_rows=1,381,657` / `filtered_signal_rows=733,083` / `unique_keys=120,273` / `ready_keys=57,986` / `48.21% ready coverage` / `62,287 below_min_signals` / `dropped_index_rows=1,355` / `dropped_unknown_stage_rows=647,219`，且 `codes_without_bars=0`；这次还修正了脚本结果里 `raw_signal_rows` 被 summary shadow 的报告 bug，所以 raw / filtered 现在分开显示；新增 blocked-reason breakdown 显示所有 blocked keys 都卡在 `below_min_signals`，其中 `macd_golden_cross` 和 stage 3/4 最弱；脚本现在还会自带 `next_action_recommendation`，直接把下一步指向 `P1 / upstream_candidate_supply`，并把 `macd_golden_cross` 与最弱 stage bins 作为优先 triage 目标；脚本默认 end 复用当前连接里的交易日历真相源，不再 nested `latest_closed_or_raise()` 新连接；LHB 侧最新只读核实显示 `raw_lhb_daily` 与 `fact_lhb_event` 都已到 `2026-05-29`，最新日 raw 94 rows / 84 codes、fact 84 rows / 84 codes，因此 LHB 是 source-sparse 事实，不是 ETL 落后；`audit_pit_coverage.py` 仍是 4/4 PASS，`fact_lhb_event` gain_20d coverage 83.9% > 60%，所以 PIT 安全性没问题；`mart_stock_fund_flow_rank_snapshot_daily` 已实跑落表到 `2026-06-01`（5,188 rows / 5,188 codes），对应 `mart_data_source_failure_queue` 也从 open 收敛为 resolved，说明研究侧排行快照已恢复到位，但它仍只是辅助观测，不等同 `need_027` exact flow；`need_027` blocked summary 现在进一步携带 `mart_data_source_failure_queue` 的 open / resolved 快照，能直接看到 `order_flow_fund_flow` 的 `individual_fund_flow` / `individual_fund_flow_rank` 现场失败证据，而不是只有抽象 blocked/unknown；2026-06-02 把 `reversal_1w` 阈值从 5-10% 放宽到 2-10% 后，历史重算把该 formula 的 `rows / keys / ready coverage` 提升到 `369,822 / 15,937 / 66.18%`，但 controller 仍把下一步指向 `P1 / upstream_candidate_supply`，说明阈值确实是杠杆但当前仍需回到上游供给设计；新增的 `blocked_reason_counts_by_formula_id` / `blocked_reason_counts_by_formula_variant` / `blocked_reason_counts_by_stage_bin` 进一步显示所有 blocked keys 都卡在 `below_min_signals`，其中 `macd_golden_cross`、stage 3/4 是当前最弱分布；脚本现在还会自带 `next_action_recommendation`，直接把下一步指向 `P1 / upstream_candidate_supply`，并把 `macd_golden_cross` 与最弱 stage bins 作为优先 triage 目标；`2024-03-06` 起的 `dropped_unknown_stage_rows` 降到 454,158，说明 2025 H2 的硬断档已修，剩余 `technical_stage='?'` 更偏结构性分类/预热缺口而不是新鲜 ETL outage；脚本默认 end 现在复用当前连接里的交易日历真相源，不再 nested `latest_closed_or_raise()` 新连接。 `probe_source_capability.py` 默认压掉 `data_sources.registry` 的 fallback warning，只输出结构化 blocked JSON；`--show-registry-warnings` 可恢复原始 fallback log。 | 这进一步说明下一调参决策应转向 upstream candidate supply / formula coverage，而不是继续微调 profile knobs；`macd_golden_cross` 的进一步扩展受 `fact_technical_trigger` PRIMARY KEY `(stock_code, date, formula_id)` 约束，若要增加多状态行，先走 schema evolution，不是单改 formula state |

## Latest Slice

2026-06-01 also repaired `data_health_snapshot.py` writer timestamp normalization and reran the official `cron_daily.py` with the local API up. The run completed later phases and surfaced the real prompts instead of crashing: `sync_raw` exceeded its 60s budget, while health snapshot returned `0 red / 0 yellow / 342 total` after the feature panel lane refresh, the `capital_behavior` lane cleared `dim_capital_behavior_latest`, the holder/shareholder-plan lane was rebuilt back out of red, and `raw_executive_trade` / `fact_executive_trade_event` were freshly rebuilt by the local writer; GPCW and raw_aif10 are now green / on-demand governance, so the remaining issue is tooling/complexity/storage debt rather than freshness rescue. `doctor` now promotes `blocking_yellow_tables` ahead of generic yellow maintenance so the controller sees blocking-quality yellow assets first, and the `mart_p0b_lambdamart_v6_predictions` ensemble v7 context writer was refreshed to 2026-06-01 so blocking yellow is now 0. The latest smoke then restored `fact_institution_event` through the safe rebuild path, and `mart_architecture_cleanup_plan` was reclassified to on-demand governance, so the remaining issue is tooling/complexity/storage debt, not freshness. `sync_raw` progress snapshots now feed `run_context.step_progress` / `/update/status`, and the raw ingest cadence now refreshes on a 10-count / 30-second threshold so future盘后全量不会只停留在“下载十大股东”这种静态标签上. The current follow-up cleanup removed the legacy `fact_top10_holder_period.idx_fact_hp_*` indexes from the live DB, which means the earlier `F10 extra parse failed` warning should be treated as a resolved index-debt issue rather than a general Python failure.
...
`need_027` exact-flow blocked status. 2026-06-02 另外新增 `mart_macd_state_history` / `build_macd_state_history.py` 作为 MACD active-state 诊断 mart，先用 180 天 warm-up 再切写入窗；`audit_stage_opt_candidate_supply.py --formula macd_golden_cross` 现在会把 `raw_trigger_rows=161,279` 与 `raw_state_history_rows=65,543` 一并计入，MACD 相关 coverage 抬到 `26.71%`（`8,330` ready keys），且 `scripts/chunkyctl doctor --fast` 现在也会把这些 `raw_trigger_rows` / `raw_state_history_rows` 一并带出来，让 controller 直接看到 MACD state mart 的组成证据，但 controller 仍指向 `P1 / upstream_candidate_supply`，说明 state-history 只是把 evidence 做实，不改变结构性供给不足。 2026-06-01 还把 registry-side `lhb_daily` 对齐到与 `services.lhb_client` 一致的 date-bounded helper，所以 resolve/probe 不再走旧的 aif10 全历史假象；LHB 仍是 source-sparse completeness evidence，不是 exact-flow 生产证据。This slice also wired `data_health_snapshot.py` into `scripts/chunkyctl doctor --fast`, so startup health now fails closed on 0 red / 0 yellow / 342 total data-health tables before any business work can claim freshness; 2026-06-01 的官方 `cron_daily.py` 也已跑完全部阶段，`sync_raw` 超过 60s budget 失败，但 health / drift / audit 都把当前问题显式吐出来了，并且 red/yellow rows now carry owner/sync_step prompts for self-triage; feature panel lane was refreshed successfully and removed from red, holder/shareholder-plan lane also cleared from red after the parse-raw-only + backfill + mart rebuild fix, and `raw_executive_trade` / `fact_executive_trade_event` were freshly rebuilt by the local writer. The current data-health is green; `fact_institution_event` has already been recovered by the safe rebuild path, so GPCW / raw_aif10 are no longer blockers, and `mart_architecture_cleanup_plan` is now on-demand governance.
...
5. `scripts/chunkyctl doctor --fast` now also surfaces the stage-opt `next_action_recommendation` and the `need_027` blocked-gap triage, so upstream candidate-supply triage and main-force source blockers are visible without rerunning the audits manually; when `macd_golden_cross` appears in the weakest cohort, the controller also sees the `fact_technical_trigger` primary-key schema limit, so a schema change is not mistaken for a state-only formula tweak. `need_027` 现在已确认 aif10 家族没有 exact `individual_fund_flow` capability，因此后续动作不是反复试 aif10，而是只有在 registry 新增能力或 route 映射时才重开 exact-flow 恢复线；probe persistence failures now downgrade to structured `persisted.status=error` instead of surfacing a traceback.
