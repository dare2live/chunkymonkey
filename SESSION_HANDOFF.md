...
2026-06-01 also repaired `data_health_snapshot.py` writer timestamp normalization and reran the official `cron_daily.py` with the local API up. The run completed later phases and surfaced the real prompts instead of crashing: `sync_raw` exceeded its 60s budget, while health ended at `0 red / 15 yellow / 341 total` after the feature panel lane refresh, the `capital_behavior` lane cleared `dim_capital_behavior_latest`, the holder/shareholder-plan lane was rebuilt back out of red, and `raw_executive_trade` / `fact_executive_trade_event` were freshly rebuilt by the local writer; GPCW and raw_aif10 are now yellow maintenance only, so the remaining work is lane triage rather than blocker rescue. `doctor` now promotes `blocking_yellow_tables` ahead of generic yellow maintenance so the controller sees blocking-quality yellow assets first, and the `mart_p0b_lambdamart_v6_predictions` ensemble v7 context writer was refreshed to 2026-06-01 so blocking yellow is now 0. The later `sync_surveys` refresh dropped health to 13 yellow, and a follow-up full-history `build_akshare_panel.py --tasks jgdy,dzjy` run refreshed `fact_jgdy_event` but was interrupted before `dzjy`, so current health sits at 12 yellow and the lane needs to be split by task/window instead of being run as one monolith.
...
`need_027` exact-flow blocked status. 2026-06-01 还把 registry-side `lhb_daily` 对齐到与 `services.lhb_client` 一致的 date-bounded helper，所以 resolve/probe 不再走旧的 aif10 全历史假象；LHB 仍是 source-sparse completeness evidence，不是 exact-flow 生产证据。This slice also wired `data_health_snapshot.py` into `scripts/chunkyctl doctor --fast`, so startup health now fails closed on 0 red / 15 yellow / 341 total data-health tables before any business work can claim freshness; 2026-06-01 的官方 `cron_daily.py` 也已跑完全部阶段，`sync_raw` 超过 60s budget 失败，但 health / drift / audit 都把当前问题显式吐出来了，并且 red/yellow rows now carry owner/sync_step prompts for self-triage; feature panel lane was refreshed successfully and removed from red, holder/shareholder-plan lane also cleared from red after the parse-raw-only + backfill + mart rebuild fix, and `raw_executive_trade` / `fact_executive_trade_event` were freshly rebuilt by the local writer. GPCW / raw_aif10 now stay in the yellow maintenance bucket.
