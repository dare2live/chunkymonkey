...
| Item | State | Action |
|---|---|---|
| Worktree | 当前 worktree 仍有 8 个 tracked mods（holder/shareholder-plan slice）；这是有意保留到这次状态更新之后的 dirty 收束窗口 | If dirty, run `scripts/chunkyctl worktree --format markdown` and resolve by bucket; never use `git add .` |
...
2026-06-01 also repaired `data_health_snapshot.py` writer timestamp normalization and reran the official `cron_daily.py` with the local API up. The run completed later phases and surfaced the real prompts instead of crashing: `sync_raw` exceeded its 60s budget, while health ended at `0 red / 18 yellow / 341 total` after the feature panel lane refresh, the `capital_behavior` lane cleared `dim_capital_behavior_latest`, and the holder/shareholder-plan lane was rebuilt back out of red; GPCW and raw_aif10 are now yellow maintenance only, so the remaining work is lane triage rather than blocker rescue. `doctor` now promotes `blocking_yellow_tables` ahead of generic yellow maintenance so the controller sees blocking-quality yellow assets first.
