...
Snapshot: `2026-06-01 14:53:21 CST`
...
| Data health | `FAIL`: 10 red / 20 yellow / 341 total; `scripts/chunkyctl doctor --fast` now includes `data_health_snapshot.py`, and `warning/monitor_only` assets are capped to yellow before verdict aggregation | Review the 10 blocking red buckets bucket-by-bucket before trusting freshness claims; missing tables and stale writers only block when their quality gate is blocking |
...
2026-06-01 also repaired `data_health_snapshot.py` writer timestamp normalization and reran the official `cron_daily.py` with the local API up. The run completed later phases and surfaced the real prompts instead of crashing: `sync_raw` exceeded its 60s budget, while health ended at `10 red / 20 yellow / 341 total`.
...
`need_027` exact-flow blocked status. 2026-06-01 还把 registry-side `lhb_daily` 对齐到与 `services.lhb_client` 一致的 date-bounded helper，所以 resolve/probe 不再走旧的 aif10 全历史假象；LHB 仍是 source-sparse completeness evidence，不是 exact-flow 生产证据。This slice also wired `data_health_snapshot.py` into `scripts/chunkyctl doctor --fast`, so startup health now fails closed on 10 red / 20 yellow / 341 total data-health tables before any business work can claim freshness; 2026-06-01 的官方 `cron_daily.py` 也已跑完全部阶段，`sync_raw` 超过 60s budget 失败，但 health / drift / audit 都把当前问题显式吐出来了。
