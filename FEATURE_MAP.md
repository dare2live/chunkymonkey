...
| `history` | 项目历史检索。`--grep <词>` 查逐刀细节（`git log`，可加 `--full` 出正文）； |
| `status` | L2 运行时状态**单一现查入口** (零文件): accepted 前沿 + 距最近已完成交易日的 |
| `gates` | 门分布策略 (backend/config/governance_gates.yaml): 无参打印三组分布表; |
| `scaffold-fix` |  |
...
| daily | tdxhub | daily | raw_tushare_daily | by_trade_date | 1 |
...
| fuyao_auction_benchmark | fuyao | auction-short-term-benchmark | raw_fuyao_auction_benchmark | by_trade_date | 1 |
| fuyao_limit_break_pool | fuyao | limit-break-pool | raw_fuyao_limit_break_pool | by_trade_date | 1 |
| fuyao_limit_down_pool | fuyao | limit-down-pool | raw_fuyao_limit_down_pool | by_trade_date | 1 |
| fuyao_limit_up_pool | fuyao | limit-up-pool | raw_fuyao_limit_up_pool | by_trade_date | 1 |
...
| stock_basic | fuyao | meta-tickers-list | raw_tushare_stock_basic | full_refresh | 30 |
| stock_st | stock_st_derive | stock_st | raw_tushare_stock_st | by_trade_date | 1 |
...
| top_inst | miaoxiang | top_inst | raw_tushare_top_inst | by_trade_date | 2 |
| top_list | miaoxiang | top_list | raw_tushare_top_list | by_trade_date | 2 |
| trade_cal | calendar_rule | query_trade_dates | raw_tushare_trade_cal | full_refresh | 30 |

## 3. 产表 writer (单 writer 契约审查素材)

统计: 表 33 张 | 单 writer 22 | 多 writer 11 | 动态表名写点 81 处 (29 文件)
...
| backend/scripts/db_split_org_holding.py | 2 |
...
| backend/services/data_sources/fuyao_kline_recon.py | 2 |
...
| backend/services/data_sources/tdxhub_kline_recon.py | 2 |
| backend/services/dc_member_publish.py | 2 |
| backend/services/index_daily_publish.py | 2 |
| backend/services/market_pulse.py | 4 |
| backend/services/org_holding_aif10.py | 2 |
| backend/services/org_holding_population.py | 2 |
...
> Codegraph: 节点 13,337 | calls 边 16,170 | imports 边 4,090 (每次 codegraph sync 波动, 不参与漂移判定)
...
| backend/services/database_manifest.py | 15 |
| backend/services/data_sources/sibling_repos.py | 12 |
...
| 文件 | 行数 |
|---|---|
| backend/services/data_sources/sync_runner.py | 4657 |
| backend/services/market_pulse.py | 1597 |
| backend/scripts/check_continuity_integrity.py | 1527 |
| backend/services/org_holding_aif10.py | 1519 |
| backend/scripts/check_contract_stamp_consistency.py | 1355 |
| backend/services/data_sources/sources/tdxhub.py | 1094 |
...
- chunkyctl 子命令 12 | launchd 任务 0 | router 8 (端点 44)
