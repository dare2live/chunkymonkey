...
3. 进入 P3 ThemeLifecycle MVP：先做主题/板块生命周期，不接入 ranker，只输出 `mart_market_perception_theme_daily` 或同级 context 表。
4. 情绪周期后续增强只补可靠源：连板/晋级/炸板/昨日涨停溢价在找到 PIT 数据源前继续保持 NULL + `unknown_metrics`。
