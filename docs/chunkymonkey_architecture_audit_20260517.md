...
| Data ingestion | `backend/scripts/build_price_kline_tdxhub.py`, `gcp/fetch_kline_via_vm.sh` | tdxhub K 线构建、本地/GCP 拉取 | tdxhub server pool stale 时 watermark 没主动 alert; `DATA_INTEGRITY_AUDIT_20260517.md:19` 记录 91% codes 缺失 | watermark freshness contract + alert sink, 超 SLA 阻断 paper/live | P0 |
...
| Ops | `ops/*`, `backend/scripts/launchd/*`, `configs/launchd/*` | launchd/cron, nightly audit | nightly audit 有报告但无主动告警; `DATA_INTEGRITY_AUDIT_20260517.md:71` 已指出 | `nightly_data_audit` 写 `mart_alert_event`, 本地 notification/Slack/email 任选一个 sink | P0 |
