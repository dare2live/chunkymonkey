...
> **状态: 研究档, 部分偏离 / deprecated (2026-06-20)** — 本文成于 2026-05-27, 引用的
> `run_position_monitor.py` 等脚本已删/未建, 应用层 (策略集成) 随 reset 退役。**cyq 现状**:
> 实测与 tushare qfq 同复权坐标可用 (本地 2023+, 待回填 2018), 见 `../goal.md` P1 数据底座研究行。
> 本文件留作 **CYQ 算法研究参考** (算法/数据源评估部分仍有效), 勿当现行实现命令源。
