...
Phase 1 inspected whether pandas, legacy SQL compatibility, old route labels, old DB paths, stale health results, and support-repo contracts could create false positives in ChunkyMonkey.
...
- `miaoxiang` no longer exposes the retired DDL dialect; the public DDL generator now allows only `duckdb` and `postgres`.
- ChunkyMonkey no longer uses old DB fixture file names.
- Tests that checked retired catalog or table-info syntax were moved to DuckDB-native `information_schema` or `DESCRIBE`.
- Runtime locked transaction calls were replaced with DuckDB `BEGIN TRANSACTION`.
- Several runtime `_table_columns` helpers now use `DESCRIBE`.
...
- `miaoxiang`: `a1501e3 Drop retired DDL dialect`
...
- `duck_adapter.py` still contained legacy SQL polyfills at the end of Phase 1. Phase 2 removes them.
...
pandas_test: 89
legacy_sql_docs: 3
legacy_sql_runtime: 8
legacy_sql_test: 11
```

The remaining legacy SQL runtime hits were in `backend/services/duck_adapter.py`.
...
- miaoxiang retired SQL residual scan: no matches

## Next Phase Queue

- Remove `duck_adapter.py` legacy SQL polyfills after remaining call sites and tests no longer require them.
