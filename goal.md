...
> **产品决策辅助 backlog（defs；E=DONE）**：`analysis/product_decision_assist_backlog_20260721.md` — A 资金流；B 形态/阶段选股；C tabs；**D 交集最强 FIXED 子集** — `analysis/capability_d_intersection_strongest_20260721.md`；**E step cards FIXED** — `analysis/capability_e_pipeline_step_cards_20260721.md`；**F 股票档案**
...
**近端 focus**：product schedule authority = `analysis/product_plan_reeval_stock_dossier_20260721.md`。Foundation **0r.1–0r.3 FIXED** + **0r.4 PARTIAL**（ths_hot 发布窗 residual；证据 `foundation_bj_dualpath_ashare_whitelist_20260721.md` / `6afea30fc`）。**0r.5 holders lineage audit = PARTIAL DONE**（canonical parse 218,444 PASS；stock↔holders↔form ~98%；机构档案 ~54%；ops「76/987k」= rewrite amp，净新增 ~30码/380行）— 证据 `holders_stock_dossier_lineage_audit_20260721.md`，**勿重审**。Cap E **DONE**（禁 revert）。**0r.5b FIXED**（WM/SLA + split ops + 机构 honesty；`387eb79b5`）。**2F deepen FIXED 子集**（episode cycle/returns + C-light tabs；`50817db0f`）。**3A+3C FIXED 子集**（decision moneyflow board/stock + market/dossier tabs；证据 `capability_a_moneyflow_assist_20260721.md`）。**4D FIXED 子集**（DC 行业∩概念交集最强股 decision list + `#/market` 交集最强 tab + dossier 交集 tab；fail-closed on stale/chain-as-of-mismatch；证据 `capability_d_intersection_strongest_20260721.md` + 决策 `decision_4d_intersection_strongest_20260721.md`；sw_industry 交集为已记录 residual，非静默丢弃）。**下一刀 = 5B 形态/阶段选股面**。仍禁 Type-B enrichment / S7 假 COMPAT / 擅自 E/F remeasure / G/H/Release / org invent。

**Deferred product（顺序已重评）**：0r.5b → E done → **F 股票档案** → A/C → **D done** → B（0r.1–0r.3 已 FIXED；0r.4 ths_hot residual 不挡）。Defs 见 backlog；排期见 `product_plan_reeval_stock_dossier_20260721.md`。禁 Optuna/Release/松 holdout；结论=Tier3/产品面，不融进 Tier0。
