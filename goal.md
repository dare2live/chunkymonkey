...
| main 工作区 | **长期 dirty 已清零 / 当前按短周期切片收束**: 2026-06-01 已按 bucket -> gate -> explicit stage -> `SAFE_COMMIT_NO_PUSH=1 scripts/safe_commit.sh` 分阶段收束；本轮 drift-safe complexity 切片限定为 2 个 `.py` 文件 + 状态文档，scoped audit 已 PASS，提交后工作区应回到 0 dirty。 | dirty 工作区问题已从长期阻塞变成受控切片流程；后续仍禁止 `git add .`，每个新改动继续按小切片、test-tool gate、CodeGraph+complexity、safe_commit 收束。 |
...
| Complexity HIGH | backend API HIGH 已清；已治理 gate scripts 无 HIGH；本轮 test-tool/realdb 隔离变更未新增 touched-file HIGH。2026-06-01 已把 `build_architecture_inventory.py`、BestChoice context/feed 脚本、`build_candidate_feature_panel.py` DDL fallback、`build_dim_listing_status.py` schema migration false-positive、`build_drift_safe_feature_candidates.py` fold-stable 重复评分/序列化 loop 切片清掉；新增 listing-status 与 drift-safe greedy-equivalence 回归测试。`scripts/chunkyctl doctor --fast` 仍 FAIL，最新 `new_high_count=9`: `build_daily_position_recommendations.py` 2、`build_ensemble_v7_phase7_context.py` 2、`build_executive_trade_events.py` 4、`build_feature_association_duck.py` 1；storage 仍为 WARN。 | 下一优先级继续 complexity diff 专项：先按真实数据量/调用频率审计 remaining 9 个 new HIGH，区分真 N+1/O(n²) 与 bounded false positive，再按小切片修或记录 reviewed exception；不把 worktree clean 当成 doctor clean。 |
