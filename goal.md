...
**Period-domain incremental（owner Q3 + hard lock；2026-07-23 纠偏；2026-07-24 bounded fill）**：每次 `daily_update` / 显式 sync 对 org（及同类 period 域）**必须** check latest plannable vs local raw+accepted；**raw 缺 → 拉一期；raw 有 accepted 无 → accept from local-raw；都有 → skip + next-period unlock log**；**plannable 完整且中间季有洞 → 每 run 填最老缺季 N=1**（`fill_older_period` via `sync_period(..., allow_existing_refresh=False)` + `plan_partition_catchup` oldest_first）。**NEVER** 每次点击全市场单期 ~830k mass re-pull / unbounded page crawl refresh / by-date invent / pipeline `backfill()`。实现：`org_holding_period_gap_report` + `org_holding_period_catchup` + `sync_org_holding_incremental`；表面：`delta_manifest.acquire_summary.incremental` + due_plan period 行；mass ban 不变。
