...
> - 不变量4 单概念单真相源: SERVE单一读路 + 删源大体DONE。**[2026-06-26 实测纠正] 消费者绕过SERVE = 0 已闭环** (roster补全 lineage元数据infra + lhb/org_holding_aif10/qfii 采集client, moth `serve-consumer-bypass-zero` 棘轮真绿; **build_segment/signal_panel 是 `build_` 加工成员非违规** —— 此前"绕SERVE直算特征=leakage洞"框定**不准**, bypass-scan 测它们=成员0违规)。**真·剩余 Gap1 = build_ 成员 PIT 锚正确性审计** (build_segment_panel 用 `duckdb.connect` 直读 market canonical 非经 data_loaders/SERVE → asof 手搓是否正确? = 真金白银 leakage 实质, 比"绕过点"精确)。M5血缘=不变量4自描述副产品(§1.5.3), T2已DONE。
