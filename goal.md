...
| **P1** | **主升浪猎手 (episode-first 结果倒推)** | **owner=`analysis/zhushenglang_hunter_plan_20260617.md` + `data_validation_backtest_plan_20260619.md`**; 5阶段 #46-50 (A0地基→B可买入率→C分层切阶段→**D因子矩阵 Optuna+Modal**→E转正门)。**F0地基 + B tradability + C分层切阶段 = done+对抗验证 (2026-06-19~20, 8 commit, 详 ledger 2026-06-19~20 条)**: 地基全连通 (fact_feature_panel 8.17M PIT / 正9070+hard-neg35198 入场点100%可查因子 / 鱼头鱼尾 fact_rally_stage 切分 panel-join100%); tradability GREEN (可买入率99.9%, 主升浪慢平滑非连板); Modal 端到端就绪 | **→ D 因子矩阵 (#49)**: **step1 done (2026-06-20, owner=analysis/rally_factor_discrimination_d1_20260620.md)**: reversal_20(AUC0.642)+vol_20(0.639) 入场判别稳定(walk-forward OOS 0.68/0.70 不衰, cap内一致), mom/mf/roe 噪音; 鱼尾 signature=高mom+高vol+极负reversal(出场雏形)。**C-R1 警示**: AUC 必要非充分, reversal=Stage1.5含成本-34.6%同族, **事件驱动低换手可能避R1墙但必含成本验证**。**D-step-2 done = 买点 reversal/vol 不可交易 (2026-06-20, owner=analysis/rally_entry_backtest_d2_20260620.md)**: 天真入场(pivot当日)净+5.8%@120d/90%胜=**纯泄漏**(pivot=±20日未来窗确认的底, 底后必涨); 现实入场(pivot+21确认后, 实时可观测)**全线负 -2~-6%含成本**, top20%仅breakeven。单因子 reversal/vol(0.64)来自 pivot look-ahead 非 alpha。**D-step-3 单因子全测尽弱(量价/资金/板块 AUC0.46-0.61)**, 我曾据此判"买点死"=**defeatist 错(用户两次纠偏对)**。**→ D-step-3 翻案 BREAKTHROUGH (2026-06-20, 用户选多因子model)**: **HistGBDT 12因子 walk-forward(train≤2022/test≥2023) OOS AUC=0.738**(train0.755差0.017不过拟合, 大幅超单因子0.61; 全PIT+按年切分无泄漏)—— GBDT 抓交互把弱因子组合成真判别力。**仍 C-R1 必要非充分**(0.738>失败的0.64, model-top分位可能真盈利但需含成本裁决)。**→ D-step-4a done = C-R1 含成本现实入场裁决: edge 真实但远不及 KPI (2026-06-20, owner=同档 + experiment_store run d4a)**: realizable 晚入场(bottom+21确认后)per-trade top10%含成本中位+5.92%/胜率62.4%@120d 单调超净负宇宙(-5.62%); 忠实fixed-hold组合 model_decile年化+6.4%/maxdd-28.5%/sharpe0.43 **beats random +6.6pp** = 判别力真转realizable含成本组合价值(非IC幻觉)。**但necessary-not-sufficient**: 6.4%≪KPI30%/-28.5%≪-20%。两新反例: (i)**churn磨没edge**(持top-k+周期重排把未持满horizon赢家提前卖→model≈random; 事件信号须忠实fixed-hold); (ii)**趋势regime gate证伪**(index>MA60只市场已涨ON占13%日, 抄底入场在底部OFF→杀光入场年化→-2.3%; 抄底⟂趋势过滤, dd须contrarian/止损/仓位)。-28.5%dd主市场beta(random也-39%,小盘熊)。**→ D-step-4b 杠杆(用户优先早入场)**: ①早入场(重建PIT trailing-low候选无未来窗+重训, B+5.9%→逼近A上界+16%,最大return杠杆) ②dd控制(止损/vol-target仓位/contrarian超卖,非趋势) ③加板块热度+cyq特征 ④鱼尾出场。Optuna/Modal须早入场证有edge后再花(grill)。(旧 D-step-3 重定向出场=入场可做后的次优, 已被 GBDT 翻案)。残: 入场若续=量价异常放大+板块热度+筹码扩因子 + 现实入场点重做。**方法论校正**: episode回测入场必用实时可确认点, 禁用pivot/peak等±窗事后点(本次教训)。(e)余/GT统一/winsorize/daily_basic2019(task_022abb42) 留 D |
