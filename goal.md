...
> 3. **[DONE] 通达信 6/7 xdxr/server** (修 4 红 K线测试 + 物删 2 表)。剩完成度收尾: akshare M4 / 通达信客户端 dead 代码整段物删(financial_client sync body + tdx_source server_health DB函数 + build_fundamental, 均标RETIRED dead不僵尸) / mart-lineage独立bug(需strategy域context)。
