...
| 系统级数据健康审计 | **FAIL: 32 red / 4 yellow / 341 total**: `scripts/chunkyctl doctor --fast` 现在把 `data_health_snapshot.py --dry-run --format json` 作为系统门禁的一部分；`raw_margin_daily` 缺表，31 个红表是 stale writer，4 个黄表是要么轻度过期要么待分级的边缘资产 | 这是 fail-closed 的系统级健康信号，不是 cosmetic warning；必须按 bucket 逐个复核红表、定位 first bad writer 或明确 evidence gap，不能继续沿用“局部补修就算通过”的旧口径 |
