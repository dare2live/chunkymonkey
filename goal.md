...
| 数据完整性实跑 | **FAIL: 2 issues** | 2026-06-01 已把 `price_kline_tdxhub`、`fact_alpha158_panel`、`fact_stock_technical_stage`、`fact_signal_context`、`fact_technical_trigger`、`mart_stock_picture_daily`、`mart_stock_survey_features`、`mart_p0a_label_panel`、`mart_p0a_feature_label_panel_v3`、`mart_p0a_feature_label_panel_v4`、`mart_sniper_score_daily`、`mart_institution_score_daily` 补到交易日历 `2026-05-29`；`fact_lhb_event` 仍停 `2026-05-25`，因为 `raw_lhb_daily` 也只到 `2026-05-25`，属于源头卡住；`fact_technical_trigger` 仍是事件表子集，仅 `1,692` codes 被记 `PARTIAL_WARN`，不能直接等同缺数。 |
