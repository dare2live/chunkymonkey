...
| stage-opt candidate supply audit | **PASS scoped / upstream supply evidence**: 新增 `backend/scripts/audit_stage_opt_candidate_supply.py` + `backend/tests/scripts/test_audit_stage_opt_candidate_supply.py`；2026-06-01 先后回填 `fact_stock_technical_stage` / `fact_signal_context` 的 2025-08-01→2026-05-29 断档，再跑 full-history 审计时已看到 `606,583 raw_signal_rows / 111,835 unique_keys / 49,586 ready_keys / 44.34% ready coverage / 62,249 below_min_signals`，且 `codes_without_bars=0`，说明当前瓶颈不是 K 线缺失而是 signal density；新增的 `blocked_reason_counts_by_formula_id` / `blocked_reason_counts_by_formula_variant` / `blocked_reason_counts_by_stage_bin` 进一步显示所有 blocked keys 都卡在 `below_min_signals`，其中 `macd_golden_cross`、stage 3/4 是当前最弱分布；`2024-03-06` 起的 `dropped_unknown_stage_rows` 降到 454,158，说明 2025 H2 的硬断档已修，剩余 `technical_stage='?'` 更偏结构性分类/预热缺口而不是新鲜 ETL outage；脚本默认 end 现在复用当前连接里的交易日历真相源，不再 nested `latest_closed_or_raise()` 新连接。 | 这进一步说明下一调参决策应转向 upstream candidate supply / formula coverage，而不是继续微调 profile knobs |
