...
| main 工作区 | **长期 dirty 已清零 / 当前按短周期切片收束**: 2026-06-01 已按 bucket -> gate -> explicit stage -> `SAFE_COMMIT_NO_PUSH=1 scripts/safe_commit.sh` 分阶段收束；feature association complexity 切片已提交；本轮 storage payload reviewed-column 切片 scoped audit、CodeGraph、doctor 均 PASS，提交后工作区应回到 0 dirty。 | dirty 工作区问题已从长期阻塞变成受控切片流程；后续仍禁止 `git add .`，每个新改动继续按小切片、test-tool gate、CodeGraph+complexity、safe_commit 收束。 |
