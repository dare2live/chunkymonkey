...
| P0 | DB retention/modularization | User directive 2026-06-12: execute the 34G reclamation now. Two dead hash-cache tables already archived+dropped (985MB parquet rollback). Split runbook = `analysis/db_split_runbook_20260612.md` | **Execute split now** via `COPY FROM DATABASE` rebuild + per-table rowcount/sample validation + atomic rename + 14-day-old-file retention; panel-family convergence (G4) deferred to second pass — first pass moves all live tables, reclaims rebuild compaction only |
