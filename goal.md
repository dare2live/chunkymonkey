...
| **P1** | **主升浪猎手执行方案** | **owner=`analysis/zhushenglang_hunter_plan_20260617.md` + `analysis/data_validation_backtest_plan_20260619.md`** (refined plan: 数据增量/阶段因子矩阵/回测12gap/执行序)。**5阶段 loops/goals = task #46-50** (A0地基止血→B可买入率→C分层切阶段→**D因子矩阵 sandbox+Optuna+Modal 单/多因子**→E转正门; Optuna/Modal 等 A0/C 地基稳否则白跑)。**2026-06-19 A0 锁外全 done (5 commit)**: (1)因子恢复(8d2ad93e); (2)加载器移services+builder un-broke+pit_guard门(00e007c7,12单测); (3)GT标签拆 fact_rally_entry_pit(c8d6177c: entry_signal_date=bottom+base_days唯一PIT特征+fwd_complete90.5%; outcome[gain/peak/dd/**bull_aligned**拉升期测]剥离禁做X,rally_labels守门); (4)死闸防回退 moth 锁门(41845922); (5)**PIT负样本 fact_rally_entry_negative**(390c8c3a: hard-negative框架=同结构pivot-low+长底base40+fwd_complete+未涨<60%+purge±250根; 35198/4846股 pos:neg≈1:3.9; 0正负重叠/0北交所/0outcome; 共享原语services/rally_detect; ST留消费侧) | **A0 完成且对抗验证 (2026-06-20)**: fact_feature_panel 物化 live (8.17M行/5427股; pit_guard门 + 独立重算600519 1784日0泄漏 + universe0违规 + 0NaN-inf + **entry点JOIN正负样本100%覆盖** mom100%/mf83%/roe45%). 结果倒推地基全连通: 9070正+35198hard-neg 入场点均可查PIT因子, outcome物理隔离. → **B(#47) done GREEN (2026-06-20)**: 起涨点可买入率99.9%(一字板0.1%); 主升浪=慢平滑(峰距底中位9月/底→顶84%, 底后5日仅+5.3%, 入场窗81%<10%@d5)→连板买不进对平滑型不成立, tradability PASS(非alpha结论, owner=analysis/rally_buyability_gonogo_20260620.md) → **C(#48) step1 done (2026-06-20)**: fact_rally_episode_strata PIT分层(申万sector as-of99%+市值PIT83%+base桶, 9070ep小盘微盘主导, 3977630f; daily_basic2019市值缺口spawn task_022abb42) → **续 C step2: fact_rally_stage 起涨/主升/顶部切分(鱼头/鱼身/鱼尾, 用户核心缺口)** | (e)余: 消费侧leakage注册留D(有consumer时); GT builder统一调rally_detect(低优); roe/mom尾值D阶段winsorize |
