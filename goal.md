...
- **B5 brick registry stronger PARTIAL**：`brick_registry.yaml` 分类 L2 primitives + L3 FeatureBlocks（4）+ Type-B edge（2：`institution_profile_edge_v0` / `rally_gt_edge_v2`）；gate 扩 Type-B orphan（`data_layers` L2_feature→outputs）+ `status=partial` 须 typed `partial_reasons` + `type_b_edge`→`store=feature_store`；qfq 已 typed PARTIAL lineage（缺 physical batch_id/`factor_as_of` + latest-factor rewrite + read COALESCE placeholders）— **未**升 FIXED / 未标 UNTRUSTED
- **近端 focus**：B5 residual = qfq physical lineage columns（或诚实 UNTRUSTED 读面）/ E0 **org BLOCKED** 维持 local-raw；S7 仅在 owner 建新 publication 或 sunset 证据时再动 — **不**假 COMPAT / 假 FIXED；**不**开 E/F remeasure / G/H/Release
