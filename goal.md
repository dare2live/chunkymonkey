...
> - **#2 库分区**: 7库DONE; 仅 **§9 reference拆库** 完成它 (Stage A 保真建库 DONE commit d678b03d; **真 blocker = high-blast 连接模型收口** [Stage B-E: 4套并行连接模型 get_conn/ATTACH/alias-routing 收口, ≥8 处生产 break-point, 见 verified plan wxs0iyxin] → 焦点 session 待用户拍执行窗口。**注: 与 K线/worktree 无关 — §9 是 reference 库 dim 表, K线在 market 库; 旧文档误标"worktree blocked"已删**)。
...
> 3. **[DONE] 通达信 6/7 xdxr/server** (修 4 红 K线测试 + 物删 2 表)。**[DONE 2026-06-27] akshare M4 核心** (capital/event/attention/profit_forecast 12表物删 + 切消费侧[scoring quality_capital/external_attention→None + signals_v2 D5解禁门] + 退役 capital_client/build_akshare_panel/build_executive_trade_events/ingest_profit_forecast/external_attention 5 source + db_compact 3.0→0.9G + data_health 删表必删caller修[dim_data_asset reality-sync red 10→1])。**剩完成度收尾**: 通达信客户端 dead 代码整段物删(financial_client sync body + tdx_source server_health DB函数 + build_fundamental, 均标RETIRED dead不僵尸, 低风险) / akshare M1·M2·M3 单独轨(calendar/ETF/benchmark/K线 fallback) / **fact_common_major_holder_stock writer 432h stale(aif10 org_holding builder 未刷新, 预存在 freshness gap, 单独flag)** / mart-lineage独立bug(需strategy域context)。
