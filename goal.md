...
| main 工作区 | **PASS / 0 dirty entries**: 2026-06-01 已按 bucket -> gate -> explicit stage -> `SAFE_COMMIT_NO_PUSH=1 scripts/safe_commit.sh` 分阶段收束；`scripts/chunkyctl worktree --format markdown` 当前 total=0、unknown=0、CodeGraph candidate untracked=0。 | dirty 工作区问题已从长期阻塞清零；后续仍禁止 `git add .`，每个新改动继续按小切片、test-tool gate、CodeGraph+complexity、safe_commit 收束。 |
| CodeGraph 索引状态 | `codegraph status .` 当前 **up to date**；files=1,052 / nodes=16,955 / edges=359,647；pending=0。 | CodeGraph 已不再被 untracked/dirty 误导；后续 `.py` 变更仍需改前 query/context、改后 `codegraph sync .`。 |
| 工具更新 | **LOCAL WRAPPER FIRST / PROJECT ASSISTANT SEED**: `audit_tooling_gate.py` / `chunkyctl doctor` 已能区分 dirty、CodeGraph pending、complexity baseline/diff，并新增 storage payload summary；`chunkyctl worktree` 当前可归桶 dirty；`chunkyctl audit` 支持 scoped gate；`safe_commit.sh` 支持 `SAFE_COMMIT_NO_PUSH=1` 本地安全提交；`docs/chunkyctl_session_quickstart.md` 已同步 doctor storage payload 解释。`doctor --fast` 当前仍 **FAIL**，原因是 complexity diff + storage payload FAIL，而不是文档权威链或 dirty 长期污染。 | GitHub 调研后暂不引入 Worktrunk/lazygit/delta/git-absorb/git-town/git-branchless；先把 dirty/codegraph/complexity/storage 机器化，再逐步收束成 `chunkyctl` 类项目专用审计/开发辅助入口；后续任何 session 启动模式、gate、工具入口或 controller/agent 工作法变化，必须同批更新 quickstart，不允许只留在对话或 handoff。 |
...
| Complexity HIGH | backend API HIGH 已清；已治理 gate scripts 无 HIGH；本轮 test-tool/realdb 隔离变更未新增 touched-file HIGH。2026-06-01 已把 `build_architecture_inventory.py` 的 nested-loop / sort-in-loop / execute-in-loop 扫描项拆成单职责 helper，目标测试 15 passed，scoped audit PASS；同日又治理 BestChoice context exit/feed 脚本：`build_bestchoice_context_exit_policy.py` 批量加载 K 线并批量 insert policy rows，`build_bestchoice_phase2_daily_feed.py` 用 `bisect_right` helper 计算 T+1，并固化 rank-before-dedup 契约，新增 helper 单测 5 passed，scoped audit PASS。`scripts/chunkyctl doctor --fast` 仍 FAIL，但 FAIL 已转移为下一批 complexity diff：`build_candidate_feature_panel.py` 1 个 HIGH、`build_daily_position_recommendations.py` 2 个 HIGH、`build_dim_listing_status.py` 1 个 HIGH、`build_drift_safe_feature_candidates.py` 5 个 HIGH；storage 仍为 WARN。 | 下一优先级继续 complexity diff 专项：先按真实数据量/调用频率审计 candidate feature panel、daily recommendations、listing status、drift-safe feature candidates 是否是真 N+1 / O(n²)，再按小切片修；不把 worktree clean 当成 doctor clean。 |
...
| F | P0 | final clean | **DONE for long-lived dirty / storage slice gated**: 所有长期 dirty 已按小切片提交；当前 storage payload 切片完成 scoped audit、迁移 dry-run/execute 和 storage re-audit，提交后工作区应回到 0 dirty。 | `scripts/chunkyctl doctor --fast` 不再因长期 dirty/unknown/pending FAIL；storage payload 已从 FAIL 降为 WARN。当前剩余 P0 FAIL 是 `build_architecture_inventory.py` 8 个 complexity diff HIGH，需作为下一独立切片处理。 |

当前裁定: dirty 问题已完成一轮系统化清零；后续继续使用 bucket -> gate -> explicit stage -> safe_commit 的收束流水线。`build_architecture_inventory.py` complexity diff 已完成一个小切片；下一步不是继续找长期 dirty，而是处理 `doctor --fast` 剩余实质 FAIL: BestChoice context exit/feed 两个脚本的 complexity diff。storage payload 已从 FAIL 降为 WARN，继续按优先级分批治理。
...
| 6.6 | P0/P1 | 工具门禁 JSON 化 | 新增 `backend/scripts/audit_tooling_gate.py`：解析 git status、CodeGraph status、complexity markdown，输出 baseline/diff；无 baseline 时输出 `baseline_unavailable` + `unclassified_high_count`，加载 baseline 后才输出有效 `new_high_count`；`chunkyctl doctor` 默认加载本地忽略文件 `data/reports/tooling/complexity_baseline.json` 并新增 storage payload summary；2026-06-01 doctor 显示 baseline loaded / current_count=40；`build_architecture_inventory.py` diff 已清，BestChoice context exit/feed diff 也已清；下一窗口 `new_high_count=9` 集中在 candidate feature panel、daily recommendations、listing status、drift-safe feature candidates；storage payload 迁移后为 320 columns / 0 FAIL / 11 WARN；`chunkyctl preflight` 改为 token 匹配 task risk，避免 `build` 误触发 `ui` frontend gate；新增 `docs/engineering_governance.md` 记录 GitHub 工具调研 | dirty worktree 不再只靠人工读长列表；历史 HIGH、新增 HIGH、大 payload/递归 JSON 风险可分开治理；当前 P0/P1 后续是判断下一窗口 4 个脚本 HIGH 是真实 N+1/嵌套问题还是可接受线性累积，再继续分批治理 storage WARN |
