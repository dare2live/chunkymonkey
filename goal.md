...
当前裁定: dirty 问题已完成一轮系统化清零；后续继续使用 bucket -> gate -> explicit stage -> safe_commit 的收束流水线。下一步不是继续找长期 dirty，而是处理 `doctor --fast` 剩余实质 FAIL: `build_architecture_inventory.py` complexity diff；storage payload 已从 FAIL 降为 WARN，继续按优先级分批治理。
