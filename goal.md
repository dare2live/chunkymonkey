...
> - 不变量2 读写边界=写锁边界=库分区: **[2026-06-27 §9 reference 拆库 DONE — 四地基最后一块完成]** 4 dim (active/trading_calendar/all_ever_listed/listing_status) 已物删迁 reference.duckdb 独立库; 全 reader 经 resolver.dim_read_conn/active_codes/active_stock_name_map/latest_completed_trade_date 路由 reference; writer reference-only; smartmoney schema-init DDL 全移除防重建。**2进程锁解耦实测 PASS** (writer 持 smartmoney RW 期间 reader 读 reference dim 不阻塞, 对照 smartmoney RO 被拒证锁真在)。**不变量2 完成 = 四地基(主键PIT/读写边界/分层/单一真相源)全部 DONE**。owner=analysis/section9_phase0_fanin_checklist_20260627.md。
...
> **下一步 (2026-06-27 §9 DONE 更新)**: **四地基全部 DONE** (不变量1 主键PIT / 不变量2 §9 库分区 [本次完成] / 不变量3 分层 / 不变量4 删源+leakage=0)。**数据底座扎实 → 转 edge 确认 (档B前置, 真金白银钱路)**。**档B(因子全量/cube/展示产品化)BLOCK 直到 sandbox 证含成本可交易 stage-conditional edge** (confirmed_by_owner=0)。下一主线 = sandbox 探索 stage-conditional edge (主升浪猎手 D1 GT 已建, 逐数据 alpha 验证待跑)。
>
> **[2026-06-26 architect re-anchor 调整计划 (用户"结合现状推进四地基")]**: 四地基整体扎实度对账 ——
> - **#1 主键PIT / #3 分层 = DONE** (无剩余)。
> - **#2 库分区**: 7库DONE; 仅 **§9 reference拆库** 完成它 (Stage A 保真建库 DONE commit d678b03d; **真 blocker = high-blast 连接模型收口**)。**[2026-06-27 Phase 0 fan-in 审计 DONE — gating 前置就绪]** workflow wf_df52c6c6 (5agent) 全清 4 连接模型 73 点(get_conn46/direct7/bestchoice10/writers10) + 对抗验证抓 7 漏点(routers/scripts/tests, services 之外), **裁决 stage_e_safe=FALSE**。**关键发现: 机制被 fan-in 现实 reframe** — 根因 services/db.py get_conn 无 reference ATTACH, alias-routing(架构师选)需 73 点逐个 rewire; 验证者荐中央 get_conn ATTACH 根治(但=曾否决的 view+ATTACH) → **机制本身有真 tradeoff 须焦点 session re-grill**。owner=analysis/section9_phase0_fanin_checklist_20260627.md。**[2026-06-27 机制 grill 实测裁决 + 设计缺口]**: DuckDB 1.5.2 实测 — 裸 FROM 不能跨库解析(中央ATTACH无效) + view 磁盘污染坐实 → **alias-routing/限定名是唯一干净路, 73点改写无捷径**(view+ATTACH 实测否决)。**但发现真设计缺口**: data_access SERVE entity 模型强制 asof_col+code_col(PIT+code-centric), 而 4 dim 是 current 快照(非PIT)+ dim_trading_calendar 无code → **阻抗失配, dim-读机制未决**(a扩data_access current-dim类型 / b专用reference-dim服务 / c限定名+ATTACH, 影响SERVE不变量) + connect_rw写侧归属未定(resolver只读铁律) + 读写耦合须逐dim整迁。**§9执行 = fresh 焦点 session: 先定 dim-读机制设计 → 逐 dim 迁 → Stage E物删** (whole-app high-blast: universe/calendar/scoring/dossier; 本session已18+commit不鲁莽起)。**注: 与 K线/worktree 无关; 旧"worktree blocked"已删**。
