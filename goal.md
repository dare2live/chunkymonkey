...
| stage-opt candidate supply audit | **PASS scoped / upstream supply evidence**: 新增 `backend/scripts/audit_stage_opt_candidate_supply.py` + `backend/tests/scripts/test_audit_stage_opt_candidate_supply.py`，在当前审计 slice（2023-01-01→2026-05-29，limit-stocks 50）上看到 3576 raw_signal_rows / 831 unique_keys / 280 ready_keys / 33.69% ready coverage / 551 below_min_signals；脚本默认 end 现在复用当前连接里的交易日历真相源，不再 nested `latest_closed_or_raise()` 新连接。 | 这进一步说明下一调参决策应转向 upstream candidate supply / formula coverage，而不是继续微调 profile knobs |
