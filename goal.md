...
- **A（当前）** A1 calendar factory attestation **PARTIAL**（`verify_calendar_generation_contract`+land/accept 门；`DatasetExecutionContract` 传播墙仍在）；A2 calendar accepted live；A3 名义K+ST+resolver；A4 landing 纯度；A5 adapter 三界。出口：对抗测绿且 `live_readiness` 可评估；禁抓数/切消费者。
