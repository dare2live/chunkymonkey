...
- 2026-06-02 `build_daily_position_recommendations.py` 已把 MACD `mart_macd_state_history` 纳入候选池，并把 cross-stage fallback 回接到现有的 `mart_per_stock_strategy_optimal`；live 2026-06-01 运行现在不会再因错表崩溃，当前快照虽然返回 0 candidates，但这是 live DB 无匹配行，不是 loader 失败；该修复已在 commit `1402bc0b` 落地，当前 worktree 仍有 1 个已修改文件待提交。
