...
| 端到端审计实跑 | **FAIL: 24 total / 18 OK / 2 WARN / 4 FAIL** | 2026-06-01 03:40 CST 复跑仍未过：`fact_technical_trigger` 停 2026-05-19、`fact_signal_context` 停 2026-05-19、`mart_stock_picture_daily` 停 2026-05-26、`mart_stock_survey_features` 停 2026-05-20；推荐 PIT coverage 仍为 0。K 线上游已补到交易日历，但下游 freshness/PIT 还不能当生产证据。 |
...
| 数据完整性实跑 | **FAIL: K 线/alpha158 OK / 下游 7 issues** | 2026-06-01 已用 tdxhub 增量把 `price_kline_tdxhub` 补到交易日历 `2026-05-29`，随后用安全窗口补齐 `fact_alpha158_panel` 到 `2026-05-29`。最新审计: `price_kline_tdxhub max=2026-05-29 / n_codes=5,188 / OK`；`fact_alpha158_panel max=2026-05-29 / n_codes=5,183 / OK`，alpha158 05-26..05-29 窗口 0 duplicate。剩余 stale: `mart_p0a_label_panel`/`mart_p0a_feature_label_panel_v3/v4`/`mart_sniper_score_daily`/`mart_institution_score_daily` 2026-05-19，`fact_technical_trigger` 2026-05-19，`fact_lhb_event` 2026-05-25。`fact_capital_flow_pit_daily`、`fact_risk_factors`、`fact_sector_momentum_daily` 仍停 2026-05-26，需随下游链路评估。 |
...
当前裁定: dirty 问题已完成一轮系统化清零；后续继续使用 bucket -> gate -> explicit stage -> safe_commit 的收束流水线。`build_architecture_inventory.py`、BestChoice context/feed、candidate feature panel DDL fallback、listing status schema migration、drift-safe fold-stable scoring/serialization、Phase7 context bulk K-line、executive trade events bulk/index、feature association fold/correlation cleanup、storage payload reviewed-column、公式信号增量刷新安全阀、daily recommendation DDL 防误删、tooling diff identity 和 alpha158 安全窗口写入切片均已完成 scoped gates；K 线与 alpha158 freshness 已补到 2026-05-29。下一步不是继续找长期 dirty，也不是追被 baseline/行号漂移误报的 complexity new HIGH，而是处理真实业务风险: `fact_signal_context` / `fact_technical_trigger` 计算起点与替换窗口耦合、stage 上游 stale、端到端下游 freshness/PIT gate FAIL、Survivorship/Data completeness 实跑 FAIL。storage payload 当前为 PASS / 11 reviewed columns，不再是剩余阻塞。
...
| 6.11 | P0 | 数据 freshness/PIT 修复流水线 | `price_kline_tdxhub` 与 `fact_alpha158_panel` 已补到交易日历 `2026-05-29`；`build_alpha158_duck.py` 已改为先建临时结果、事务内按窗口替换，空窗口 fail-closed。下一刀按安全边界排序: stage-only freshness 入口 -> `fact_signal_context` lookback-safe 计算起点/替换窗口分离 -> `fact_technical_trigger` CLI compute/replace 分离 -> picture 单日刷新 -> label/v3/v4/score marts；`fact_lhb_event` 与 `mart_stock_survey_features` 先补 start/end/lookback/drop 安全阀 | 写库前 test-tool gate；改 `.py` 前 CodeGraph context/query，改后 CodeGraph sync + complexity；`build_signal_context.py` / `build_formula_signals_history.py` 当前不能直接小窗口跑，因为 `--start` 同时控制计算和 DELETE；生产刷新后跑 `audit_data_completeness.py`、`audit_end_to_end.py`、`audit_universe_coverage.py`、`audit_pit_integrity.py`、`audit_survivorship_gate.py`，label version 与当前训练面板对齐 |
