...
| P1 | Stage-opt supply | Structural upstream blocker remains; readiness threshold is now config-owned and diagnostic source-load errors are reportable | Harden source/schema join evidence and upstream formula density; do not tune knobs as a substitute |
...
| `audit_stage_opt_candidate_supply.py` | `WARN` | Short live audit `2026-06-01..2026-06-05`: `min_signals=5` from config, `source_load_error_count=0`, `12155` keys blocked by `below_min_signals`; blocker remains upstream supply |
