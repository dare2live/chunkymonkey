...
| **P1** | **主升浪猎手执行方案** | **owner=`analysis/zhushenglang_hunter_plan_20260617.md` + `analysis/data_validation_backtest_plan_20260619.md`** (refined plan: 数据增量/阶段因子矩阵/回测12gap/执行序)。**5阶段 loops/goals = task #46-50** (A0地基止血→B可买入率→C分层切阶段→**D因子矩阵 sandbox+Optuna+Modal 单/多因子**→E转正门; Optuna/Modal 等 A0/C 地基稳否则白跑)。**2026-06-19 A0 大半 done**: (1)因子恢复(8d2ad93e); (2)加载器移services+builder un-broke+pit_guard门(00e007c7, 12单测); (3)GT标签拆 fact_rally_entry_pit(c8d6177c: entry_signal_date=bottom + base_days唯一PIT特征 + fwd_complete 90.5%; outcome[gain/peak/dd/**bull_aligned**拉升期测]剥离禁做X, rally_labels守门); (4)死闸防回退 moth 锁两门(41845922) | **A0 余 (锁门, 续拉PID19115 持 tushare_raw 锁释放后跑)**: build_feature_panel RUN 物化panel+验universe-clean / PIT负样本(框架决定=**hard-negative**: 同结构pivot-low长底但未涨,holding PIT-setup隔离rally信号; ST-PIT排除需raw) / audit_panel_leakage 注册panel消费者 |
