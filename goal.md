...
| stage-opt candidate supply audit | **PASS scoped / upstream supply evidence**: 新增 `backend/scripts/audit_stage_opt_candidate_supply.py` + `backend/tests/scripts/test_audit_stage_opt_candidate_supply.py`；2026-06-01 先后回填 `fact_stock_technical_stage` / `fact_signal_context` 的 2025-08-01→2026-05-29 断档，再跑 full-history 审计时已看到 4929 raw_signal_rows / 1033 unique_keys / 398 ready_keys / 38.53% ready coverage / 635 below_min_signals，`2024-03-06` 起的 `dropped_unknown_stage_rows` 降到 454,158，说明 2025 H2 的硬断档已修，剩余 `technical_stage='?'` 更偏结构性分类/预热缺口而不是新鲜 ETL outage；脚本默认 end 现在复用当前连接里的交易日历真相源，不再 nested `latest_closed_or_raise()` 新连接。 | 这进一步说明下一调参决策应转向 upstream candidate supply / formula coverage，而不是继续微调 profile knobs |
