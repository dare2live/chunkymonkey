...
| 端到端审计实跑 | **FAIL: 24 total / 18 OK / 4 WARN / 2 FAIL** | 2026-06-01 04:07 CST 复跑仍未过：`mart_stock_picture_daily` 停 2026-05-26、`mart_stock_survey_features` 停 2026-05-20；`fact_signal_context` 与 `fact_technical_trigger` 已到 2026-05-29 但仍按 days_behind=3 计 WARN；推荐 PIT coverage 仍为 0，不能当生产证据。 |
...
| 数据完整性实跑 | **FAIL: K线/alpha158/stage/context/trigger OK-ish / 下游 7 issues** | 2026-06-01 已把 `price_kline_tdxhub`、`fact_alpha158_panel`、`fact_stock_technical_stage`、`fact_signal_context`、`fact_technical_trigger` 补到交易日历 `2026-05-29`；trigger 05-20..05-29 窗口 17,950 signals，context 05-20..05-29 窗口 41,149 rows，stage 窗口 25,894 rows。最新 completeness 仍 FAIL: label/v3/v4/sniper/institution 停 2026-05-19，`fact_lhb_event` 停 2026-05-25，trigger 仅事件型覆盖 1,692 codes 被记 `PARTIAL_WARN`。`fact_capital_flow_pit_daily`、`fact_risk_factors`、`fact_sector_momentum_daily` 仍停 2026-05-26，需随下游链路评估。 |
...
当前裁定: dirty 问题已完成一轮系统化清零；后续继续使用 bucket -> gate -> explicit stage -> safe_commit 的收束流水线。`build_architecture_inventory.py`、BestChoice context/feed、candidate feature panel DDL fallback、listing status schema migration、drift-safe fold-stable scoring/serialization、Phase7 context bulk K-line、executive trade events bulk/index、feature association fold/correlation cleanup、storage payload reviewed-column、公式信号增量刷新安全阀、daily recommendation DDL 防误删、tooling diff identity、alpha158 安全窗口写入，以及 stage/context/technical-trigger 安全窗口刷新切片均已完成 scoped gates；K 线、alpha158、stage、signal_context、technical_trigger freshness 已补到 2026-05-29。下一步不是继续找长期 dirty，也不是追被 baseline/行号漂移误报的 complexity new HIGH，而是处理真实业务风险: `mart_stock_picture_daily` / `mart_stock_survey_features` freshness FAIL、label/v3/v4/score marts stale、LHB stale、Survivorship/Data completeness 实跑 FAIL。storage payload 当前为 PASS / 11 reviewed columns，不再是剩余阻塞。
...
| 6.11 | P0 | 数据 freshness/PIT 修复流水线 | `price_kline_tdxhub`、`fact_alpha158_panel`、`fact_stock_technical_stage`、`fact_signal_context`、`fact_technical_trigger` 已补到交易日历 `2026-05-29`；`build_alpha158_duck.py`、`build_stage_formula_fitness.py`、`build_signal_context.py`、`build_formula_signals_history.py` 都已支持计算起点与替换窗口分离，空窗口/不安全窗口 fail-closed 或显式 scoped replace。下一刀按安全边界排序: `mart_stock_picture_daily` 单日/窗口刷新 -> `mart_stock_survey_features` start/end/drop 安全阀 -> label/v3/v4/score marts；`fact_lhb_event` 仍需补 start/end/lookback/drop 安全阀 | 写库前 test-tool gate；改 `.py` 前 CodeGraph context/query，改后 CodeGraph sync + complexity；生产刷新后跑 `audit_data_completeness.py`、`audit_end_to_end.py`、`audit_universe_coverage.py`、`audit_pit_integrity.py`、`audit_survivorship_gate.py`，label version 与当前训练面板对齐；trigger 是事件表，`PARTIAL_WARN` 不能直接等同缺数 |
