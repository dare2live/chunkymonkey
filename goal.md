...
| 库内 artifact/cache 污染审计 | **WARN / 未发现 LifeHack 自嵌套，单行大 payload FAIL 已迁移**: `audit_storage_payloads.py` 已改为只扫 payload-like 列，并用 JSON key 形态识别递归，避免把普通 `stock_code/date/built_at` 或命令字符串误判为循环引用；`chunkyctl doctor --fast` 现在默认包含 storage payload summary。2026-06-01 已将 `mart_today_signal_cache.signals_json` 20,220,095 bytes / 9,286 signals 从主表整包 JSON 迁入 `mart_today_signal_cache_signal` 明细表，主表只留 summary/cache metadata + `signals_json='[]'` 兼容字段。最新真实库审计: 320 columns scanned / 0 FAIL / 11 WARN / recursive hits 0。 | 当前 WARN 中 `mart_today_signal_cache_signal.signal_json` 单条最大 2,254 bytes、总量约 20.2MB，属于有界明细缓存而非递归/整包 blob；raw F10/GPCW、`fact_technical_trigger.reason_codes_json` 与 lineage/path markers 仍需后续按 source/lineage 价值分批治理。禁止直接清表/VACUUM 造成 API 空响应；后续 storage WARN 先看单条 max、消费者和可重算性。 |
...
| main 工作区 | **长期 dirty 已清零 / 当前按短周期切片收束**: 2026-06-01 已按 bucket -> gate -> explicit stage -> `SAFE_COMMIT_NO_PUSH=1 scripts/safe_commit.sh` 分阶段收束；本轮 executive trade events complexity 切片限定为 1 个脚本 + 1 个测试 + 状态文档，scoped audit 已 PASS，提交后工作区应回到 0 dirty。 | dirty 工作区问题已从长期阻塞变成受控切片流程；后续仍禁止 `git add .`，每个新改动继续按小切片、test-tool gate、CodeGraph+complexity、safe_commit 收束。 |
| CodeGraph 索引状态 | 本轮 `.py` 变更后已 `codegraph sync .`；新增 indexable 测试文件必须随同 scope 显式 stage/commit，不能用强制 sync 掩盖 pending。 | CodeGraph 已不再被长期 untracked/dirty 误导；后续 `.py` 变更仍需改前 query/context、改后 `codegraph sync .`，最终交付前 `codegraph status .` 应回到 up to date。 |
| 工具更新 | **LOCAL WRAPPER FIRST / PROJECT ASSISTANT SEED**: `audit_tooling_gate.py` / `chunkyctl doctor` 已能区分 dirty、CodeGraph pending、complexity baseline/diff，并新增 storage payload summary；`chunkyctl worktree` 当前可归桶 dirty；`chunkyctl audit` 支持 scoped gate；`safe_commit.sh` 支持 `SAFE_COMMIT_NO_PUSH=1` 本地安全提交；`docs/chunkyctl_session_quickstart.md` 已同步 doctor storage payload 解释。`doctor --fast` 当前仍 **FAIL**，原因是 remaining complexity diff + storage payload WARN，而不是文档权威链或长期 dirty 污染。 | GitHub 调研后暂不引入 Worktrunk/lazygit/delta/git-absorb/git-town/git-branchless；先把 dirty/codegraph/complexity/storage 机器化，再逐步收束成 `chunkyctl` 类项目专用审计/开发辅助入口；后续任何 session 启动模式、gate、工具入口或 controller/agent 工作法变化，必须同批更新 quickstart，不允许只留在对话或 handoff。 |
...
| Complexity HIGH | backend API HIGH 已清；已治理 gate scripts 无 HIGH。2026-06-01 已把 `build_architecture_inventory.py`、BestChoice context/feed 脚本、`build_candidate_feature_panel.py` DDL fallback、`build_dim_listing_status.py` schema migration false-positive、`build_drift_safe_feature_candidates.py` fold-stable 重复评分/序列化 loop、`build_ensemble_v7_phase7_context.py` per-stock K-line N+1、`build_executive_trade_events.py` forward-return history rescan / chunk query loop 切片清掉；新增 Phase7 与 executive bulk/index 回归测试。`scripts/chunkyctl doctor --fast` 仍 FAIL，最新 `new_high_count=9`: `build_daily_position_recommendations.py` 2、`build_feature_association_duck.py` 7；storage 仍为 WARN。 | 下一优先级继续 complexity diff 专项：P0/P1 审计 `build_feature_association_duck.py` 7 个新 HIGH，区分真实批量化机会与 DDL/chunked insert false positive；daily recommendation 多为 cap-bound 小集合，后续可记录 reviewed exception 或做轻量 group cleanup。继续按小切片修或记录 reviewed exception；不把 worktree clean 当成 doctor clean。 |
...
| F | P0 | final clean | **DONE for long-lived dirty / current slice gated**: 所有长期 dirty 已按小切片提交；当前 executive trade events bulk/index 切片完成 scoped audit，提交后工作区应回到 0 dirty。 | `scripts/chunkyctl doctor --fast` 不再因长期 dirty/unknown/pending FAIL；当前剩余 P0/P1 FAIL 是 complexity diff 9 个 new HIGH + storage payload 11 WARN，其中 `build_feature_association_duck.py` 是下一重点审计对象。 |

当前裁定: dirty 问题已完成一轮系统化清零；后续继续使用 bucket -> gate -> explicit stage -> safe_commit 的收束流水线。`build_architecture_inventory.py`、BestChoice context/feed、candidate feature panel DDL fallback、listing status schema migration、drift-safe fold-stable scoring/serialization、Phase7 context bulk K-line、executive trade events bulk/index 切片已完成 scoped gates；下一步不是继续找长期 dirty，而是处理 `doctor --fast` 剩余实质 FAIL: 9 个 remaining complexity diff HIGH + storage payload 11 WARN。storage payload 已从 FAIL 降为 WARN，继续按优先级分批治理。
...
| 6.6 | P0/P1 | 工具门禁 JSON 化 | 新增 `backend/scripts/audit_tooling_gate.py`：解析 git status、CodeGraph status、complexity markdown，输出 baseline/diff；无 baseline 时输出 `baseline_unavailable` + `unclassified_high_count`，加载 baseline 后才输出有效 `new_high_count`；`chunkyctl doctor` 默认加载本地忽略文件 `data/reports/tooling/complexity_baseline.json` 并新增 storage payload summary；2026-06-01 doctor 显示 baseline loaded / current_count=40；`build_architecture_inventory.py`、BestChoice context/feed、candidate feature panel、listing status、drift-safe feature candidates、Phase7 context、executive trade events diff 已清；最新 `new_high_count=9` 剩余在 daily recommendations、feature association；storage payload 迁移后为 320 columns / 0 FAIL / 11 WARN；`chunkyctl preflight` 改为 token 匹配 task risk，避免 `build` 误触发 `ui` frontend gate；新增 `docs/engineering_governance.md` 记录 GitHub 工具调研 | dirty worktree 不再只靠人工读长列表；历史 HIGH、新增 HIGH、大 payload/递归 JSON 风险可分开治理；当前 P0/P1 后续是按真实热路径审计 remaining 9 个 new HIGH，再决定实修或记录 reviewed exception，继续分批治理 storage WARN |
...
| 6.9 | P0/P1 | artifact/cache storage 治理 | 已按 LifeHack 24.7GB 事故模型做本项目只读排查并落证据到 `analysis/lifehack_storage_bloat_analog_audit_20260531.md`；`audit_storage_payloads.py` 已固化为可重复门禁并接入 `chunkyctl doctor`。2026-06-01 完成 today signal cache 迁移：`mart_today_signal_cache` 主表只存 summary/cache metadata，新增 `mart_today_signal_cache_signal` 按 `cache_key + signal_rank` 存有界 `signal_json`；`backend/scripts/migrate_today_signal_cache_payload.py` 支持默认 dry-run、`--execute` 才写库。真实库迁移结果: 1 row / 9,286 signals / payload 20,220,095 -> 2 bytes；storage audit 0 FAIL / 11 WARN。 | `/api/signals/today` cache hit/miss 与 `signals_v2` 今日缓存回归 42 passed；后续继续治理 raw F10/GPCW、`fact_technical_trigger.reason_codes_json`、lineage/path marker WARN。禁止直接清表/VACUUM 造成 API 空响应；storage WARN 是否需要动作按单条 max、总量、可重算性和消费者价值排序。 |
