...
| **P1** | **主升浪猎手执行方案** | **owner=`analysis/zhushenglang_hunter_plan_20260617.md` + `analysis/data_validation_backtest_plan_20260619.md`** (refined plan: 数据增量/阶段因子矩阵/回测12gap/执行序)。**5阶段 loops/goals = task #46-50** (A0地基止血→B可买入率→C分层切阶段→**D因子矩阵 sandbox+Optuna+Modal 单/多因子**→E转正门; Optuna/Modal 等 A0/C 地基稳否则白跑)。**2026-06-19 A0-1 done**: 3 PIT stage因子(动量/资金/财务as-of)恢复进 services 消除 builder→experiment 倒挂 (commit 8d2ad93e, 5单测) | A0 续 (#46): 加载器移services + build_feature_panel 重写物化panel(PIT) + GT标签拆(entry_signal_date/fwd_complete) + PIT负样本 + 死闸 |
