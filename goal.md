...
| 库内 artifact/cache 污染审计 | **FAIL / 未发现 LifeHack 自嵌套，但大 payload gate 已固定**: `audit_storage_payloads.py` 已改为只扫 payload-like 列，并用 JSON key 形态识别递归，避免把普通 `stock_code/date/built_at` 或命令字符串误判为循环引用；`chunkyctl doctor --fast` 现在默认包含 storage payload summary。最新真实库只读实跑: 319 columns scanned / 1 FAIL / 10 WARN / recursive hits 0。 | 唯一 FAIL 是 `mart_today_signal_cache.signals_json` 单行 20,220,095 bytes / 9,286 signals；这不是循环嵌套日志污染，但必须作为 P0/P1 storage/artifact contract 迁移：DB 放 summary/index/可查询事实，大 payload 进明细表或受控 artifact。WARN 包括 raw F10/GPCW 有界原始 payload 总量大、若干 lineage/path markers；清理前必须用 CodeGraph + `rg` 定消费者并补回归。 |
...
| Complexity HIGH | backend API HIGH 已清；已治理 gate scripts 无 HIGH；本轮 test-tool/realdb 隔离变更未新增 touched-file HIGH。`scripts/chunkyctl doctor --fast` 在 clean worktree 下仍 FAIL: complexity baseline loaded, current_count=40, unchanged_count=32, resolved_count=8, **new_high_count=8**，全部集中在 `scripts/build_architecture_inventory.py` 行 538/596/598/600/603/605/623/649。 | 下一优先级从 dirty cleanup 转为 complexity diff 专项：先用 CodeGraph + 复杂度扫描审计 `build_architecture_inventory.py` 新 HIGH 是否为真实新增风险或 line drift，再修/调整 baseline；不把 worktree clean 当成 doctor clean。 |
...
| F | P0 | final clean | **DONE for dirty / gate FAIL narrowed**: 所有长期 dirty 已按小切片提交；本轮只剩当前 storage-tool 切片未提交前 dirty；CodeGraph pending=0。 | `scripts/chunkyctl doctor --fast` 不再因长期 dirty/unknown/pending FAIL；当前剩余 FAIL 是 `build_architecture_inventory.py` 8 个 new complexity HIGH + `mart_today_signal_cache.signals_json` 20MB 大 payload。 |

当前裁定: dirty 问题已完成一轮系统化清零；后续继续使用 bucket -> gate -> explicit stage -> safe_commit 的收束流水线。下一步不是继续找长期 dirty，而是处理 `doctor --fast` 的两个实质 FAIL: architecture inventory complexity diff 与 today signal cache 大 payload。
...
| 6.6 | P0/P1 | 工具门禁 JSON 化 | 新增 `backend/scripts/audit_tooling_gate.py`：解析 git status、CodeGraph status、complexity markdown，输出 baseline/diff；无 baseline 时输出 `baseline_unavailable` + `unclassified_high_count`，加载 baseline 后才输出有效 `new_high_count`；`chunkyctl doctor` 默认加载本地忽略文件 `data/reports/tooling/complexity_baseline.json` 并新增 storage payload summary；2026-06-01 doctor 显示 baseline loaded / current_count=40 / `unchanged_count=32` / `resolved_count=8` / `new_high_count=8`，storage payload 319 columns / 1 FAIL / 10 WARN；`chunkyctl preflight` 改为 token 匹配 task risk，避免 `build` 误触发 `ui` frontend gate；新增 `docs/engineering_governance.md` 记录 GitHub 工具调研 | dirty worktree 不再只靠人工读长列表；历史 HIGH、新增 HIGH、大 payload/递归 JSON 风险可分开治理；当前 P0/P1 后续是判断 `build_architecture_inventory.py` 8 个 new HIGH 是真实新增、行号漂移还是 baseline 需要重建，并迁移 `mart_today_signal_cache.signals_json` |
...
| 6.9 | P0/P1 | artifact/cache storage 治理 | 已按 LifeHack 24.7GB 事故模型做本项目只读排查并落证据到 `analysis/lifehack_storage_bloat_analog_audit_20260531.md`；本轮把 `audit_storage_payloads.py` 固化为可重复门禁并接入 `chunkyctl doctor`。最新真实库门禁未发现递归 JSON key 命中，但发现 `mart_today_signal_cache.signals_json` 20,220,095 bytes 单行 FAIL；raw F10/GPCW 与 lineage/path markers 为 WARN。 | 先用 CodeGraph/`rg` 锁定 `signals_v2`、`routers/signals.py`、Workbench 读模型消费者；将 today signal cache 从“整包 JSON 入库”迁为 summary/index + 明细表或 artifact ref；补 `/api/signals/today` cache hit/miss 与 updater `refresh_today_signals` 回归；禁止直接清表/VACUUM 造成 API 空响应。 |
