...
- **B5 brick registry PARTIAL（first knife）**：`backend/config/brick_registry.yaml` 分类 L2 primitives（qfq/form/Tier1·Tier2 publish/pulse）vs L3 FeatureBlocks（4 ids）；gate=`check_brick_registry.py`（silent raw bypass / max 2-hop / orphan FEATURE_BLOCK_ID）；doctor section `brick_registry` 报告 orphans；moth claim `brick-registry-l2-l3`；**未**升 FIXED（qfq lineage placeholder + Type-B/feature_store 深登记仍 residual）
- **近端 focus**：B5 deepen（qfq lineage / Type-B）或 E0 **org BLOCKED** 维持 local-raw；S7 仅在 owner 建新 publication 或 sunset 证据时再动 — **不**假 COMPAT / 假 FIXED；**不**开 E/F remeasure / G/H/Release
