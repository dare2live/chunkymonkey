...
- **B-pit PARTIAL** 120d shadow contract tightened（K∩ST）：
  ...
  semantic delta（ST+board 外 533 行）。**Mart cutover gate FIXED**：
  `b_pit_mart_cutover` + pulse `/sentiment` + B2 读边界；default →
  LEGACY；opt-in 需 MATCH 证 + policy hash + 显式 yaml；**未**
  `cutover_allowed=true` / **未** mart 数值切读。Artifacts=
  `data/lineage/b_pit_breadth_shadow/`。
