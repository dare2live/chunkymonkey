...
| main 工作区 | **长期 dirty 已清零 / 当前规则是每刀短周期收束**: 2026-06-01 已按 bucket -> gate -> explicit stage -> `SAFE_COMMIT_NO_PUSH=1 scripts/safe_commit.sh` 分阶段收束；后续任何 dirty 都必须归入明确 scope 并在本切片内提交或移除。 | dirty 工作区问题已从长期阻塞变成受控切片流程；后续仍禁止 `git add .`，每个新改动继续按小切片、test-tool gate、CodeGraph+complexity、safe_commit 收束。 |
...
| Complexity HIGH | backend API HIGH 已清；已治理 gate scripts 无 HIGH；本轮 test-tool/realdb 隔离变更未新增 touched-file HIGH。2026-06-01 已把 `build_architecture_inventory.py`、BestChoice context/feed 脚本、`build_candidate_feature_panel.py` DDL fallback、`build_dim_listing_status.py` schema migration false-positive 切片清掉；新增 listing-status 回归测试。`scripts/chunkyctl doctor --fast` 仍 FAIL，但下一窗口已收敛为 `build_daily_position_recommendations.py` 2 个 HIGH + `build_drift_safe_feature_candidates.py` 7 个 HIGH；storage 仍为 WARN。 | 下一优先级继续 complexity diff 专项：先按真实数据量/调用频率审计 daily recommendations 与 drift-safe feature candidates 是否是真 N+1 / O(n²)，再按小切片修；不把 worktree clean 当成 doctor clean。 |
...
当前裁定: dirty 问题已完成一轮系统化清零；后续继续使用 bucket -> gate -> explicit stage -> safe_commit 的收束流水线。`build_architecture_inventory.py`、BestChoice context/feed、candidate feature panel DDL fallback、listing status schema migration 切片已完成 scoped gates；下一步不是继续找长期 dirty，而是处理 `doctor --fast` 剩余实质 FAIL: daily recommendations / drift-safe feature candidates complexity diff，其中 subagent 审计认为多数是 bounded false positive，真正优先的真实热点是 drift-safe fold-stable scoring 重复计算。storage payload 已从 FAIL 降为 WARN，继续按优先级分批治理。
...
| 6.6 | P0/P1 | 工具门禁 JSON 化 | 新增 `backend/scripts/audit_tooling_gate.py`：解析 git status、CodeGraph status、complexity markdown，输出 baseline/diff；无 baseline 时输出 `baseline_unavailable` + `unclassified_high_count`，加载 baseline 后才输出有效 `new_high_count`；`chunkyctl doctor` 默认加载本地忽略文件 `data/reports/tooling/complexity_baseline.json` 并新增 storage payload summary；2026-06-01 doctor 显示 baseline loaded / current_count=40；`build_architecture_inventory.py`、BestChoice context/feed、candidate feature panel、listing status diff 已清；下一窗口 `new_high_count=9` 集中在 daily recommendations 与 drift-safe feature candidates，subagent 审计将多数分流为 bounded false positive / low risk，真实优先热点为 drift-safe fold-stable scoring；storage payload 迁移后为 320 columns / 0 FAIL / 11 WARN；`chunkyctl preflight` 改为 token 匹配 task risk，避免 `build` 误触发 `ui` frontend gate；新增 `docs/engineering_governance.md` 记录 GitHub 工具调研 | dirty worktree 不再只靠人工读长列表；历史 HIGH、新增 HIGH、大 payload/递归 JSON 风险可分开治理；当前 P0/P1 后续是先修 drift-safe scoring 真实重复计算，再决定是否用 allowlist/证明文档处理 bounded scanner false positives，继续分批治理 storage WARN |
