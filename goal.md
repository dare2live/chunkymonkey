...
| main 工作区 | **长期 dirty 已清零 / 当前短周期切片收束中**: 2026-06-01 已按 bucket -> gate -> explicit stage -> `SAFE_COMMIT_NO_PUSH=1 scripts/safe_commit.sh` 分阶段收束；当前剩余为本轮安全切片: daily recommendation DDL 防误删历史、complexity diff 行号漂移误报修复、quickstart/goal/handoff/index 同步。 | dirty 工作区问题已从长期阻塞变成受控切片流程；后续仍禁止 `git add .`，每个新改动继续按小切片、test-tool gate、CodeGraph+complexity、safe_commit 收束。 |
...
| 端到端审计实跑 | **FAIL: 24 total / 18 OK / 2 WARN / 4 FAIL** | 真实数据 freshness/PIT 未过：`fact_technical_trigger` 停 2026-05-19、`fact_signal_context` 停 2026-05-19、`mart_stock_picture_daily` 停 2026-05-26、`mart_stock_survey_features` 停 2026-05-20；推荐 PIT coverage 仍为 0。已先修公式信号窗口刷新和 daily recommendation DDL 防误删，下一步按 K 线 -> alpha/label/feature -> context/picture/scores 的顺序补 freshness。 |
...
| 数据完整性实跑 | **FAIL: 8 issues / 多表 stale** | `price_kline_tdxhub` 最新 2026-05-26，交易日历到 2026-05-29；`fact_alpha158_panel` 2026-05-25，`mart_p0a_label_panel`/`mart_p0a_feature_label_panel_v3/v4`/`mart_sniper_score_daily`/`mart_institution_score_daily` 2026-05-19，`fact_technical_trigger` 2026-05-19，`fact_lhb_event` 2026-05-25。只读 explorer 确认 `alpha158` 与 `fact_lhb_event` builder 仍有全表 DROP 风险，`mart_stock_survey_features` 小窗口缺 lookback 安全阀；写生产库前先补安全边界。 |
...
| 6.11 | P0 | 数据 freshness/PIT 修复流水线 | 先补 `price_kline_tdxhub` 到交易日历 `2026-05-29`；再处理 `fact_alpha158_panel` 全表重建安全边界、`fact_signal_context`/`fact_technical_trigger` 窗口刷新、picture 单日刷新、label/v3/v4/score marts；`fact_lhb_event` 与 `mart_stock_survey_features` 先补 start/end/lookback/drop 安全阀 | 写库前 test-tool gate；改 `.py` 前 CodeGraph context/query，改后 CodeGraph sync + complexity；生产刷新后跑 `audit_data_completeness.py`、`audit_end_to_end.py`、`audit_universe_coverage.py`、`audit_pit_integrity.py`、`audit_survivorship_gate.py`，label version 与当前训练面板对齐 |
