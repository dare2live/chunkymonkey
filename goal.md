...
| main 工作区 | **长期 dirty 已清零 / 当前按短周期切片收束**: 2026-06-01 已按 bucket -> gate -> explicit stage -> `SAFE_COMMIT_NO_PUSH=1 scripts/safe_commit.sh` 分阶段收束；storage payload reviewed-column 切片已提交；当前公式信号增量刷新安全阀切片 scoped audit、CodeGraph、doctor 均 PASS，提交后工作区应回到 0 dirty。 | dirty 工作区问题已从长期阻塞变成受控切片流程；后续仍禁止 `git add .`，每个新改动继续按小切片、test-tool gate、CodeGraph+complexity、safe_commit 收束。 |
...
| 工具更新 | **LOCAL WRAPPER FIRST / PROJECT ASSISTANT SEED**: `audit_tooling_gate.py` / `chunkyctl doctor` 已能区分 dirty、CodeGraph pending、complexity baseline/diff，并新增 storage payload summary；`chunkyctl worktree` 当前可归桶 dirty；`chunkyctl audit` 支持 scoped gate；`safe_commit.sh` 支持 `SAFE_COMMIT_NO_PUSH=1` 本地安全提交；`docs/chunkyctl_session_quickstart.md` 已同步 doctor storage payload 解释。2026-06-01 发现旧本地 `complexity_baseline.json` 只有前 40 条，清掉 feature association 后会把后续历史 HIGH 误报为 new；已刷新本地忽略 baseline 到 220 条，`audit_tooling_gate --max-findings 40` 为 PASS / `new_high_count=0`。 | GitHub 调研后暂不引入 Worktrunk/lazygit/delta/git-absorb/git-town/git-branchless；先把 dirty/codegraph/complexity/storage 机器化，再逐步收束成 `chunkyctl` 类项目专用审计/开发辅助入口；后续任何 session 启动模式、gate、工具入口或 controller/agent 工作法变化，必须同批更新 quickstart，不允许只留在对话或 handoff。 |
...
| Complexity HIGH | backend API HIGH 已清；已治理 gate scripts 无 HIGH。2026-06-01 已把 `build_architecture_inventory.py`、BestChoice context/feed 脚本、`build_candidate_feature_panel.py` DDL fallback、`build_dim_listing_status.py` schema migration false-positive、`build_drift_safe_feature_candidates.py` fold-stable 重复评分/序列化 loop、`build_ensemble_v7_phase7_context.py` per-stock K-line N+1、`build_executive_trade_events.py` forward-return history rescan / chunk query loop、`build_feature_association_duck.py` fold temp-table loop / correlation batch loop 切片清掉；新增 Phase7、executive、feature association 回归测试证据。公式信号增量刷新切片触碰 `build_formula_signals_history.py` 历史 HIGH 文件，但 `doctor --fast` 显示 baseline loaded / `new_high_count=0`。 | 下一优先级不再追截断误报的 complexity `new_high`，也不再把已审过的 storage payload 当 dirty；优先处理真实业务风险: 端到端数据 freshness/PIT FAIL、Survivorship/Data completeness 实跑 FAIL。若后续新增 `.py` 再触发 new HIGH，仍按 CodeGraph+complexity 小切片实修或 reviewed exception。 |
| 端到端审计实跑 | **FAIL: 24 total / 18 OK / 2 WARN / 4 FAIL** | 真实数据 freshness/PIT 未过：`fact_technical_trigger` 停 2026-05-19、`fact_signal_context` 停 2026-05-19、`mart_stock_picture_daily` 停 2026-05-26、`mart_stock_survey_features` 停 2026-05-20；推荐 PIT coverage 仍为 0。本轮先修 `build_formula_signals_history.py` 小窗口刷新会全量误删历史信号的安全 bug，避免后续 freshness 修复制造新污染。 |
...
| 数据完整性实跑 | **FAIL: 8 issues / 多表 stale** | `price_kline_tdxhub` 最新 2026-05-26，交易日历到 2026-05-29；`fact_alpha158_panel` 2026-05-25，`mart_p0a_label_panel`/`mart_p0a_feature_label_panel_v3/v4`/`mart_sniper_score_daily`/`mart_institution_score_daily` 2026-05-19，`fact_technical_trigger` 2026-05-19，`fact_lhb_event` 2026-05-25。这是数据新鲜度风险，不是架构代码 PASS。 |
...
| F | P0 | final clean | **DONE for long-lived dirty / current slice gated**: 所有长期 dirty 已按小切片提交；当前公式信号增量刷新安全阀切片完成 scoped audit、CodeGraph sync、全后端 complexity scan 和 doctor，提交后工作区应回到 0 dirty。 | `scripts/chunkyctl doctor --fast` 不再因长期 dirty/unknown/pending FAIL；本地 complexity baseline 已刷新，`new_high_count=0`，下一剩余真实治理对象是端到端数据 freshness/PIT、Survivorship 与 Data completeness。 |

当前裁定: dirty 问题已完成一轮系统化清零；后续继续使用 bucket -> gate -> explicit stage -> safe_commit 的收束流水线。`build_architecture_inventory.py`、BestChoice context/feed、candidate feature panel DDL fallback、listing status schema migration、drift-safe fold-stable scoring/serialization、Phase7 context bulk K-line、executive trade events bulk/index、feature association fold/correlation cleanup、storage payload reviewed-column、公式信号增量刷新安全阀切片均已完成 scoped gates；下一步不是继续找长期 dirty，也不是追被 baseline 截断误报的 complexity new HIGH，而是处理真实业务风险: 端到端数据 freshness/PIT gate FAIL、Survivorship/Data completeness 实跑 FAIL。storage payload 当前为 PASS / 11 reviewed columns，不再是剩余阻塞。
