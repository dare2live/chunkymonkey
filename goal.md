...
| 轨道 | 资源 | 内容 | 状态 |
|---|---|---|---|
| A 存储 | smartmoney IO | 34G 拆分 (逐表重建 19.3G + validation + 换名, 14 天保留) | 执行中 |
| B 数据 | tushare gateway (并发 2) | chain7 drain top_list 历史 → chain8 C0 → W-B 剩余接口逐域实弹注册 | chain7 跑 |
| C 实验 | 本地 CPU (判决全 SQL 秒-分钟级) | C0 口径审计 → LF V0 → LHB 退出 → T3 筹码轴 (judge 执行序) | C0 排队 |
| D 算力 | **modal ($30/月)** | 判正 combo 的扫参/walk-forward 网格/模型训练 = modal 主场 (experiment_jobs 契约, dry_run=False 须 reviewed plan); 判决实验不上 modal (本地秒级, 不为用而用) | 待首个判正 |
...
| P0 | DB retention/modularization | User directive 2026-06-12: execute the 34G reclamation now. Two dead hash-cache tables already archived+dropped (985MB parquet rollback). Split runbook = `analysis/db_split_runbook_20260612.md` | **DONE 2026-06-12**: per-table rebuild 36.1G→19.3G (compaction 16.8G), validation PASS (343 tables + 7-table row hash + view-set), atomic rename, manifest unchanged. Old file `smartmoney_v1_retired_20260612.duckdb` — **delete manually after 2026-06-26** (frees 34G more). Second pass = G4 panel convergence (~5-8G more) |
