...
| `reversal_short_term` 配置化与 deep supply lift | **PASS scoped / config-owned**: `backend/services/formula_engine/reversal_short_term.py` 已把 1m mild / 1m deep / 1w 三个 variant 的阈值外提到 `backend/config/formula_reversal_short_term.yaml`，并将 `reversal_1m_deep` 的收益率上界从 `-0.08` 放宽到 `-0.05`；`backend/tests/test_formula_engine.py` 新增 loader 回归 + 6% 深跌样例，`pytest -q backend/tests/test_formula_engine.py` 39 passed。真实历史重写后 `reversal_1m_deep` 从 `118,098 signal_rows / 14,389 keys / 51.30% coverage` 抬到 `210,293 signal_rows / 17,138 keys / 66.55% coverage`，整条 stage-opt 刷新为 `raw_signal_rows=5,401,248 / filtered_signal_rows=2,701,616 / unique_keys=150,866 / ready_keys=108,009 / ready coverage=71.59% / below_min_signals=42,857`，`min_signals=4/3/2` 现在对应 `77.57% / 83.92% / 91.09%`；`weakest_formula_ids` 也从 `reversal_1m_deep` 转成 `dynamic_ma_iterative_cross` / `turtle_breakout_55` / `reversal_1m_mild`，但 controller 仍指向 `P1 / upstream_candidate_supply`，说明这刀确实抬升了 supply，但结构性瓶颈仍然在上游候选供给。 |
