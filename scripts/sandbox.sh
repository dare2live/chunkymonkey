...
case "$cmd" in
  new)
    ...
    [ -f "$SB/$name/notes.md" ] || printf '# %s 探索笔记 (sandbox, 用完删; 不进入控制面)\n' "$name" > "$SB/$name/notes.md"
...
enable_sandbox_guard()  # 此后 read_write 打开 manifest 主库 = raise

# 读主库 (只读唯一正路):  con = read_only_main("market")
# 写探索数据 (per-exp):    scr = sandbox_scratch("$name")
# 探索结果默认随 sandbox wipe。值得保留时按 strategy_validation_contract 重写为
# versioned Tier3 package + tests + evidence，再走 Rule 10；不要从这里直写主库。
PYEOF
    fi
  ...
  wipe)
    ...
    rm -rf "${SB:?}/${name:?}"
    echo "wiped $SB/$name (含 per-exp scratch) — 主代码/文档/git 0 残留"
    ;;
  wipe-all)
