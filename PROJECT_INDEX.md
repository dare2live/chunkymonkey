...
| Factor-family inventory (RX 前) | Config `factor_family_inventory.yaml` + `check_factor_family_inventory.py`（结构门）+ `check_factor_family_gates.py`（frequency continuity 矩阵）+ 设计 `analysis/factor_family_governance_toplevel_20260724.md` |
...
| Live continuity | `PYTHONPATH=backend python backend/scripts/check_continuity_integrity.py` (`FAIL` 直接非零；daily/ST 读 `accepted_partition` formal frontier；**F1 typed gaps FIXED** — `hk_holidays`/`event_sparse`/known_empty → 预期空 PASS；应有却缺 FAIL；禁 mute/READY cosmetics；证据 `analysis/continuity_f1_typed_gaps_20260723.md`）；**2026-07-25 full audit** `analysis/foundation_full_audit_20260725.md`；**dual-plane faucet FIXED** `analysis/dual_plane_fix_plan_20260725.md`（DataAccess holders→canonical notice；fact/raw tip = compat/fill only） |
