...
| T3 institution | `institution_profile.py` + `holders_aif10.py` + `disclosure_boundaries.py` + `holders_top10_acceptance.py` + router/tests | **首个正式策略包目标**；E0：holders_top10 land→accept tracer 已有；研究仍读 fact NONCONFORMING；另两域直写 → E 仍 BLOCKED |
...
| P0 | 披露域 E0 未闭合（holders tracer 外两域仍直写；研究未切 canonical） | DatasetSnapshot 冻结 / 直写退役 / institution_follow 仍不可满足 |
