...
| T1 stock state | `technical_states/`, `segments.py`, `tier12_publish_{contract,writer,accept}.py`, `tier12_consumer_cutover.py`, `tier12_nominal_canary.py` | 多轴状态可复用；C canary `20260717` writer→accept（`accepted_*.json`/`published=true`；`accepted_at`=ingest stamp≠trade end_date）；`tier12_consumer_cutover` 显式门默认 false（resolver 唯一生产读；未接线消费者）；legacy form bridge=NOT_PUBLISHABLE；未全市场 |
| T2 market sensing | `market_pulse.py`, `tier12_publish_{contract,writer,accept}.py`, `tier12_consumer_cutover.py`, `tier12_nominal_canary.py`, API/frontend | 展示可用但 breadth/margin UNTRUSTED（B-ext）；C accept envelope 同 canary；consumer cutover 门已落地仍默认 false；禁 mart cutover 直至 B-pit |
...
| P0 | E 120d checkpointed measured reject/no-gain；C writer+PIT+live smoke 已有但未 publish-complete（`WRITTEN_UNPUBLISHED`）；enrichment 历史仍 field-level PARTIAL | 下一刀 C accepted publish 路径 / daily 下一 eligible **或** stop（非 Optuna / 非松门 / 非 mass backfill / 非 B-pit cutover / 非 StrategyRelease） |
