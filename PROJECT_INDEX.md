...
| `raw_tushare_index_member_all` (tushare_raw) **申万行业 PIT** | **7787行 (5847当前Y + 1940历史剔除N) / out_date填1940 / 同股多区间1609** | sync_runner --domain index_member_all(_hist) --backfill | 2026-06-15 **行业迁移 S1 (P0)**: 原只拉 is_new='Y' → out_date 100% NULL = latest-snapshot leakage; 加 `index_member_all_hist` 域 (is_new='N' 探针实测给 out_date) 补真 PIT 区间。下接 通达信→申万 迁移 (owner=analysis/industry_migration_tdx_to_sw_20260615.md, S2 视图/S3 repoint industry.py/S4 删STALE mart/S6 双轨/S7 退役; 06-11 ANOVA 已定 申万L2 主口径)。taxonomy 桶 13→31 历史不可比 (§4.5) |
