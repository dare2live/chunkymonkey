...
- **TDX need coverage source-registration evidence**: 2026-06-01 进一步把 source registration facts 写进 blocked summary；`need_027` 的 preferred `akshare` 在当前 registry 中已注册，但 declared fallback 标签 `miaoxiang` 不匹配 registry source name，所以 fallback 仍是概念路径。`backend/tests/scripts/test_audit_tdx_data_need_coverage.py` now 16 passed，当前 audit 输出仍是 27 coverage / 10 priority / 14 reassignment rows + 1 blocked need, 但 triage 可以直接看到注册源与声明 fallback 的差异。
