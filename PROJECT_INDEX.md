...
- **Stage-opt candidate supply/readiness contract**: `backend/config/stage_opt_candidate_supply.yaml` + `backend/services/stage_opt_candidate_supply.py` 现在拥有 stage-opt upstream supply 的 source/readiness contract：`fact_technical_trigger` 是 PIT trade-trigger source，`mart_macd_state_history` 是 MACD diagnostic state-history source，allowed stage bins、`readiness.min_signals_per_key=5` 和 research-challenger formula scope override 不再藏在 audit 脚本常量里。`audit_stage_opt_candidate_supply.py` 输出 `schema_version=1`、`candidate_supply_contract` 和 `source_load_errors`；`chunkyctl doctor` 会透传 contract summary、`source_load_errors` 和 `summary.source_load_error_count`，避免 diagnostic source 失败被当成 0 行。这个切片只固化 truth-source / eligibility / grain / consumer / readiness 边界，不跑 writer、不迁表、不调阈值，所以 stage-opt 仍是 `P1 / upstream_candidate_supply` blocker。2026-06-06 短 live audit `2026-06-01..2026-06-05` 为 `WARN`: `min_signals=5` from config, `source_load_error_count=0`, `12155` keys blocked by `below_min_signals`。
