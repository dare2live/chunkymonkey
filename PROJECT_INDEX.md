...
- **Stage-opt candidate supply contract**: `backend/config/stage_opt_candidate_supply.yaml` + `backend/services/stage_opt_candidate_supply.py` 现在拥有 stage-opt upstream supply 的 source contract：`fact_technical_trigger` 是 PIT trade-trigger source，`mart_macd_state_history` 是 MACD diagnostic state-history source，allowed stage bins 和 research-challenger formula scope override 不再藏在 audit 脚本常量里。`audit_stage_opt_candidate_supply.py` 输出 `schema_version=1` 和 `candidate_supply_contract`，`chunkyctl doctor` 会透传该 contract summary。这个切片只固化 truth-source / eligibility / grain / consumer 边界，不跑 writer、不迁表、不调 `min_signals`，所以 stage-opt 仍是 `P1 / upstream_candidate_supply` blocker。
