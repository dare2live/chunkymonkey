...
| Holders / frontier primitive | Shared `frontier_decision.decide_frontier` + **`plan_partition_catchup`**（tip-leap due=`source\accepted` P≤wm ≤40；非 tip+1）；holders=`notice_date` catchup on law + forward by_notice；`stk_holdertrade` raw→canon catchup（日常 newest_first；全史 oldest_first 至 raw_only=0，live 已清）；证据 `partition_leap_integrity_20260724.md`；legacy accept=`assign_unique_holders_row_seq`（防 DUPLICATE_GRAIN）；`stk_holdernumber`=`by_ann_date`；`by_ann_date`=`ann_reprobe`；`by_trade_date`=`atomic_skip`；org period hook 禁 by-date invent；holders 路径仍 `acquire._sync_holders_aif10`；**org trunc ops** `org_holding_period_repair_truncated.py` |
| Serve→derive closed loop | Law `analysis/serve_derive_closed_loop_law_20260723.md` + config `serve_derive_closed_loop.yaml` + `data_sources/pagination_integrity.py`（paginated land ≠ complete）；process `institution_profile` delta-gate + as_of seed；org `repair_accept_from_local_raw` / `provider_truncated`→单期 sharded repair / F6 `min_org_accepted_stocks`；证据 `org_provider_page_cap_fix_20260724.md`；… |
| Factor-family inventory (RX 前) | Config `factor_family_inventory.yaml`（语义族 + frequency gate matrix stub；**门未接线**）+ 设计 `analysis/factor_family_governance_toplevel_20260724.md`；与 `brick_registry.yaml` / `sync_registry` / legacy `data_layers.yaml` 正交；下一刀 inventory gate（脚本未建）；**禁**开 RX 前跳过 inventory+gates |
