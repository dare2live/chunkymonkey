...
| `backend/scripts/experiment_l0_baseline.py` | **L0 裸K线基准驱动** (Tier-1 RankIC): v_price_kline_qfq→PIT特征→前向收益→walk-forward OOS RankIC→experiment_store (consumer_id=L0_baseline_<formula>)。**防泄露 3 门固化内联** (门1 PIT行为/门2 切分纪律 check_split_discipline/门3 异常红线 check_metric_anomaly, 任一失败BLOCK, moth `l0-leakage-gates-wired` 反孤儿守)。默认参数=测量非寻优(不触grill); 实测 reversal OOS RankIC +0.059 最佳。寻优RUN留 task#17 |
