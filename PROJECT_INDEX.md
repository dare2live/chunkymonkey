...
- **Daily recommendation DDL 防误删 + tooling diff 行号漂移修复**: `backend/scripts/build_daily_position_recommendations.py` 的 `DDL` 不再 `DROP TABLE mart_daily_position_recommendation`，避免每日推荐刷新时清掉历史推荐日期；新增 `backend/tests/scripts/test_build_daily_position_recommendations.py` 证明 DDL 重跑仍保留旧行。`backend/scripts/audit_tooling_gate.py` 的 complexity baseline/diff 默认改为 `path_kind_message` 多重集身份，行号只做定位提示，避免删除/新增一行把历史 HIGH 误报成新 HIGH；同时保留 duplicate finding count，防止同文件同类新增热点被吞掉。验证: test-tool scoped PASS，py_compile PASS，`pytest ...test_audit_tooling_gate.py ...test_chunkyctl.py ...test_build_daily_position_recommendations.py` 30 passed，`scripts/chunkyctl audit --run --scope ...` PASS，`audit_tooling_gate.py` 单文件 complexity 无热点，CodeGraph synced。
