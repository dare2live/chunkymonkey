...
| Manual full data update | UI `#/workbench`「数据更新」→ `POST /api/v3/ops/jobs/daily_update/run`；或 `bash scripts/daily_update.sh --date YYYYMMDD`；acquire 先 formal `daily`/`stock_st` 最新 eligible 日 land_then_accept（已接受则 skip；盘前 empty→drain 解耦见 peer acquire knife / `foundation_ths_hot_ui_catchup_20260722.md`）；on_demand 不进 all-due；Cap E S1/S2 → `POST /api/v3/ops/pipeline/land-accept/run`；margin=`on_demand`+`scope_blocked`（frozen，不挡 preflight） |
