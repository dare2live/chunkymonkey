...
| P0 | E 120d checkpointed measured reject/no-gain；C full-universe accept `20260717`（4989=4989；cutover 默认 false）；D PARTIAL（`research_runtime` prereg+binding+offline loop；未 complete）；B-pit 120d shadow **120/120 MATCH** + mart cutover gate FIXED（默认 false；pulse/B2 已接线；未切读）；enrichment 历史仍 field-level PARTIAL | 下一刀 provider-ready daily accept / 显式 opt-in C/B-pit cutover（强证据；仍 false）/ D persist or real-fold bind **或** stop（非 Optuna / 非松门 / 非 mass backfill / 非擅翻 cutover / 非 StrategyRelease / 非 claim D complete） |
...
| P1 | Phase D PARTIAL — prereg+binding+offline loop; no persisted ExperimentRun artifact / real fold engine / StrategyRelease | Strategy evidence still cannot reach decision/product safely |
