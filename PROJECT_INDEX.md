...
| P0 | E 120d checkpointed measured reject/no-gain；C full-universe accept `20260717`（4989=4989；cutover 默认 false）；D scaffold PARTIAL（`research_runtime`）；B-pit 120d shadow **120/120 MATCH**（baseline=membership_restricted_proxy；unfiltered=semantic delta；cutover false）；enrichment 历史仍 field-level PARTIAL | 下一刀 provider-ready daily accept / 显式 opt-in C cutover（强证据）/ B-pit mart 切读（MATCH+独立 gate，现仍 false）/ 生产读仍默认 false **或** stop（非 Optuna / 非松门 / 非 mass backfill / 非擅翻 cutover / 非 StrategyRelease / 非 claim D complete） |
...
| P1 | Market pulse mixes taxonomy, measurements, rolling/regime, write/read；仍读错误 scope raw | B-ext FIXED；B-pit shadow MATCH 120/120（membership proxy）；mart 未切；cutover 禁 |
