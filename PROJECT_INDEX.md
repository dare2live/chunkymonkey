...
- **Main-force / risk-factor catch-up + sharpe cap relaxation**: `backend/scripts/backfill_capital_flow_pit.py --start 2026-05-27 --end 2026-05-29` 把 `fact_capital_flow_pit_daily` 补到 `2026-05-29`（13,169 rows / 5,163 stocks / 822 dates）；`backend/scripts/backfill_risk_factors_history.py --start 2026-05-27 --end 2026-05-29` 先因 `sharpe_60d` 字典 cap `[-5, 5]` 拒绝写入，随后把 `backend/config/field_dictionary.yaml` 与 `backend/scripts/backfill_risk_factors_history.py` 同步到 `[-10, 10]` 并顺利重跑（15,530 rows / 5,178 stocks / 3 dates）；`backend/scripts/backfill_sector_momentum_history.py --start 2026-05-27 --end 2026-05-29` 将 `fact_sector_momentum_daily` 补到 `2026-05-29`（39 rows / 13 sectors）。`audit_data_completeness.py` 现在只剩 2 个问题：`fact_lhb_event` 仅 84 个 code 的事件覆盖 `PARTIAL_WARN`，以及 `fact_technical_trigger` 事件子集 `PARTIAL_WARN`。这轮主力资金链路已从 stale 变成 fresh，但 LHB 仍只能当事件覆盖证据，不能上升为生产结论。

- **Picture / survey freshness catch-up and survey safe-window guard**: `backend/scripts/build_picture_daily.py` 已按单日事务补 2026-05-27/28/29，每日 5,203 stocks x 4 tables，`mart_stock_picture_daily` max 到 2026-05-29；`backend/scripts/build_survey_features.py` 新增 `--write-start`，读取 lookback 与写入窗口分离，默认阻断 0 行窗口误删，新增 `backend/tests/sentiment/test_build_survey_features_script.py` 覆盖 lookback、显式 read-start、非法窗口和 empty-window override。真实库用 `--write-start 2026-05-21 --end 2026-05-29` 读 2026-03-23 起事件并写 25,897 rows，`mart_stock_survey_features` max 到 2026-05-29。`audit_end_to_end.py` 已 PASS with WARN: 24 total / 18 OK / 6 WARN / 0 FAIL；`audit_data_completeness.py` 和 survivorship gate 仍 FAIL，不能恢复策略主线。
- **Stage / signal_context / technical_trigger safe-window freshness catch-up**: `backend/scripts/build_stage_formula_fitness.py` 新增 `--write-start` 与 `--stage-only`，只刷新 `fact_stock_technical_stage` 窗口，不误跑 `mart_stage_formula_fitness` 全量矩阵；`backend/scripts/build_signal_context.py` 新增可测试的 `build_signal_context()` 与 `--write-start`，把 120 日滚动计算预热起点和写入替换窗口拆开，空窗口默认拒删；`backend/scripts/build_formula_signals_history.py` 新增 `--write-start` 调用层过滤，计算可用宽窗口，写库只替换声明窗口，horizon evidence 默认不重算。真实库刷新: stage 25,894 rows、context 41,149 rows、trigger 17,950 signals，三表 max 均到 2026-05-29。`audit_end_to_end.py` 仍 FAIL，但 freshness FAIL 从 `fact_signal_context`/`fact_technical_trigger` 转移到 `mart_stock_picture_daily` 与 `mart_stock_survey_features`；`audit_data_completeness.py` 仍 7 issues，不能恢复策略主线。
- **Alpha158 safe-window writer + freshness catch-up**: `backend/scripts/build_alpha158_duck.py` 不再默认先 `DROP TABLE fact_alpha158_panel` 再整表重建；现在先计算临时 build table，再事务内按 `--write-start..--end` 替换目标窗口，空窗口默认 fail-closed，显式未来 `--end` 会拒绝。新增 `backend/tests/scripts/test_build_alpha158_duck.py` 覆盖窗口替换保留旧行、空窗口拒删、未来日期拒绝。真实库用 `--start 2026-03-01 --write-start 2026-05-26 --end 2026-05-29` 补齐 alpha158：`replace_window` / 20,736 rows / 5,185 codes / 4 dates；`audit_data_completeness.py` 仍 FAIL，但 `fact_alpha158_panel` 已 OK: max=2026-05-29 / n_codes=5,183 / `= cal`，刷新窗口 duplicate=0。后续 stage/context/trigger 已在同日补齐，剩余 freshness 阻塞转为 picture/survey/label/score/LHB。
- **K-line freshness catch-up completed; downstream freshness still FAIL**: 本地 `price_kline_tdxhub` 已用 tdxhub 增量补到交易日历 `2026-05-29`。首轮 `build_price_kline_tdxhub.py --skip-existing --target-date 2026-05-29 --pages 1 --workers 8 --max-inflight 32` 写入 15,281 行，92 股失败；随后以 `--workers 4 --max-inflight 16 --per-stock-retry-attempts 2` 重试 stale active，93 股成功 / 0 失败 / 275 行写入。`audit_data_completeness.py` 仍 FAIL，但 `price_kline_tdxhub` 已 OK: max=2026-05-29 / n_codes=5,188 / `= cal`；最近覆盖为 05-26=5201、05-27=5184、05-28=5184、05-29=5188，仍有 16 个历史/停牌/异常 code latest < 2026-05-29。`audit_end_to_end.py` 仍 FAIL: 24 total / 18 OK / 2 WARN / 4 FAIL，阻塞项为 `fact_technical_trigger`、`fact_signal_context`、`mart_stock_picture_daily`、`mart_stock_survey_features` freshness 和推荐 PIT coverage 0；不能据此恢复策略主线。
