...
- **stage-opt 2026-06-02 reversal_1m_deep config lift**: `backend/services/formula_engine/reversal_short_term.py` 已把 1m mild / 1m deep / 1w 三个 variant 的阈值外提到 `backend/config/formula_reversal_short_term.yaml`，并把 `reversal_1m_deep` 的收益率上界从 `-0.08` 放宽到 `-0.05`；`backend/tests/test_formula_engine.py` 新增 loader 回归 + 6% 深跌样例，`pytest -q backend/tests/test_formula_engine.py` 39 passed。历史重写后 `reversal_1m_deep` 从 `118,098 signal_rows / 14,389 keys / 51.30% coverage` 抬到 `210,293 signal_rows / 17,138 keys / 66.55% coverage`，整条 stage-opt 刷新为 `5,401,248 raw_signal_rows / 2,701,616 filtered_signal_rows / 150,866 unique_keys / 108,009 ready_keys / 71.59% ready coverage / 42,857 below_min_signals`，`min_signals=4/3/2` 现在对应 `77.57% / 83.92% / 91.09%`（`117,029 / 126,614 / 137,423` ready keys）。弱公式集合转成 `dynamic_ma_iterative_cross` / `turtle_breakout_55` / `reversal_1m_mild`，但 controller 仍是 `P1 / upstream_candidate_supply`，说明这刀继续抬供给，但结构性瓶颈没变。
