...
| T1 stock state | `technical_states/`, `segments.py`, `tier12_publish_{contract,writer,accept}.py`, `tier12_consumer_cutover.py`, `tier12_nominal_canary.py`, `tier12_project_universe.py` | 多轴状态可复用；C accept 分 `publish_scope=canary|project_universe`（后者需 membership/coverage 证）；`tier12_consumer_cutover` 默认 false；legacy form bridge=NOT_PUBLISHABLE；live full-universe accept 证据见 lineage（cutover 仍 false） |
| T2 market sensing | `market_pulse.py`, `tier12_publish_{contract,writer,accept}.py`, `tier12_consumer_cutover.py`, `tier12_nominal_canary.py`, `tier12_project_universe.py`, API/frontend | 展示可用但 breadth/margin UNTRUSTED（B-ext）；C envelope 可 project_universe scope；consumer cutover 默认 false；禁 mart cutover 直至 B-pit |
...
| P0 | E 120d checkpointed measured reject/no-gain；C accept 路径+scope 门已有（canary/project_universe）；consumer cutover 默认 false；enrichment 历史仍 field-level PARTIAL | 下一刀 live full-universe accept 证据 / daily 下一 eligible **或** stop（非 Optuna / 非松门 / 非 mass backfill / 非 B-pit cutover / 非 StrategyRelease） |
