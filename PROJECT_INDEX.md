...
|---:|---|---|
| P0 | K accepted + form/qfq/segments/pulse 至 `20260721`（扇区/DC pulse 可能滞后）；legacy raw daily 仍 `20260716`（预期）；`index_dailybasic` 短窗 min_rows 拒写；margin **1b DONE** `contract_version=3` transport=SSE+SZSE；`enabled`/`bounded_calendar_catchup`/`on_demand`；accepted scope=SSE+SZSE `external_aggregate`；v2 BSE evidence read-only；continuity honest（lag may FAIL until catchup）；rzrqye UNTRUSTED；`foundation_residual_fix_plan_20260723.md`+`margin_v3_bounded_catchup_1b_20260723.md`；禁 mass backfill / product thaw | 两融列诚实 unknown；估值水位可能 stale |
...
`execution_contract_not_propagated`。margin **1b** live land/accept = v3 SSE+SZSE bounded catchup only；v2 live-write still frozen；
