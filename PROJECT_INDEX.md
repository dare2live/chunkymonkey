...
- **Picture / survey freshness catch-up and survey safe-window guard**: `backend/scripts/build_picture_daily.py` 已按单日事务补 2026-05-27/28/29，每日 5,203 stocks x 4 tables，`mart_stock_picture_daily` max 到 2026-05-29；`backend/scripts/build_survey_features.py` 新增 `--write-start`，读取 lookback 与写入窗口分离，默认阻断 0 行窗口误删，新增 `backend/tests/sentiment/test_build_survey_features_script.py` 覆盖 lookback、显式 read-start、非法窗口和 empty-window override。真实库用 `--write-start 2026-05-21 --end 2026-05-29` 读 2026-03-23 起事件并写 25,897 rows，`mart_stock_survey_features` max 到 2026-05-29。`audit_end_to_end.py` 已 PASS with WARN: 24 total / 18 OK / 6 WARN / 0 FAIL；`audit_data_completeness.py` now PASS with WARN（0 FAIL / 2 WARN），survivorship gate PASS，不能恢复策略主线的原因只剩 recommendation PIT coverage 0 与 LHB coverage WARN。
