...
| T1 stock state | `technical_states/`, `segments.py`, `tier12_publish_{contract,writer,accept}.py`, `tier12_consumer_cutover.py`, `tier12_nominal_canary.py`, `tier12_project_universe.py` | 多轴状态可复用；C accept 分 `publish_scope=canary|project_universe`；`resolve_tier12_production_read` + B1 `load_stock_state_by_day` 接线（默认 LEGACY→`fact_stock_form_daily`）；cutover yaml 仍 false；legacy form bridge=NOT_PUBLISHABLE |
