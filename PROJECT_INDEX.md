...
- **2026-06-16: 条件化 Alpha 工程纲领立法 + sync 限流分类根因修复**: (1) 权威方案 `docs/conditional_alpha_program.md` (形态×阶段×分层 segment → 公式买卖 episode → 条件化增强 5 层; 出场/持仓周期条件在 cell 度量无全市场统一卖出信号; L5 辅助源板块概念热度/成交量/资金流向 + Alpha158 待真建; 3 道强制门防漂移; 先 G 修法再 B 建), supersede `analysis/alpha_validation_program_spec_20260614.md`。(2) 根因审计 (agent): 旧重型 family 框架被判'想象复杂度'弃用沦孤儿(H1) + 无 gate 拦绕过(H2) + Alpha158 从未动工却挂 in_progress(H3) = 方案写进文档却未执行。(3) **sync 限流分类根因修复** (sync_runner.py): `_QUOTA_WALL_MARKERS` 过宽标记'请求过多/访问频率'把瞬态'并发请求过多(上限2)请稍后重试'误判当日墙误停链 (advrecv backfill 两次 0 行停链根因) — 拆 `_HARD_WALL_MARKERS`(今日上限/明天/攻击=停链) vs `_TRANSIENT_RATELIMIT_MARKERS`(并发/请稍后/访问频率=退避重试[60,120,180]s不停链), 终败入 failure_queue 由 drain 补; 用户纠偏代理限流=每分钟级非当日封顶, 13 测试通过。(4) 注册 balancesheet_advrecv + block_trade 域 (用户提议预收/大宗因子)。(5) **G2 退役 consumer_alpha_validation 孤儿** (executor docstring + experiment_jobs family 加 DEPRECATED 横幅; 新 alpha 实验唯一入口 = services/phaseD_signal_eval, 真相源 docs/conditional_alpha_program.md §6)。(6) **主管思路收敛**: 真金白银等哲学进 mio skill (核心视角#8 等), CLAUDE.md 哲学改指针留机械红线 (单一真相源)。(7) advrecv backfill 实测受 lifehack datahub fina_indicator 任务 (PID 75000, 跨项目) 共享 tushare 账号并发争用 (上限2) 致 0 行; 跨项目边界不动它, advrecv 是 F3 末段因子不挡主线 (segment/episode/Alpha158 全本地价量)。(8) **G3 门#1 散落死落地**: `check_experiment_harness.py` (experiment_*.py 必 import phaseD_signal_eval/experiment_store 否则裸跑红) + safe_commit Step 3.7 (staged 触发 exit 4) + moth 断言 experiment-harness-no-naked-script (全扫, 实测 15/15 合规) + 5 测试红绿; check_doc_governance DOCS_MAX_FILES 10→11 (纲领 doc)。moth verdict=PASS 33/0/0。(9) **G1/B2 episode 引擎落地** `services/episode_engine.py` (公式买→卖 episode + segment 标 + 含成本/一字板剔/PIT, 6 测试) + runner `experiment_formula_episode.py` (G3 门放行实测); 首跑 MACD 金叉→死叉 273,651 episode × segment: **条件化命题实证支持** — 无条件均净 +0.78%/年化单段 +15.9%, 但 4|below(下跌+零轴下) +1.86%/+32%、1|below(底部+零轴下) +1.56%/+24.7% vs 3|above(顶部+零轴上) -0.76%/-21.9% (低位/超卖 below-zero 才有 edge, "抓最猛" above-zero 负 — 印证用户直觉)。**注: 年化为单 episode 口径非组合 NAV (不可当 KPI; 真组合回测=B 阶段下一步)**; 胜率全 <40% 高赔率低胜率 profile。(10) episode 引擎扩 **trailing 时间窗** (近 3/6/9/12/18/24/36 月按入场日, 看 edge 稳定性 mio#6) + runner 扩 **3 公式** (macd 金叉死叉/ma_base 上穿跌破MA145/turtle 通道新高; 其余 5 yaml 公式生成器随 reset wipe 待重建); 8 测试。tushare 限流并发腾出 (lifehack fina_indicator 退出), advrecv backfill#3 重拉中 (该接口 Read timeout/间歇空响应慢, 档C 低优先)。(11) **重建 tushare 文档 catalog** (用户派: /tushare 网页文档重做成易读 json): 工具 /Users/dp/Documents/M/stock/tushare/build_catalog.py 解析 331 doc HTML → 241 接口 (output_fields 非空率 99.2%), 替换 backend/config/tushare_api_catalog.json 旧残缺版 (239 接口/output_fields 全空)。实测口径核证: **cyq_perf 数据从 2018** (用户对; 我们 data_start 冻 2023 是 C0 winner_rate 复权口径 FAIL 非数据缺失), moneyflow 2010起, balancesheet 含 adv_receipts+contract_liab, daily 6000条/分钟500次。raw HTML 源 (29M) 待用户确认删除。
...
| `backend/config/experiments/formula_candidates.yaml` + `analysis/l0_bare_kline_baseline_spec_20260614.md` | **L0 裸K线基准 + 公式候选库** (用户 2026-06-14: 公式全保留为 config 备选省重建 + 裸K线寻优最佳OOS参数作基准 + **不要过拟合**): 9 公式索引 (全 ohlcv_only, 信号参数 yaml 全幸存, 评估器 macd live/其余 recoverable@639e0dfb~1), active 子集 4 (防过拟合池子小) / 其余 candidate 待解锁; L0 spec 定义=walk-forward OOS 寻优最佳参数标尺, 防过拟合第一约束 (OOS选参/DSR/pre-reg/限维度/诚实报弱, 复用幸存 optuna_config.yaml 治理; moth `optuna-require-walk-forward`/`optuna-realistic-sharpe-cap`/`l0-baseline-pool-bounded` 固化); 待重建 walk_forward OOS 引擎+治理层 (reset 删) |
