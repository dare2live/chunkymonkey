...
- **System data-health snapshot fail-closed in `doctor`**: `backend/scripts/data_health_snapshot.py` is now a first-class system gate inside `scripts/chunkyctl doctor --fast`; dry-run JSON now caps `warning` / `monitor_only` assets to yellow and returns `FAIL` on 13 red / 23 yellow / 341 total, so startup health now blocks only on the 13 truly blocking data-health tables before any business work can claim freshness.
