...
| `services/data_access/` + `config/data_access.yaml` | **SERVE 读侧统一层** (2026-06-22 P1, 数据模块顶层设计 v2 §4, task#52): 读侧唯一取数+PIT+口径清洗点, 消费者全走 `DataAccess.get(entity,codes,start,as_of)->DataResult(rows,provenance)`。薄分发器(__init__)+内部分层防god-module: `keys`(code↔ts_code+日期归一 canonical 单一源, 不变量#1)/`spec`(yaml载入+EntitySpec+provenance信封)/`resolver`(manifest路由+connect read_only 不变量#2+preflight schema自校验)/`asof`(PIT≤t 读层单一执行点)/`cleaner`(code6位/asof ISO 归一)/`drivers/generic`(简单entity取数, 加entity=加yaml条目+可加driver不改本体)。data_access.yaml 7 entity(kline_qfq/moneyflow/valuation/index_daily/moneyflow_dc/cyq/stk_limit)。血缘=provenance信封声明链。7单测+真库冒烟。**dossier 迁移进度: load_capital/load_cyq/load_limits/load_share_float/load_block_trade/load_analyst_reports/load_benchmark/load_valuation/load_fundamentals/load_forecast/load_one/load_sector_kline/load_lhb/scan枚举/_top10/sector_membership/market_regime 全18内联裸查迁完(dossier duck_connect=0, 硬门+2scan, +DataAccess.distinct_codes, parity一致+消费者依赖实测)**; code_input支持指数码; 剩~8 复杂/敏感(load_lhb order敏感/forecast grouping/market_regime聚合/sector+holders+fundamentals 需复杂PIT driver — 真金白银敏感需谨慎不rush) loader待迁。**剩余 loader 已识别 nuance (逐个细迁不批量, 防parity错)**: load_valuation/analyst/lhb/block/share_float 带 as_of-window(传start)+order DESC(loader重排); load_analyst tp/10000 单位(待移entity clean); load_lhb top_inst exalter LIKE(loader过滤); load_market_regime limit_list_d COUNT(DISTINCT)聚合+index码; load_benchmark/regime index码(000300.SH 破code_to_ts_code启发, 需 code passthrough); _top10_tushare/fundamentals/sector 需复杂driver(ann_date版本锁/latest-per-end_date/in-out_date PIT)。待: 复杂entity driver(holders ann_date版本锁/fundamentals latest-per-end_date/sector in-out_date PIT)+4 moth门(read-no-inline-table/read-no-self-asof/feature-from-l2/lineage-complete) |
