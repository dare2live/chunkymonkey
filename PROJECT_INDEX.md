...
- **2026-06-23 by_ts_code 事件数据按报告期增量 (修谄媚死: 十大股东/财报日常流停更)**: 实测根因 — by_ts_code 域(十大股东/财报)在 --all-due --drain 被显式跳过(line814 "归专门调度")=日常流根本不更新, 且手动 --resume 跳整股=存量股新季永不补。修 (sync_runner): `_latest_expected_report_period(today)` 按A股季报截止日(Q1→4/30/半年→8/31/Q3→10/31/年报→次年4/30)算最新应披露期 + `_stocks_up_to_date` 查每股 MAX(end_date) + `_by_ts_code_batches` increment_mode=by_report_period 跳已最新股只抓缺新期 + drain wiring(by_report_period 域不再跳)。sync_registry: top10_floatholders/fina_indicator 加 increment_mode(fina_mainbz 固定年报期不适用)。实测今天目标期Q1(20260331): 5177已最新跳/只抓31缺新期股(vs全量5208)。10单测(报告期9 cases+增量跳过) + calendar-gate/rule-compliance过 + moth44/0/0。owner=sync_runner.py + data_module_architecture_v2 §核心1。
