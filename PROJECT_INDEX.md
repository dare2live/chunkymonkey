...
- **doctor live registry boundary**: 2026-06-02 `audit_stage_opt_candidate_supply.py` / `doctor --fast` 会把 live formula registry 显式带出来，live stage-opt supply 只有 7 个公式 id (`macd_golden_cross`, `turtle_breakout_20`, `turtle_breakout_55`, `dynamic_ma_iterative_cross`, `reversal_1m_mild`, `reversal_1m_deep`, `reversal_1w`)；`bestchoice` 的 `gs_*` / `volume_base_breakout` / `activity_breakout` / `ma_base_breakout` families 仍是 research-only challengers，不计入 live supply。
