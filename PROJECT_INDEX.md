...
| `backend/services/portfolio_returnbacktest.py` + `backend/tests/test_portfolio_returnbacktest.py` | **Tier-2 含成本回测引擎 (2026-06-15 干净重建; 旧 portfolio_backtest.py[5-07,reset漏网]退役标P2)**: 透明 return-based — 等权 long-only, 日度 mark-to-market, 显式 T+1 (决策日下一交易日入场), 显式换手成本 (卖+买权重×bps), 内联 metrics (年化/回撤/sharpe/calmar/月胜率), 零旧依赖。4 单测手算证伪门 (已知场景/T+1无lookahead/缺价剔除/空篮)。用户 push back "旧引擎可能有问题" 证实: 旧引擎 cash*0.95欠仓+5%容差致 -42%假象, clean 引擎实测 Stage1.5 reversal 周度 net-2.8%/gross+7.1%(Sharpe+0.42) — edge真但~5天快衰减+成本结构杀 (KPI_FAIL, 详 tier2_conditional_backtest_20260615) |
