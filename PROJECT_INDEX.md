...
（禁 all-due；K/ST 禁 drain）；sync 禁 legacy raw 直写 canonical。margin=`on_demand`+`scope_blocked`（frozen；禁 thaw；acquire observe_frozen + continuity observe；硬门仅 enabled 时启用）。
