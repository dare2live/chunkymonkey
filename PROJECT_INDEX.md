...
- **stage-opt candidate supply audit**: 新增 `backend/scripts/audit_stage_opt_candidate_supply.py` + `backend/tests/scripts/test_audit_stage_opt_candidate_supply.py`，`audit_test_tool_health.py` scoped PASS，`pytest -q backend/tests/scripts/test_audit_stage_opt_candidate_supply.py` 2 passed，`codegraph sync .` 已更新 2 个文件，`complexity-optimizer` 复扫后该 slice 没有引入新的 HIGH；在当前审计 slice（2023-01-01→2026-05-29, limit-stocks 50）看到 3576 raw_signal_rows / 831 unique_keys / 280 ready_keys / 33.69% ready coverage / 551 below_min_signals，说明 stage-opt 供给依旧偏薄，下一 tuning lever 仍是 upstream candidate supply / formula coverage。脚本默认 end 现在复用当前连接里的交易日历真相源，不再 nested `latest_closed_or_raise()` 新连接。
