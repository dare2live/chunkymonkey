...
- **Shared config boundary cleanup**: 通用 holding_days、stage 口径、MACD 诊断窗口、turtle_breakout 量能确认门槛、shareholder-plan walk-forward 默认 fold/train/holdout、multidim walk-forward 默认模型参数/degenerate 门槛、stock-formula optuna bucket / high-conviction 阈值和 backtest 默认 stop/target/trailing 已收敛到 `backend/config/formula_shared_windows.yaml`、`backend/config/technical_stage.yaml`、`backend/config/formula_macd_golden_cross.yaml`、`backend/config/formula_turtle_breakout.yaml`、`backend/config/shareholder_plan_family_walkforward.yaml`、`backend/config/run_multidim_walkforward.yaml`、`backend/config/stock_formula_optuna.yaml`、`backend/config/strategy_defaults.yaml`；`backend/services/formula_engine/shared_windows.py` 统一加载共享窗口，formula-specific YAML 只保留专用阈值，per-stock 最佳持股周期继续表化在 `mart_per_stock_stage_strategy_optimal_pit` / `mart_stock_horizon_profile` 这类 marts 中，不再在 Python 常量里复制一套一样的窗口。
- **stock-formula optuna shared bins externalization**: `backend/config/stock_formula_optuna.yaml` 现在统一承载 `min_n_per_bucket` / `min_win_high_conviction` / `min_n_high_conviction` 以及共享 `vol_bins` / `amt_bins` / `p60_bins`；`backend/services/stock_formula_optuna_config.py` 做 strict fail-closed loader，`backend/scripts/build_stock_formula_optuna.py` 和 `_v2.py` 只消费共享 config，不再各自 hardcode 一套相同 bins；`backend/tests/test_stock_formula_optuna_config.py` 锁住正常加载、缺 key fail 和 bin shape 校验。验证结果：`pytest` 3 passed，`audit_test_tool_health` PASS，`docs graph` PASS，`codegraph sync .` PASS，`worktree` PASS。共享规则继续留在 config，个股最佳持股周期继续表化，不回 Python literal。
