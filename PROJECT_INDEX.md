...
最后更新: **2026-06-03** (K-line + alpha158 + stage/context/trigger + capital_flow/risk/sector + picture/survey + label/score freshness catch-up + portfolio_sizer config + dirty safety/tooling ledger + model monitor widget extraction + workbench health widget extraction + ETF workbench widget extraction + signal adapter aggregation cleanup + stock summary delegate cleanup + data-view audit model cleanup + paper sim model extraction + storage model extraction + tdx f10 source-date audit model extraction + tdx f10 source-date dq model extraction + research model extraction + pipelines model extraction + data-view cockpit panel model extraction + stock timeline ordering delegation + data-view source cards detail cache + data-view link overview model extraction).
