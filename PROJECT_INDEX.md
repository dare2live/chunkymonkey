...
(a) 4 复用点: ① pipeline `_step_sync_lhb`(**[DONE]** 搬 services.lhb_client, pipeline 直调 routers.updater 引用=0, daily dry 跑通) ② etf.py connectivity ③ data_sources.py DAG topology(RUNNERS/STEPS/HARD_DEPS) ④ main.py 路由挂载。爆炸半径实测=仅 main.py 挂载(updater/etf/data_sources 是 leaf, 无 service 依赖)。剩余: 解耦②③④ → 去 FastAPI 挂载 → 删20文件+tdx F10 client(tdx_f10_extra_client/ingest_holders_tdxhub)+raw表 → daily 验证。增量做不 big-bang。**产品3 同大股东个股决议**: 不迁不丢 → 机构档案"某机构持哪些股"实测可从 aif10 holder 数据 holder_name 分组 derive(香港中央结算持3462股/社保2442/摩根343), 退役 tdx 产品, 用例 derive。
