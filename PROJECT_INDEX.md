...
- **stage-opt 2026-06-02 04:48 latest rebuild**: `reversal_1m_deep` 继续维持 8-30%，`mart_macd_state_history` 仍作为 180-day warm-up 的诊断 mart；最新 full-history audit 变成 `raw_signal_rows=5,085,286 / filtered_signal_rows=2,550,775 / unique_keys=147,441 / ready_keys=101,382 / ready coverage=68.76% / below_min_signals=46,059`，其中 `macd_golden_cross` 达到 `1,714,731 signal_rows / 46,120 keys / 84.75% coverage`，`min_signals=4/3/2` 现在对应 `75.33% / 82.36% / 90.25%`。这说明阈值和 state-history 只是继续抬高 evidence density，但 controller 仍指向 `P1 / upstream_candidate_supply`，所以瓶颈结论没变。
