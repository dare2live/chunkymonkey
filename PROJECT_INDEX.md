...
- **BestChoice context/feed complexity diff 收束**: `backend/scripts/build_bestchoice_context_exit_policy.py` 改为按候选股票批量加载 K 线并用 `executemany` 写入 context policy，移除 per-candidate K-line query 与 per-row insert；`backend/scripts/build_bestchoice_phase2_daily_feed.py` 改用 `bisect_right` helper 计算严格 T+1 buy date，并把 rank-before-dedup 契约固化为 helper 和单测。新增 `backend/tests/scripts/test_bestchoice_complexity_helpers.py`，5 passed；scoped audit PASS；broad complexity scan 前 120 项不再出现这两个脚本。`doctor --fast` 仍 FAIL，下一窗口转为 candidate feature panel / daily recommendations / listing status / drift-safe feature candidates。
