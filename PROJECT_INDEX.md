...
| Factor-family inventory (RX 前) | Config `factor_family_inventory.yaml` + `check_factor_family_inventory.py`（结构门：族字段/brick/sync_domain/gate_matrix）+ 设计 `analysis/factor_family_governance_toplevel_20260724.md`；与 `brick_registry.yaml` / `sync_registry` 正交；**禁**开 RX 前跳过 inventory+gates |
