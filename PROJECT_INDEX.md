...
- **Main-force / risk-factor catch-up + sharpe cap relaxation**: `backend/scripts/backfill_capital_flow_pit.py --start 2026-05-27 --end 2026-05-29` 把 `fact_capital_flow_pit_daily` 补到 `2026-05-29`（13,169 rows / 5,163 stocks / 822 dates）；`backend/scripts/backfill_risk_factors_history.py --start 2026-05-27 --end 2026-05-29` 先因 `sharpe_60d` 字典 cap `[-5, 5]` 拒绝写入，随后把 `backend/config/field_dictionary.yaml` 与 `backend/scripts/backfill_risk_factors_history.py` 同步到 `[-10, 10]` 并顺利重跑（15,530 rows / 5,178 stocks / 3 dates）；`backend/scripts/backfill_sector_momentum_history.py --start 2026-05-27 --end 2026-05-29` 将 `fact_sector_momentum_daily` 补到 `2026-05-29`（39 rows / 13 sectors）。`audit_data_completeness.py` 现在是 PASS with WARN（0 FAIL / 2 WARN）：`fact_lhb_event` 与 `fact_technical_trigger` 已注册为 `sparse_event_presence_only`，因此只作为事件覆盖 WARN evidence，不再是 completeness blocker。主力资金链路已从 stale 变成 fresh，但 LHB 仍只能当事件覆盖证据，不能上升为生产结论。
