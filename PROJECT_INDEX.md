...
- **Bestchoice blueprint intake**: 已记录一份新的系统蓝图，方向是 `CatBoost` 特征矩阵 + 贝叶斯后验更新 + `Optuna TPE` 做参数寻优，执行层用 `ATR` / 时间止损 / 分数凯利做风险控制，底座是本地 `DuckDB` / `VectorBT`，数据湖走 `httpx` -> JSON -> `Parquet` -> `GCS`，重回测和大规模寻优放到 `GCP Spot`，最终信号分发可落到 `Cloud Run` / `FastAPI`；推荐顺序是先收敛数据管道和 truth-source，再跑通本地研究闭环，最后才上云做 Spot / Cloud Run 编排。`bestchoice` 公式事项暂列后续，不和当前 `stage-opt` / `need_027` blocker 线程混在一起。记录已同步到 `goal.md` / `SESSION_HANDOFF.md`，后续推进时按这条顺序拆任务。
