...
| `backend/services/technical_states/` + `config/technical_states.yaml` | **技术形态识别工具 (形态地基, 后续因子叠加的基础)** (2026-06-21 promote, 经 sandbox state_quantify v1/P0/P1/v3 验证: 全宇宙5421股各状态/子态语义准确+OOS稳+100%覆盖): 给任一股任一时点(日/周/月)识别技术状态+子态+量化特征。**7主态**(低位横盘/放量突破/上升通道/缩量上涨/高位滞涨/下跌通道/缩量回踩) **各自细分**(上升通道→温和/震荡/加速 等)。config 驱动(judgment 参数/k/τ/子态分位), 公式结构在 classifier.py; **软隶属度**(softmax τ, 一股可部分属多态+报熵)替 argmax(根因修复 覆盖70→100%); **多时间框架**(日主+周/月确认 mtf_aligned, 窗口按TF缩放); PIT(特征≤t, 量基准排当日)。API: compute/classify_bar/classify_series/classify_stock/classify_multi_timeframe。test_technical_states 6测(含PIT无前瞻+语义)。待: per-TF tune(现周月用日线参数=近似)+物化fact_stock_technical_state+FSM破位门(v2 spec, owner=sandbox/state_quantify/notes.md) |
