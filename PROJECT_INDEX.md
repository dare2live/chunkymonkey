...
| Moneyflow decision assist (3A+3C) | Tier3 `GET /api/v3/decision/moneyflow/board` + `/stock/{code}`；config `moneyflow_assist.yaml`；service `moneyflow_assist.py`（读 pulse mart + stock moneyflow facts；`flow_regime`→潜伏/抢筹/出货迹象；窗未满→unknown）；UI `#/market` 页签「市场感知｜资金决策辅助」+ dossier 资金；证据 `analysis/capability_a_moneyflow_assist_20260721.md`；感知卡保持零买卖暗示 |
