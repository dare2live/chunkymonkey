...
| Manual full data update | UI `#/workbench`「数据更新」→ `POST /api/v3/ops/jobs/daily_update/run`；或 `bash scripts/daily_update.sh --date YYYYMMDD`；acquire 先 formal `daily`/`stock_st` 最新 eligible 日 land_then_accept（已接受则 skip；same-day vacuum→`pending_publish` soft-continue→`--all-due`；stock_st post-09:20 同；证据 `foundation_acquire_all_due_unblock_20260722.md`）；due-plan 预览读 SLA JSON；holders skip-if-wm-unchanged；on_demand 不进 all-due；Cap E S1/S2 → `POST /api/v3/ops/pipeline/land-accept/run`；margin=`on_demand`+`scope_blocked`（frozen，不挡 preflight） |
...
| Holders 新鲜度水位 (0r.5b) | `holders_aif10.formal_holders_watermark` = formal canonical `notice_date` frontier（非 legacy fact `page_update_date`）；`sync_holders_aif10_incremental` 分栏 `net_new_notice_rows`/`notice_partitions_touched`/`rewrite_amplification_rows`+`watermark_source`；provider max ≤ wm → `skip_reason=watermark_unchanged` + heartbeat；`source_watermarks` 域 `holders_top10_float`→canonical，legacy fact 降为 `holders_top10_float_legacy_observer`（tier2 观察者）；证据 `analysis/foundation_holders_wm_ops_counters_20260721.md` + `foundation_acquire_all_due_unblock_20260722.md` |
