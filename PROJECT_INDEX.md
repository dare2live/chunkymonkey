...
| `services/data_access/` + `config/data_access.yaml` | **SERVE 读侧统一层** (2026-06-22 P1, 数据模块顶层设计 v2 §4, task#52): 读侧唯一取数+PIT+口径清洗点, 消费者全走 `DataAccess.get(entity,codes,start,as_of)->DataResult(rows,provenance)`。薄分发器(__init__)+内部分层防god-module: `keys`(code↔ts_code+日期归一 canonical 单一源, 不变量#1)/`spec`(yaml载入+EntitySpec+provenance信封)/`resolver`(manifest路由+connect read_only 不变量#2+preflight schema自校验)/`asof`(PIT≤t 读层单一执行点)/`cleaner`(code6位/asof ISO 归一)/`drivers/generic`(简单entity取数, 加entity=加yaml条目+可加driver不改本体)。data_access.yaml 6 entity(kline_qfq/moneyflow/valuation/index_daily/moneyflow_dc/cyq)。血缘=provenance信封声明链。7单测+真库冒烟。**dossier 迁移进度: load_capital/load_cyq 已迁(parity逐字一致 2050/667/1808行)**, 剩~14 loader待迁(load_limits/benchmark/sector/fundamentals/forecast/analyst/lhb/block/share_float/market_regime/_top10_*)。待: 复杂entity driver(holders ann_date版本锁/fundamentals latest-per-end_date/sector in-out_date PIT)+4 moth门(read-no-inline-table/read-no-self-asof/feature-from-l2/lineage-complete) |
