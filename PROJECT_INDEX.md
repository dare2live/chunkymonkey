...
| Holders 新鲜度水位 (0r.5b) | `holders_aif10.formal_holders_watermark` = formal canonical `notice_date` frontier（非 legacy fact `page_update_date`）；`sync_holders_aif10_incremental` 分栏 `net_new_notice_rows`/`notice_partitions_touched`/`rewrite_amplification_rows`+`watermark_source`；provider max ≤ wm → `skip_reason=watermark_unchanged`（1-row probe 须 `UPDATE_DATE>=` 边界，空 filter 返 0）+ heartbeat；**同日晚披露**可能在 wm 已对齐时漏网→稀疏 `--symbols` repair（非 mass）；`source_watermarks` 域 `holders_top10_float`→canonical，legacy fact 降为 `holders_top10_float_legacy_observer`（tier2 观察者）；证据 `analysis/foundation_holders_wm_ops_counters_20260721.md` + `foundation_acquire_all_due_unblock_20260722.md` + `holders_stock_coverage_alignment_20260723.md` |
