...
| `fact_rally_ground_truth` (**smartmoney.duckdb** L1) **D1 主升浪 ground truth 标签y** | **43,202 突破事件 / 4,345 TRUE主升浪(10.1%) / 5516股 / 2019-04→2025-05** | rally_ground_truth_scan.py --end 2025-05-31 --land (从git找回+改读clean tushare) | 2026-06-16 监督式 episode-first D1: 突破=close>前60日high, 主升浪=突破后60-180天≥50%涨幅 AND max_dd>-20%(读法B+60日冷却, S1复现99.92%); 买点=event_date=t(PIT锚, 特征<=t/label=t+1..180后验); TRUE中位涨幅75.8%(对原锚75.5%)峰位62日; **D2(analyze_episode_forms.py)买点形态弱判别(低位lift1.16/stage~1.0); D3(analyze_episode_trajectory多窗量价/analyze_episode_richfactors资金筹码/experiment_rally_multivariate多因子LightGBM)单因子+浅多因子全弱: OOS AUC 0.51≈shuffle null 0.48/top-decile 1.14x → 主升浪买点在此深度难预测, 待Optuna+Modal系统化搜(DSR-gated)重检**; **pivot 鱼身**(不抓起涨点改骑确认主升, owner=analysis/yushen_framework_design_20260616.md): experiment_yushen_backtest/portfolio(裸基组合含成本+3.1%/max_dd-46.8%=FAIL, 粗突破入场比随机仅+0.12%无alpha) → experiment_yushen_selective_entry(experiment_store family=yushen_entry_alpha): **二次突破(出箱→缩量回踩→再破前峰)入场比随机+2.81%/笔, 样本内bootstrap p=1.0/下界+1.40%铁, OOS+2.55% p=0.90欠样本(n=137); 粗突破死/二次突破活**; 下一步 Optuna 调入场参数(目标=比随机增量防拟合beta)提频→组合NAV→叠context因子; data_layers=L1_foundation |
| `fact_macd_episode_ground_truth` (**smartmoney.duckdb** L1) **D1 MACD金叉峰值 ground truth** | **280,324 episode / 5500+股 / 金叉买点→持有窗波峰** | experiment_macd_episode_scan.py --land (experiment_store family=macd_episode_ground_truth) | 2026-06-16 用户口径(金叉=买点唯一, 死叉≠卖点; 卖点=金叉后波峰探索): peak_gain_pct/peak_offset_days/max_dd_pct/is_win(峰>30%); win率10.8%/完整中位峰+6.6%; 出场对比(experiment_macd_exit_explore, family=macd_exit_rule): 死叉=最差(中位-2.25%)/hold_20最好(-0.24%)/raw金叉无edge; data_layers=L1_foundation |
