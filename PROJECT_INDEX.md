...
| T1 stock state | `technical_states/`, `segments.py`, `tier12_publish_{contract,writer,accept}.py`, `tier12_nominal_canary.py` | 多轴状态可复用；C canary `20260717` writer→accept（`accepted_*.json`/`published=true`/`cutover_allowed=false`；`accepted_at`=ingest stamp≠trade end_date）；legacy form bridge=NOT_PUBLISHABLE；未 consumer cutover / 未全市场 |
| T2 market sensing | `market_pulse.py`, `tier12_publish_{contract,writer,accept}.py`, `tier12_nominal_canary.py`, API/frontend | 展示可用但 breadth/margin UNTRUSTED（B-ext）；C accept envelope 同 canary；禁 mart/consumer cutover 直至显式门 + B-pit |
