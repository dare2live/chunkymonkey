...
| Tier1/2 full-universe accept (manual) | `PYTHONPATH=backend python backend/scripts/persist_tier12_full_universe_accept.py --decision-date YYYYMMDD`；accept 不自动翻 `consumer_cutover`（yaml opt-in 才切读） |
...
| P0 | E 120d checkpointed measured reject/no-gain；C full-universe accept `20260717`（4989=4989）；D FIXED（runtime-owned measured offline + persist）；B-pit 120d shadow **120/120 MATCH**；C/B-pit cutover **OWNER_OPTED_IN_CUTOVER_ON**（owner 显式 yaml flip 2026-07-20：`consumer_cutover.cutover_allowed=true`+`mart_cutover.cutover_allowed=true`→ ACCEPTED_CUTOVER / MART_CUTOVER；无 accept/窗外日 fail-closed→legacy；认证 `data/lineage/c_b_pit_cutover_readiness.json`）；enrichment 历史仍 field-level PARTIAL | 下一刀 stop **或** substrate 补富 form（非 Optuna / 非松门 / 非 mass backfill / 非静默 cutover / 非 StrategyRelease） |
