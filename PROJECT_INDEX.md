...
> **2026-06-20 D 按阶段因子矩阵 重定向 (用户纠偏; 前期买点 detour 全删)**: 前期 D-step 起涨点买点判别偏离计划 §0 诚实先验 (买点=secondary, 主攻=鱼身延续+鱼尾出场+仓位)，已全删。**正确路径 (用户确认)**: 在 `fact_rally_stage`(起涨/主升/顶部) 上逐阶段验因子, 优先级 **鱼尾出场 > 鱼身延续 > 鱼头买点**; 判据=**stage 窗内条件化持有(持到信号反转非固定调仓)→含成本绝对收益, IC 仅快筛非 AUC**; 鱼身=是否续持, 鱼尾=何时卖。**2026-06-20 进展+重优化**: 鱼尾 CYQ winner_rate 单用弱(REJECT_STANDALONE_WEAK, 早饱和提前79d触发); 补负样本分层 `fact_rally_negative_strata`; **净化后多因子 per-cap GBDT OOS 证"特征形态条件化"成立**(小盘筹码/大盘资金质量, Spearman0.39, SUPPORT_LIGHT_CONDITIONAL)。**优化后前向优先级 owner=`zhushenglang_hunter_plan_20260617.md` §4**: P1决胜=cap-条件化含成本逐日回放 OOS honest baseline(答30%) → P2扩面板 → P4 Optuna/Modal → P5上线。
>
...
| `fact_rally_negative_strata` (**smartmoney.duckdb** L1) **hard-negative PIT 分层** | **35,198 hard-neg** | build_rally_negative_strata.py | 2026-06-20: 镜像 episode_strata 给负样本同口径分层(cap/base/申万 as-of), 桶定义逐字复用(单一真相源)。缘起 wf_c5cc441d: 负样本无分层=做不了层内 pos-vs-neg 对照。**注**: 负样本年份/底长分布系统不同正样本(rally聚rally年)=真实总体, 不在builder重采样; year×base匹配是消费侧分析时做。data_layers=L1_foundation |
