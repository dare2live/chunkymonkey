...
- **2026-06-23 数据底座架构 v2 定稿 (用户愿景: 两模块解耦 + 统一采集规划器)**: owner=analysis/data_module_architecture_v2_20260623.md。**① 数据更新模块**(全部域: 交易日历定目标→diff存量定增量→抓+清洗+正确存储+每节点RULE ZERO)与**② 数据加工模块**(读存量按需加工供展示+前端API)**解耦, 独立 entry**。**核心缺口实测**: by_ts_code 事件数据(十大股东/财报by股 4域)无"按上一公告日扫增量"逻辑(现全量重拉或--resume跳整股=存量股新季永不补=谄媚死); by_trade_date(26)/by_period(2)有 watermark 增量。设计: AcquisitionPlanner.plan(domain) 统一增量(by_ts_code 新建 plan_ts_code_increments 每股 MAX(ann_date)→扫新期) + assert_node_clean 每节点核日历+排除列表+数量。Phased: P1 holders物删(双轨99.62%过)→P2 采集规划器+RULE ZERO门(核心)→P3 update/process解耦entry→P4 加工API收口→P5删源续。
