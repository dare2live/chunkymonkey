...
- **System data-health snapshot fail-closed in `doctor`**: `backend/scripts/data_health_snapshot.py` is now a first-class system gate inside `scripts/chunkyctl doctor --fast`; dry-run JSON now caps `warning` / `monitor_only` assets to yellow and returns `WARN` on 0 red / 12 yellow / 341 total, so startup health now surfaces the 12 yellow maintenance items before any business work can claim freshness. 2026-06-01 also repaired the writer timestamp normalization path, so compact timestamps no longer crash the insert, and the official `cron_daily.py` run now surfaces the real system prompts instead of dying on a parse error: `sync_raw` exceeded its 60s budget, later phases still ran, health settled at 0 red / 12 yellow / 341 total, and each red/yellow row now emits `writer_prompt` owner/sync_step hints for self-triage. Feature panel lane was refreshed successfully and removed `fact_feature_panel` / `mart_feature_panel_validation` / `mart_feature_panel_prune_run` from red, the subsequent `capital_behavior` lane cleared `dim_capital_behavior_latest`, holder/shareholder-plan lane also cleared red after parse-raw-only + backfill + mart rebuild, and GPCW / raw_aif10 are now yellow maintenance only rather than blockers; `blocking_yellow_tables` are surfaced separately so `quality_gate_level=blocking` yellow assets get next-action priority before generic yellow maintenance, and `mart_p0b_lambdamart_v6_predictions` was refreshed on 2026-06-01 so the only blocking yellow table is now gone; `raw_executive_trade` and `fact_executive_trade_event` were freshly rebuilt locally, and `sync_surveys` then refreshed `raw_institution_surveys` and `mart_stock_survey_activity`, dropping health from 17 yellow to 13 yellow before the full-history `akshare_panel` refresh was interrupted after `fact_jgdy_event` and left `fact_dzjy_event` stale, so current health is 12 yellow.
