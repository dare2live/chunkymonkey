...
> 更新：2026-07-21
> 当前目标看 `goal.md`（**foundation solidify CLOSED / phase_closure_ready** — `analysis/foundation_phase_reeval_20260721.md`；S1–S6 FIXED；S7 near-FIXED 23/46 typed wall；**E0-HIST/F6 PASS**；**FND-GATE PASS** F1–F10；**F8 §15-VERIFY PASS**；org BLOCKED；Type-B **defer**；策略 paused；`BOARD.md`；`scripts/chunkyctl agent-boot`）。
...
| 交集最强股 decision assist (4D) | Tier3 `GET /api/v3/decision/intersection/strongest` + `/intersection/stock/{code}`；config `decision_intersection.yaml`；service `decision_intersection.py`（DC 行业∩概念 strong-sector 会员交集，复用 `moneyflow_assist` behavior 标签 + `dc_member` 观察日 PIT）；链 as-of 不一致/滞后 SLA → `status=stale`（fail-closed，非假新鲜）；UI `#/market` 页签「交集最强」+ dossier 交集 tab；证据 `analysis/capability_d_intersection_strongest_20260721.md`；pytest `tests/test_decision_intersection.py` ∈ blocking |
| 形态/阶段选股面 (5B) | Tier3 `GET /api/v3/screener/options`（活体 facet 计数）+ `/form_stage`（筛选决策列表，纯过滤无评分/排序）；config `stock_screener.yaml`；service `stock_screener.py`（直读 `fact_stock_form_daily`，与档案 F `_load_form` 同一读路径/表；`sql_where_active_a_share` 防御性过滤）；全局 `MAX(trade_date)` 滞后 SLA → `status=stale`；UI `#/market` 页签「形态/阶段选股」，结果行点击跳 `#/stock/:code`；证据 `analysis/capability_b_stock_screener_20260721.md`；pytest `tests/test_stock_screener.py` ∈ blocking |
| Holders 新鲜度水位 (0r.5b) | `holders_aif10.formal_holders_watermark` = formal canonical `notice_date` frontier（非 legacy fact `page_update_date`）；`sync_holders_aif10_incremental` 分栏 `net_new_notice_rows`/`notice_partitions_touched`/`rewrite_amplification_rows`+`watermark_source`；`source_watermarks` 域 `holders_top10_float`→canonical，legacy fact 降为 `holders_top10_float_legacy_observer`（tier2 观察者）；证据 `analysis/foundation_holders_wm_ops_counters_20260721.md` |
