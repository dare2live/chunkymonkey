...
| `etf_price_kline_qfq_tushare` (etf.duckdb) **ETF前复权 (M2 Stage C)** | 136万行 / 1826 ETF / **2019-01 → 2026-06** | build_etf_kline_qfq_tushare.py | 2026-06-25 §4.3 M2: raw_tushare_fund_daily×fund_adj 前复权(rebased, OHLC同乘, vol不×100/amount千元×1000); 510300逐码1811==raw MATCH; 对账vs mootdx avg 8.7e-05 PASS, **修mootdx ETF分红未复权bug**(mootdx除息日不调+有glitch); 7单测(合成数据验复权公式); data_layers=L1k; 余 D repoint etf_engine/snapshot/mining + E物删mootdx链(escalate) |
