...
| T1 stock state | `technical_states/`, `segments.py`, `tier12_publish_{contract,writer,accept}.py`, `tier12_nominal_canary.py` | 多轴状态可复用；C canary `20260717` writer→accept（`accepted_*.json`/`published=true`/`cutover_allowed=false`）；legacy form bridge=NOT_PUBLISHABLE；未 consumer cutover / 未全市场 |
