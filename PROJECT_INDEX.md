...
- **Shared config boundary cleanup**: 通用 holding_days、stage 口径、MACD 诊断窗口和 turtle_breakout 量能确认门槛已收敛到 `backend/config/formula_shared_windows.yaml`、`backend/config/technical_stage.yaml`、`backend/config/formula_macd_golden_cross.yaml`、`backend/config/formula_turtle_breakout.yaml`；`backend/services/formula_engine/shared_windows.py` 统一加载共享窗口，formula-specific YAML 只保留专用阈值，per-stock 最佳持股周期继续表化在 `mart_per_stock_stage_strategy_optimal_pit` / `mart_stock_horizon_profile` 这类 marts 中，不再在 Python 常量里复制一套一样的窗口。
