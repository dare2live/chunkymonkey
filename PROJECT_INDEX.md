...
| Stock dossier MVP (F) | UI `#/stock/:code` → `GET /api/v3/stock/{code}/dossier`（沪深A 白名单；form/stage/holders；lineage audit **PARTIAL** — parse+~98% join PASS，机构档案~54%；**0r.5b**：per-row `has_institution_profile` 只在 profile 存在时 deep-link，coverage 标注，无假链；排期 `analysis/product_plan_reeval_stock_dossier_20260721.md`）；pytest `tests/test_stock_dossier_api.py` ∈ blocking |
