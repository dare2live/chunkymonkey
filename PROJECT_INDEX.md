...
最后更新: **2026-06-01** (K-line + alpha158 freshness catch-up + dirty safety/tooling ledger).

## [INDEX] 2026-06-01 增量

- **Alpha158 safe-window writer + freshness catch-up**: `backend/scripts/build_alpha158_duck.py` 不再默认先 `DROP TABLE fact_alpha158_panel` 再整表重建；现在先计算临时 build table，再事务内按 `--write-start..--end` 替换目标窗口，空窗口默认 fail-closed，显式未来 `--end` 会拒绝。新增 `backend/tests/scripts/test_build_alpha158_duck.py` 覆盖窗口替换保留旧行、空窗口拒删、未来日期拒绝。真实库用 `--start 2026-03-01 --write-start 2026-05-26 --end 2026-05-29` 补齐 alpha158：`replace_window` / 20,736 rows / 5,185 codes / 4 dates；`audit_data_completeness.py` 仍 FAIL，但 `fact_alpha158_panel` 已 OK: max=2026-05-29 / n_codes=5,183 / `= cal`，刷新窗口 duplicate=0。下游仍阻塞于 label/v3/v4/trigger/LHB/sniper/institution stale，`audit_end_to_end.py` 仍 FAIL 4。
