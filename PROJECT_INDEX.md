...
- `backend/scripts/probe_source_capability.py`: 新增通用 capability probe CLI, 可按 capability + prefer_source 输出 JSON 摘要或 blocked 证据；`need_027` 现场 probe 仍会被 Eastmoney `ConnectionError` / `JSONDecodeError` remote disconnect 卡住，因此能力注册和实际 exact-flow 仍需分开看。
