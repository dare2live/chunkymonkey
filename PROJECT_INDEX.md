...
- **System data-health snapshot fail-closed in `doctor`**: `backend/scripts/data_health_snapshot.py` is now a first-class system gate inside `scripts/chunkyctl doctor --fast`; dry-run JSON now caps `warning` / `monitor_only` assets to yellow and returns `FAIL` on 10 red / 20 yellow / 341 total, so startup health now blocks only on the 10 truly blocking data-health tables before any business work can claim freshness. 2026-06-01 also repaired the writer timestamp normalization path, so compact timestamps no longer crash the insert, and the official `cron_daily.py` run now surfaces the real system prompts instead of dying on a parse error: `sync_raw` exceeded its 60s budget, later phases still ran, and health stayed critical at 10 red / 20 yellow / 341 total.
