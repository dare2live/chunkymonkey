...
| Live continuity | `PYTHONPATH=backend python backend/scripts/check_continuity_integrity.py` (`FAIL` 直接非零；daily/ST 读 `accepted_partition` formal frontier；**F1 typed gaps FIXED** — `hk_holidays`/`event_sparse` → live **PASS warn=0**；禁 mute/READY cosmetics；证据 `analysis/continuity_f1_typed_gaps_20260723.md`） |
...
|---:|---|---|
| P0 | K accepted + form/qfq/segments/pulse 至 `20260721`（扇区/DC pulse 可能滞后）；legacy raw daily 仍 `20260716`（预期）；`index_dailybasic` 短窗 min_rows 拒写；margin **1b FIXED** = click-update/`daily_update` acquire 日历缺口自动 land/accept（v3 SSE+SZSE；`on_demand`；禁 all-due）；frontier=accepted `contract_version=3`；rzrqye **UNTRUSTED** + F4 **`promote_gate` PARTIAL**（PENDING_SERVE_CUTOVER；证据 `margin_f4_promote_gate_20260723.md`）；禁 mass / product thaw | 两融列诚实 unknown until serve→accepted cutover；估值水位可能 stale |
...
| P0 | qfq physical lineage FIXED (batch_id/ingested_at/factor_as_of on rebuild); still not execution truth; used broadly as analysis input; **Knife 3 FIXED** — full DROP+CTAS rebuild then **in-module** `db_compact` (default; escape `--no-compact`) so update/derive/clean 不遗漏 free-block reclaim; orchestrator 不知 DuckDB free pages | Pin batch_id for reproducibility; never treat qfq as nominal execution price; ban one-shot补跑-only “fix” |
