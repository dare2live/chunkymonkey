...
最后更新: **2026-06-01** (Phase7 context bulk K-line + remaining complexity diff).
...
- **Drift-safe feature candidates complexity diff 收束**: `backend/scripts/build_drift_safe_feature_candidates.py` 将 fold-stable 贪心评分改为 `_FoldScoreState` 增量 fold-sum/state scoring，避免每个候选都重算 `stable + [row]` 全子集；将 balanced variant、feature-row serialization、drift JSON fold parsing 和 DDL fallback 拆成单层 helper，并把循环内重复 `sorted(excluded_severities)` 提到循环外。新增 `backend/tests/pipeline/test_drift_safe_feature_candidates.py::test_fold_stable_variant_matches_subset_score_greedy_order` 固化与旧 `_subset_fold_score` 贪心顺序等价。`scripts/chunkyctl audit --run --scope ...` PASS；per-file complexity 无热点；CodeGraph 已 sync。`doctor --fast` 仍 FAIL，但 remaining `new_high_count=9` 已转移到 daily recommendations / ensemble context / executive trade events / feature association，storage payload 仍为 WARN。
- **Phase7 context complexity diff 收束**: `backend/scripts/build_ensemble_v7_phase7_context.py` 将 per-stock K-line query 改成一次批量 JOIN + context map，避免 v7 预测行/股票数放大时的 DB N+1；新增 `backend/tests/scripts/test_build_ensemble_v7_phase7_context.py` 覆盖 code/date/freq/adjust 过滤、短/空价格序列跳过、正向 context 才保留 score。`scripts/chunkyctl audit --run --scope backend/scripts/build_ensemble_v7_phase7_context.py --scope backend/tests/scripts/test_build_ensemble_v7_phase7_context.py` PASS；`doctor --fast` 仍 FAIL，剩余 `new_high_count=9`: daily recommendations 2、executive trade events 4、feature association 3；下一真实热点是 executive forward-return lookup。
