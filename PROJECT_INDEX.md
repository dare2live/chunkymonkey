...
- **doctor stage-opt surfacing**: `scripts/chunkyctl doctor --fast` 现在会把 stage-opt 的 `next_action_recommendation` 一并带出来，controller 可以直接看到 upstream candidate-supply triage 提示，不必单独 rerun audit 才拿推荐。
