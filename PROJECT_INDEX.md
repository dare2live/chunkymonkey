...
|---|---|
| Session boot context (git+moth+codegraph+board, one page) | `scripts/chunkyctl agent-boot [--format json]`；cutover 行标注 **yaml 意图**，并在 yaml 说 true 而窗口已 lapsed 时多打一行 `**b_pit 实际已不生效**`（携 `b_pit_effective` 探针：窗末+1 的 resolver 裁决）——只改 `BOARD.md` 不改本页 = 没修到 agent 每次 session 实际读的消费面 |
