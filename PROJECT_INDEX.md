...
- **2026-06-26 通达信全删 gpcw整体退役 (财务迁移已promote上线后, 用户"退役然后继续主线")**: 财务迁移 promote DONE(live dim/fact 已 tushare 正确值)。gpcw 簇物删 fan-in 审计: gpcw 摄取流水线全 dead(tdx_affair_client 0import / sync_financial_data 0caller / build_tdx_gpcw_auto_features 0caller 未物化), 唯一 live 消费方=signals_v2(raw_gpcw_detail D1/D3 过滤器)。**Stage1 DONE**: 切 signals_v2 gpcw 依赖(_load_gpcw_feature_maps 返空→D1散户化/D3业绩预告过滤器优雅 no-op, 用户"信号重做不用旧的") + 删 data_access financial_gpcw entity; signals_v2 78测试全过。**sync确认全DEAD**(sync_financial_data 0caller; daily财务sync走registry tushare _sync_registry_drain; tdx_affair_client/build脚本 0daily caller; gpcw 7表=冻结legacy无active写)。**剩余=物删+wholesale代码退役=focused session级**(跨~8 live-shared文件: ensure_tables[live calc共用]/audit.py import[FIN_HISTORY_*]/schema_core DDL/10 sync测试/单元5耦合 fact_fundamental_quarterly L1须保/~15 config; 不可逆)。安全已确认但**不在session尾 big-bang赶**(用户反例:删探索物没审耦合连崩CI), 下焦点session专做。owner=analysis/tdxhub_full_retire_plan_20260626.md。
