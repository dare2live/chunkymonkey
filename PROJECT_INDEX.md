...
> 更新：见 `git log -1 --format=%ad -- PROJECT_INDEX.md`（不手写日期；本文件由 map 重生）
> 当前目标看 `goal.md`（2026-09-02 起：策略验证项目，系统只回答"手写公式历史上触发后发生了什么"）；进度看 `scripts/chunkyctl status`。
> 执行方案不在本文件；数据底座与判例引擎的施工顺序见 `docs/README.md` 指向的 owner 文档。
...
AGENTS.md
  ...
  -> scripts/chunkyctl status | scripts/chunkyctl status + strategy_validation_contract.md §3.2/§3.3  (execution only)
```
...
| 强势股三步漏斗 v1 (备用规格, 未接线) | `backend/config/strategy_packages/strong_stock_funnel_v1.yaml`；业主 2026-09-02 口述转录。三级漏斗: 量能(涨幅3-5% + 量>前5日均量2倍 + 换手3-10%) → 走势结构(均线多头排列 + 站上MA5 + 近一月放量拉升 + 缩量回调不破MA10) → 筹码(90%成本区间宽度<20% 且 70%成本集中比例<15%)。**刻意不入 `strategy_spec.ALLOWED_PACKAGES`** —— `load_all_strategy_packages` 按白名单遍历而非扫目录(strategy_spec.py:435), 故多放一个 YAML 无副作用, 与既有三包同目录但不被旧 StrategySpec/claimable 机制加载。未定阈值**全部标 TODO_OWNER 未填**(哪几条均线/放量拉升阈值/缩量比例/"一个月"是20交易日还是30自然日/成本区间宽度相对什么), 不替业主猜。两条实测约束写进文件头: [C1] 第三步数据 `raw_tushare_cyq_perf` 2026-09-10 起冻结(tushare 专有算法输出, 授权到期不续); [C2] 操作段的分时/15min/60min 本项目无数据(goal.md 不做高频/日内), 记录为人工纪律。**结构观察**: 三级漏斗与 goal.md:21-29 的"逐因子分桶"同构(量能/形态/筹码 = 三个单因子), 故其价值不止选出 1-2 只, 更在于回答"只用第一步 vs 加第二步 vs 加第三步各差多少"; 某级增量近零即该级应被质疑 |
| 冻结落地戳 vs 活契约 静态门 | `PYTHONPATH=backend python backend/scripts/check_frozen_stamp_compare.py`；AST 扫 `backend/services`，抓三种真实写法形状：①直接比 `str(batch["config_hash"]) != contract.config_hash` ②同函数内经名字流过的元组比 ③SQL `WHERE ib.contract_hash = ?`。**判据**：算法派生的指纹(`contract_hash`/`config_hash`/`payload_hash`)与传输轴标签(`source_name`)是**落地那一刻的封印**，只能用来重算封印，**不能与任何「现在」的值比相等**；可跨侧比的只有人声明的身份(`contract_version`/`writer_id`/`dataset_id`/`partition_value`)、内容(`canonical_hash`/`row_count`)与时间链。起因：2c4af4a08 把 source/api 移出 config_hash 后 `accepted_partition`/`canonical_*` 重打了戳而 `ingest_batch` 有意冻结，于是散落在 N 处的「批次戳 == 指针戳」断言必然为假——实测造成 `open_calendar_truth` 永久 BLOCKED（连带项目宇宙人口与 universe 门）、`margin_acceptance` 追赶卡在 stale checkpoint。门 docstring **诚实列出自身盲区**（dict 键循环/下标取列/拿活契约重算封印/跨函数），这些由 3 个行为测试兜底；pytest `tests/scripts/test_check_frozen_stamp_compare.py` + `tests/services/test_{calendar,margin,accept}_frozen_landing_stamp.py` ∈ blocking |
...
| Rewrite must-keep vs delete | 裁决折入 `scripts/chunkyctl status` §4：KEEP = sync replace / qfq incremental+full CTAS+in-module compact / landing+skip / delta rebuild；**DELETED** = `rewrite_legacy` True + canary CLI；禁 periodic dedupe/compact fixer |
