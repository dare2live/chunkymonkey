...
| 表 | 数据量 | freshness | 用途 |
|---|---|---|---|
| `dim_stock_sw_industry` | dim | **每日刷新 (daily_update Step 2.96c)** | 申万行业映射 (serving 行业真相源, 6 消费方读此) |
| `dim_stock_tdx_industry` / `_history` | dim / dim history | **源退役冻结 2026-06-23** | 通达信行业 (删源不删数据: sync 删/消费方切申万/表保留不物删) |
