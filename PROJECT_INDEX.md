...
| TDX HQ host catalog | Official = TDX client `connect.cfg` `[HQHOST]` via `TDXHUB_CONNECT_CFG` (read-only parse, never `bestip`); `tdxhub.consts.HQ_HOSTS` is community fallback; pin `TDXHUB_HQ=ip:port` when a handshake-ready host is known |
