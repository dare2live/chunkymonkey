...
| T3 institution | `institution_profile.py` + `holders_aif10.py` + `disclosure_boundaries.py` + `disclosure_dual_write.py` + `disclosure_shadow_compare.py` + `*_acceptance.py` (holders/org/stk) + router/tests | **首个正式策略包目标**；E0：formal→legacy-mirror 写 + `/api/v3/inst` shadow sidecar；研究数值仍 legacy；live canonical 未落 → DatasetSnapshot/E 仍 BLOCKED |
...
| P0 | 披露域 E0 未闭合（formal+mirror+fixture shadow 齐；live canonical 未落；研究未切） | 生产 accepted partitions → live shadow MATCH → 研究切读 → 停 mirror → DatasetSnapshot；institution_follow 仍不可满足 |
