...
| T3 main rally | `main_rally_dataset_snapshot.py`, `main_rally_b0.py`, `main_rally_b0_measure.py`, `main_rally_b1.py`, `main_rally_b1_measure.py`, `rally_gt.py`, `rally_detect.py`, rally config/tests | GT 资产成熟；**F0+F1+F2 FIXED**：`main_rally_v1` freeze + B0 setup-entry short-horizon + B1(+Tier1 stock state, 同 B0 folds/costs, `REQUIRE_HOLDOUT_LIFT_VS_B0`) measured → both reject/`claimable=false`（共享 `research_runtime`；非 full-episode；F3 B2 未做） |
