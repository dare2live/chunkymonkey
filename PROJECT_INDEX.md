...
| Stock dossier MVP (F) | UI `#/stock/:code` → `GET /api/v3/stock/{code}/dossier`（沪深A；form/stage/holders + episode overlay；机构档案 deep-link ≈ episode coverage + thin/passive honesty labels）；**资金 tab** → `GET /api/v3/decision/moneyflow/stock/{code}`（3A）；排期 `analysis/product_plan_reeval_stock_dossier_20260721.md`；pytest `tests/test_stock_dossier_api.py` + `tests/test_moneyflow_assist.py` ∈ blocking |
