...
| T3 institution | `institution_profile.py` + `holders_aif10.py` + `disclosure_boundaries.py` + `disclosure_dual_write.py` + `disclosure_shadow_compare.py` + `*_acceptance.py` (holders/org/stk) + router/tests | **首个正式策略包目标**；E0：formal→legacy-mirror 写 + `/api/v3/inst` shadow sidecar；研究数值仍 legacy；live holders `20260717` canonical MATCH（org/stk 仍缺）→ DatasetSnapshot/E 仍 BLOCKED |
