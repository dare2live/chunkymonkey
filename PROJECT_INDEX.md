...
| T3 institution | `institution_profile.py` + `disclosure_research_read.py` + `disclosure_enrichment_projection.py` + `disclosure_dataset_snapshot.py` + dual-write/shadow/boundaries + `*_acceptance.py` + router/tests | **首个正式策略包目标**；E0 FIXED：formal_only 写；三域 MATCH → `cutover_allowed`；provider-field prefer canonical；canary DatasetSnapshot；feature_store typed enrichment PARTIAL；E smoke 可开 / 全量消融仍禁 |
...
| P0 | E0 gate+mirror-off 已绿但 canary-only + enrichment 历史仍 field-level PARTIAL | E smoke 可开；禁 institution_follow 全量消融；下一刀扩 accepted 分区（非本刀 mass backfill） |
