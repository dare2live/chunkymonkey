...
- **doctor stage-opt sensitivity surfacing**: `scripts/chunkyctl doctor --fast` 现在也会带出 `min_signals_sensitivity`，把 `min_signals=4` / `3` / `2` 对 `ready_keys`、`ready_coverage_pct`、`below_min_signals` 的提升幅度直接写进 `doctor`，分别是 `57.15% / 72,180`、`67.49% / 85,246` 和 `82.42% / 107,212`，但 controller 推荐仍然是 `P1 / upstream_candidate_supply`，说明阈值是有效杠杆而不是最终修复。
