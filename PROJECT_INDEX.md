...
- **Picture / survey freshness catch-up and survey safe-window guard**: `backend/scripts/build_picture_daily.py` 已按单日事务补 2026-05-27/28/29，每日 5,203 stocks x 4 tables，`mart_stock_picture_daily` max 到 2026-05-29；`backend/scripts/build_survey_features.py` 新增 `--write-start`，读取 lookback 与写入窗口分离，默认阻断 0 行窗口误删，新增 `backend/tests/sentiment/test_build_survey_features_script.py` 覆盖 lookback、显式 read-start、非法窗口和 empty-window override。真实库用 `--write-start 2026-05-21 --end 2026-05-29` 读 2026-03-23 起事件并写 25,897 rows，`mart_stock_survey_features` max 到 2026-05-29。`audit_end_to_end.py` 已 PASS with WARN: 24 total / 19 OK / 5 WARN / 0 FAIL；`audit_data_completeness.py` now PASS with WARN（0 FAIL / 2 WARN），survivorship gate PASS，不能恢复策略主线的原因只剩 recommendation PIT coverage 0 与 LHB coverage WARN。后续诊断补充：targeted PIT backfill 只把最新 cutoff 从 3 行补到 4 行，2-stock min-signals=3 smoke 仍 0 governance-pass rows，说明 current exact PIT 组合仍然太稀。
- **Stage / signal_context / technical_trigger safe-window freshness catch-up**: `backend/scripts/build_stage_formula_fitness.py` 新增 `--write-start` 与 `--stage-only`，只刷新 `fact_stock_technical_stage` 窗口，不误跑 `mart_stage_formula_fitness` 全量矩阵；`backend/scripts/build_signal_context.py` 新增可测试的 `build_signal_context()` 与 `--write-start`，把 120 日滚动计算预热起点和写入替换窗口拆开，空窗口默认拒删；`backend/scripts/build_formula_signals_history.py` 新增 `--write-start` 调用层过滤，计算可用宽窗口，写库只替换声明窗口，horizon evidence 默认不重算。真实库刷新: stage 25,894 rows、context 41,149 rows、trigger 17,950 signals，三表 max 均到 2026-05-29。2026-06-01 后续又把 `fact_stock_technical_stage` 早期窗口从 `2022-01-01` 开始补到 `2023-09-11`，新增 427,436 行并把 min date 推到 `2023-01-13`；`fact_signal_context` 早期窗口则补到 `2023-07-05`，新增 233,939 行并把 min date 推到 `2023-07-05`。`audit_stage_opt_candidate_supply.py` 在这些早期 backfill 之后仍维持 `1,381,657 raw_signal_rows / 733,083 filtered_signal_rows / 120,273 unique_keys / 57,986 ready_keys / 48.21% ready coverage / 62,287 below_min_signals` 不变，说明候选稀疏不是 2023 年初空窗导致。`audit_end_to_end.py` 已 PASS with WARN；freshness WARN 已收敛到 picture/survey/recommendation PIT，而 `audit_data_completeness.py` 是 PASS with WARN（0 FAIL / 2 WARN），不再阻断策略主线。
