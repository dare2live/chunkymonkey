...
- **System data-health snapshot fail-closed in `doctor`**: `backend/scripts/data_health_snapshot.py` is now a first-class system gate inside `scripts/chunkyctl doctor --fast`; dry-run JSON returns `FAIL` on 32 red / 4 yellow / 341 total and surfaces missing/stale-writer buckets, so startup health now blocks on the data-health red set before any business work can claim freshness.
