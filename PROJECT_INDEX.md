...
| Serve→derive closed loop | Law `analysis/serve_derive_closed_loop_law_20260723.md` + config `serve_derive_closed_loop.yaml`；`pipeline.closed_loop` + process `institution_profile` delta-gate；org `under_populated_accepted`；`check_serve_derive_closed_loop.py`；pytest `tests/test_pipeline_closed_loop.py` |
